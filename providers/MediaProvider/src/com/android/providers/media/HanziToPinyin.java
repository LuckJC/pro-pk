...
public class HanziToPinyin {
    ...
    public static HanziToPinyin getInstance() {
        synchronized (HanziToPinyin.class) {
            ...
            if (PinyinKeyUtils.hasChina()) {
                // Do self validation just once.
                if (DEBUG) {
                    MtkLog.d(TAG, "Self validation. Result: " + doSelfValidation());
                }
                sInstance = new HanziToPinyin(true);
                return sInstance;
            }
            MtkLog.w(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
    ...
    private static boolean doSelfValidation() {
        ...
        for (char c : UNIHANS) {
            ...
            if (cmp >= 0) {
                MtkLog.e(TAG, "Internal error in Unihan table. " + "The last string \"" + lastString
