...
public class UpdateKeyRequest {
    ...
    private void uniqueArtist(DatabaseHelper helper, SQLiteDatabase db, HashMap<String, List<Integer>> nameIdsMap) {
        ...
        deleteNotReferencedKey(helper, db, "artists", MediaStore.Audio.Media.ARTIST_ID, "audio",
                MediaStore.Audio.Media.ARTIST_ID);
        /// M: Clear helper.mArtistCache when delete not reference album key in artist table, so that
        /// when insert these audio to database again, provider will call getKeyIdForName to insert
        /// them to artist table(it will cause there are all unknown artist in Music when change locale
        /// after plug out sdcard and plug in it again).
        helper.mArtistCache.clear();
    }
    ...
    private void uniqueAlbum(DatabaseHelper helper, SQLiteDatabase db, HashMap<String, List<Integer>> nameIdsMap,
        ...
        deleteNotReferencedKey(helper, db, "albums", MediaStore.Audio.Media.ALBUM_ID, "audio",
                MediaStore.Audio.Media.ALBUM_ID);
        /// M: Clear helper.mAlbumCache when delete not reference album key in album table, so that
        /// when insert these audio to database again, provider will call getKeyIdForName to insert
        /// them to album table(it will cause there are all unknown album in Music when change locale
        /// after plug out sdcard and plug in it again).
        helper.mAlbumCache.clear();
    }
