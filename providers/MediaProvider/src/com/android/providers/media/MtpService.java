...
public class MtpService extends Service {
    ...
    public void onCreate() {
        ...
        synchronized (mBinder) {
            ...
            SXlog.w(TAG, "onCreate: volumes.length=" + volumes.length);
            for (int i = 0; i < volumes.length; i++) {
                String path = volumes[i].getPath();
                String state = mStorageManager.getVolumeState(path);
                /// M: ALPS00241636, add log for support MTP debugging @{
                SXlog.w(TAG, "onCreate: path of volumes[" + i + "]=" + path);
                SXlog.w(TAG, "onCreate: state of volumes[" + i + "]=" + state);
                /// M: @}
                if (Environment.MEDIA_MOUNTED.equals(state)) {
                    volumeMountedLocked(path);
                }
    ...
    private void updateDisabledStateLocked() {
        ...
        mMtpDisabled = (keyguardManager.isKeyguardLocked() && keyguardManager.isKeyguardSecure())
                || !isCurrentUser;
        if (LOGD) {
            Log.w(TAG, "updating state; isCurrentUser=" + isCurrentUser + ", mMtpDisabled="
                    + mMtpDisabled);
        }
    }
		...
		if (storage.getPath().equals(PATH_SDCARD0) && !storage.isRemovable()){
			...
			for (int i = 0; i < volumes.length; i++) {
				...
				if(removable && path.equals(PATH_SDCARD1)){
					...
					SXlog.d(TAG, "volume info, "  
							+ ", storageID : " + storageId 
							+ ",Path : " + path 
							+ ",removable : " + removable 
							+ ",Desc : " + description );
					// only this two is hard value, others dynamic get by path, storage id is not affected by swap issue
    ...
    private void addStorageLocked(StorageVolume volume) {
        ...
        if (volume.getPath().equals(ICUSB_STORAGE_1_MNT_POINT)){
            SXlog.d(TAG, "addStorageLocked: meet icusb storage " + storage.getPath() + " , and make it unshared");
            return;                        
        }
