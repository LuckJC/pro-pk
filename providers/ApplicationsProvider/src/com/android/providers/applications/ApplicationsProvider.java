...
//        qb.setTables(APPLICATIONS_LOOKUP_JOIN);
        /// M: For fuzzy search, query APPLICATIONS_TABLE directly
        qb.setTables(APPLICATIONS_TABLE);
        qb.setProjectionMap(columnMap);
        String orderBy = null;
        if (!zeroQuery) {
//            qb.appendWhere(buildTokenFilter(query));
            /// M: For fuzzy search, search app name contains 'query'
            qb.appendWhere(NAME + " like '%" + query + "%'");
        } else {
    ...
    private String getOrderBy(boolean zeroQuery) {
        ...
        if (!zeroQuery) {
            /// M: For fuzzy search, no "token_index" in APPLICATIONS_TABLE
