...
//MTK-END [mtk04170][111215]
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        //MTK-START [mtk04170][111215]
        private SQLiteStatement [] mCarrierIntertGeminiStatement = new SQLiteStatement[PhoneConstants.GEMINI_SIM_NUM];
        ...
        public DatabaseHelper(Context context) {
            ...
            for (int i = 0; i < PhoneConstants.GEMINI_SIM_NUM; i++) {
                mCarrierIntertGeminiStatement[i] = null;
            }
        }
...
//MTK-START [mtk80601][111215][ALPS00093395] add SIMInfo
            db.execSQL("CREATE TABLE " + SIMINFO_TABLE + " ( "
                    ...
                    + SimInfoManager.DISPLAY_NUMBER_FORMAT + " INTEGER NOT NULL DEFAULT "
                    + SimInfoManager.DISLPAY_NUMBER_DEFAULT + ","
                    + SimInfoManager.ICC_ID + " TEXT NOT NULL,"
                    + SimInfoManager.COLOR + " INTEGER DEFAULT " + SimInfoManager.COLOR_DEFAULT + ","
                    + SimInfoManager.SLOT + " INTEGER DEFAULT " + SimInfoManager.SLOT_NONE + ","
                    + SimInfoManager.OPERATOR + " TEXT,"
...
//MTK-END [mtk80601][111215][ALPS00093395] add SIMInfo
            ...
            if (FeatureOption.MTK_OMACP_SUPPORT == true) {
            ...
            if (oldVersion < (13 << 16 | 6)) {
                for (int i=0; i<PhoneConstants.GEMINI_SIM_NUM; ++i) {
                    try {
                        ...
                        e.printStackTrace();
                        Log.e(TAG, "Add ppp column fail with table " + CARRIERS_TABLE_GEMINI[i] + ".");
                    }
                }
                oldVersion = 13 << 16 | 6;
            ...
            if (oldVersion < (14 << 16 | 6)) {
                for (int i=0; i<PhoneConstants.GEMINI_SIM_NUM; ++i) {
                    try {
                        ...
                        e.printStackTrace();
                        Log.e(TAG, "Add mvno column fail with table " + CARRIERS_TABLE_GEMINI[i] + ".");
                    }
                }
                oldVersion = 14 << 16 | 6;
...
//MTK-END [mtk04170][111215]
        ...
        private void insertAddingDefaults(SQLiteDatabase db, String table, ContentValues row) {
            ...
            Boolean bTableEqual = false;
            for (int i=0; i<PhoneConstants.GEMINI_SIM_NUM; ++i) {
                if (CARRIERS_TABLE_GEMINI[i].equals(table)) {
                    bTableEqual = true;
                    String sqlCmd = ReplaceSqlStatement.CARRIER_INSERT + CARRIERS_TABLE_GEMINI[i];

                    if (FeatureOption.MTK_OMACP_SUPPORT) {
                        sqlCmd += ReplaceSqlStatement.CARRIER_MVNO_OMACP_GEMINI_TABLE_COLUMN;
                    } else {
                        sqlCmd += ReplaceSqlStatement.CARRIER_MVNO_GEMINI_TABLE_COLUMN; 
                    } 
                
                    if (mCarrierIntertGeminiStatement[i] == null) {
                        mCarrierIntertGeminiStatement[i] = db.compileStatement(sqlCmd);
                    }

                    bindStatement(mCarrierIntertGeminiStatement[i], row);
                    break;
                }
            } 
...
//            }
            ...
            //[mr2] mvno
            String mvnotype = row.getAsString(Carriers.MVNO_TYPE);
            if (mvnotype != null) {
                ss.bindString(ReplaceSqlStatement.MVNO_TYPE, mvnotype);
            } else {
                ss.bindNull(ReplaceSqlStatement.MVNO_TYPE);
            }
            //[mr2] mvno
            String mvnomatchdata = row.getAsString(Carriers.MVNO_MATCH_DATA);
            if (mvnomatchdata != null) {
                ss.bindString(ReplaceSqlStatement.MVNO_MATCH_DATA, mvnomatchdata);
            } else {
                ss.bindNull(ReplaceSqlStatement.MVNO_MATCH_DATA);
            }
            ...
            if (FeatureOption.MTK_OMACP_SUPPORT) {
                ...
                String ppp = row.getAsString(Carriers.PPP);
                if (ppp != null) {
                    ss.bindString(ReplaceSqlStatement.PPP, ppp);
                } else {
                    ss.bindNull(ReplaceSqlStatement.PPP);
                }
    ...
    public boolean onCreate() {
        ...
        iTelephony = ITelephonyEx.Stub.asInterface(ServiceManager.checkService("phoneEx"));
...
//MTK-END [mtk04170][111215]
    ...
    public Cursor query(Uri url, String[] projectionIn, String selection,
        ...
        switch (match) {
            ...
            case URL_SIMINFO_ID: {
                qb.setTables(SIMINFO_TABLE);
                qb.appendWhere(SimInfoManager._ID + "=" + ContentUris.parseId(url));
                break;
            }
            case URL_GPRSINFO: {
        ...
        switch (match)
        {
            ...
            case URL_SIMINFO_ID: {
                String selectionWithId =
                    (SimInfoManager._ID + "=" + ContentUris.parseId(url) + " ")
    ...
    public int update(Uri url, ContentValues values, String where, String[] whereArgs)
    {
        ...
        switch (match)
        {
            ...
            case URL_PREFERTETHERINGAPN:
            {
                if (values != null) {
                    if (values.containsKey(COLUMN_APN_ID)) {
                        setPreferredApnId(PREF_TETHERING_FILE_GEMINI[simId], values
                                .getAsLong(COLUMN_APN_ID));
                        count = 1;
                        urlType = URL_CARD_GEMINI[simId];
                    }
        ...
        if (count > 0) {
            ...
            switch (urlType) {
                ...
                case URL_SIMINFO_ID:
                    getContext().getContentResolver().notifyChange(
                            SimInfoManager.CONTENT_URI, null);
                    break;
                case URL_GPRSINFO:
                case URL_GPRSINFO_ID:
                    getContext().getContentResolver().notifyChange(
...
//MTK-END [mtk04170][111215]    
    private interface ReplaceSqlStatement {
        String CARRIER_INSERT = "INSERT INTO ";
        String CARRIER_MVNO_GEMINI_TABLE_COLUMN = 
            " ("    + Carriers.NAME + ", "
                        + Carriers.NUMERIC + ", "
                        + Carriers.MCC + ", "
                        + Carriers.MNC + ", "
        ...
        String CARRIER_MVNO_OMACP_GEMINI_TABLE_COLUMN = 
            " ("    + Carriers.NAME + ", "
                    + Carriers.NUMERIC + ", "
                    + Carriers.MCC + ", "
                    + Carriers.MNC + ", "
        ...
        int PPP = 21;
        int MVNO_TYPE =22;
        int MVNO_MATCH_DATA = 23;        
        int OMACP_OMACPID = 24;
        int OMACP_NAPID = 25;
        int OMACP_PROXYID = 26;
        int OMACP_CARRIER_ENABLED = 27;
        int OMACP_BEARER = 28;
        int OMACP_SPN = 29;
        int OMACP_IMSI = 30;
        int OMACP_PNN = 31;
        int CARRIER_ENABLED = 24;
        int BEARER = 25;
        int SPN = 26;
        int IMSI = 27;
        int PNN = 28;
