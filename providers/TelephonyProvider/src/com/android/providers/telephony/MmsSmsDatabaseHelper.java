...
//            "  WHERE threads._id = new." + Sms.THREAD_ID + "; ";
    ...
    private static final String SMS_UPDATE_THREAD_DATE_SENT =
            ...
            "        (SELECT date_sent * 1000 AS date_sent, thread_id FROM pdu " +
            "         WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = " + "new." + Sms.THREAD_ID + ") "+
            "         UNION SELECT date_sent, thread_id FROM sms " +
            "         WHERE thread_id = " + "new." + Sms.THREAD_ID + ") " +
            "     WHERE thread_id = " + "new." + Sms.THREAD_ID + " ORDER BY date_sent DESC LIMIT 1) " +
            "  WHERE threads._id = " + "new." + Sms.THREAD_ID + "; ";
    /// M: @}
...
//            "  WHERE (new.m_type=132 OR new.m_type=130 OR new.m_type=128) AND threads._id = new." + Mms.THREAD_ID + "; " +
    ...
    private static final String UPDATE_THREAD_SNIPPET_SNIPPET_CS_ON_DELETE =
            ...
            "   (SELECT date_sent FROM" +
            "     (SELECT date_sent * 1000 as date_sent, date * 1000 AS date, thread_id FROM pdu WHERE m_type=132 OR m_type=130 OR m_type=128" +
    ...
    private static final String SMS_UPDATE_THREAD_DATE_CT = "UPDATE threads SET date = "
        ...
        + "(SELECT date * 1000 as date FROM pdu WHERE (thread_id = new.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT date FROM sms WHERE (thread_id = new.thread_id))) " + "WHERE _id = new.thread_id;"
        + "UPDATE threads SET date_sent = "
        + "(SELECT max(date_sent) as date_sent FROM "
        + "(SELECT date_sent * 1000 as date_sent FROM pdu WHERE (thread_id = new.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT date_sent FROM sms WHERE (thread_id = new.thread_id))) " + "WHERE _id = new.thread_id;";

    private static final String SMS_UPDATE_THREAD_MESSAGE_COUNT_CT = "UPDATE threads SET message_count = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE thread_id = new.thread_id UNION SELECT _id FROM pdu WHERE thread_id = new.thread_id)) "
        + "WHERE _id = new.thread_id;"
        + "UPDATE threads SET readcount = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE (thread_id = new.thread_id AND (read = 1) AND (type != 3)) UNION SELECT _id FROM pdu WHERE (thread_id = new.thread_id AND read = 1 AND msg_box != 3))) "
        + "WHERE _id = new.thread_id;";
    ...
    private static final String SMS_UPDATE_THREAD_SNIPPET_CT = "UPDATE threads SET snippet = "
        ...
        + "(SELECT max(date) * 1000 as date, sub as snippet FROM pdu WHERE (thread_id = new.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT max(date), body as snippet FROM sms WHERE (thread_id = new.thread_id)) WHERE snippet is not null)) " + "WHERE _id = new.thread_id;"
        + "UPDATE threads SET snippet_cs = " + "(SELECT snippet_cs FROM (SELECT max(date), snippet_cs FROM "
        + "(SELECT max(date) * 1000 as date, sub_cs as snippet_cs FROM pdu WHERE (thread_id = new.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT max(date), 0 as snippet_cs FROM sms WHERE (thread_id = new.thread_id)))) " + "WHERE _id = new.thread_id;";

    // update the old thread status.
    private static final String SMS_UPDATE_OLD_THREAD_DATE_CT = "UPDATE threads SET date = "
        + "(SELECT max(date) as date FROM "
        + "(SELECT date * 1000 as date FROM pdu WHERE (thread_id = old.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT date FROM sms WHERE (thread_id = old.thread_id))) " + "WHERE _id = old.thread_id;"
        + "UPDATE threads SET date_sent = "
        + "(SELECT max(date_sent) as date_sent FROM "
        + "(SELECT date_sent * 1000 as date_sent FROM pdu WHERE (thread_id = old.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT date_sent FROM sms WHERE (thread_id = old.thread_id))) " + "WHERE _id = old.thread_id;";

    private static final String SMS_UPDATE_OLD_THREAD_MESSAGE_COUNT_CT = "UPDATE threads SET message_count = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE thread_id = old.thread_id UNION SELECT _id FROM pdu WHERE thread_id = old.thread_id)) "
        + "WHERE _id = old.thread_id;"
        + "UPDATE threads SET readcount = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE (thread_id = old.thread_id AND (read = 1) AND (type != 3)) UNION SELECT _id FROM pdu WHERE (thread_id = old.thread_id AND read = 1 AND msg_box != 3))) "
        + "WHERE _id = old.thread_id;";

    private static final String SMS_UPDATE_OLD_THREAD_SNIPPET_CT = "UPDATE threads SET snippet = "
        + "(SELECT snippet FROM (SELECT max(date), snippet FROM "
        + "(SELECT max(date) * 1000 as date, sub as snippet FROM pdu WHERE (thread_id = old.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT max(date), body as snippet FROM sms WHERE (thread_id = old.thread_id)) WHERE snippet is not null)) " + "WHERE _id = old.thread_id;"
        + "UPDATE threads SET snippet_cs = " + "(SELECT snippet_cs FROM (SELECT max(date), snippet_cs FROM "
        + "(SELECT max(date) * 1000 as date, sub_cs as snippet_cs FROM pdu WHERE (thread_id = old.thread_id AND m_type=132 OR m_type=130 OR m_type=128) "
        + "UNION SELECT max(date), 0 as snippet_cs FROM sms WHERE (thread_id = old.thread_id)))) " + "WHERE _id = old.thread_id;";
    /// M: @}
    ...
    static final int DATABASE_VERSION = 560400;
            ...
            if (rows > 0) {
                ...
                removeOrphanedAddresses(db);
                    ...
                    db.execSQL(
                        ...
                        "    (SELECT count(_id) FROM " +
                        "      (SELECT DISTINCT date * 1 AS normalized_date, _id, read FROM sms " +
                        "       WHERE ((read=1) AND thread_id = "+ thread_id +" AND (type != 3)) " +
                        "       UNION " +
                        "       SELECT DISTINCT date * 1000 AS normalized_date, pdu._id, read " +
                        "       FROM pdu LEFT JOIN pending_msgs ON pdu._id = pending_msgs.msg_id " +
                        "       WHERE ((read=1) AND thread_id = "+ thread_id +" AND msg_box != 3 AND (msg_box != 3 " +
                        "       AND (m_type = 128 OR m_type = 132 OR m_type = 130))) " +
                        "       UNION " +
                        "       SELECT DISTINCT date * 1 AS normalized_date, _id, read FROM cellbroadcast " +
                        "       WHERE ((read=1) AND thread_id = "+ thread_id +") ORDER BY normalized_date ASC)), " +
                        "  date_sent =" +
                        "    (SELECT date_sent FROM" +
                        "      (SELECT date_sent * 1000 as date_sent, date * 1000 AS date, thread_id FROM pdu WHERE m_type=132 OR m_type=130 OR m_type=128" +
                        "      UNION SELECT date_sent, date, thread_id FROM sms)" +
...
//                                            + " LIMIT 1";
                    ...
                    ///M: For ALPS01009088, update error by correct number of sending failed message(mms and sms)
                    if (c != null) {
                        setError = c.getCount();
                        c.close();
                    }
                    ...
                    in += ")";
                    String pending_query = "SELECT _id, err_type FROM pending_msgs WHERE err_type >= " + MmsSms.ERR_TYPE_GENERIC_PERMANENT + " AND msg_id " + in ;
                    Cursor c_pending = db.rawQuery(pending_query, null);
                    if (c_pending != null) {
                        setError += c_pending.getCount();
                        c_pending.close();
                    }
                    Log.d(TAG, "updateThread, set error with mms id " + in + "; setError = " + setError);
                    /// end
    ...
    public static void updateAllThreads(SQLiteDatabase db, String where, String[] whereArgs) {
        MmsLog.d(TAG, "updateAllThreads start");
        db.beginTransaction();
        try {
            if (where == null) {
                where = "";
        ...
        MmsLog.d(TAG, "updateAllThreads end");
    }
    ...
    protected static void removeOrphanedAddresses(SQLiteDatabase db) {
        ...
        MmsLog.d(TAG, "removeOrphanedAddresses, recipient ids = " + ids);
    ...
    private void createMmsTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_PDU + " (" +
                   ...
                   // status_ext: the status of the pdu which is downloading,the value maybe 0,1,2,3,4
                   // 0 for STATE_UNKNOWN, 1 for STATE_DOWNLOADING 2 for STATE_CANCELLING 3 for STATE_COMPLETE
                   // 4 for STATE_ABORTED
    ...
    private void createCommonTriggers(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TRIGGER sms_update_thread_on_add AFTER" + "  UPDATE OF " + Sms.THREAD_ID
            ...
            + SMS_UPDATE_THREAD_SNIPPET_CT + "END;");

        db.execSQL("CREATE TRIGGER sms_update_thread_on_split AFTER" + "  UPDATE OF " + Sms.THREAD_ID
            + " ON sms " + "BEGIN " + SMS_UPDATE_OLD_THREAD_DATE_CT + SMS_UPDATE_OLD_THREAD_MESSAGE_COUNT_CT
            + SMS_UPDATE_OLD_THREAD_SNIPPET_CT + "END;");
        /// M: @}
...
//                   "END;");
    ...
    public static void updateMultiThreads(SQLiteDatabase db, long[] deletedThreads) {
        MmsLog.d(TAG, "updateMultiThreads start, deletedThreads.length = " + deletedThreads.length);
        if(EncapsulatedFeatureOption.MTK_WAPPUSH_SUPPORT){
            db.delete(MmsSmsProvider.TABLE_THREADS,
                  "type <> ? AND _id NOT IN" +
                  "          (SELECT thread_id FROM sms where thread_id is not null " +
                  "           UNION SELECT thread_id FROM pdu where thread_id is not null " +
                  "           UNION SELECT DISTINCT thread_id FROM cellbroadcast where thread_id is not null)",
                  new String[] {String.valueOf(EncapsulatedTelephony.Threads.WAPPUSH_THREAD) });
        }else{
            db.delete(MmsSmsProvider.TABLE_THREADS,
                  "_id NOT IN" +
                  "          (SELECT thread_id FROM sms where thread_id is not null " +
        ...
        removeOrphanedAddresses(db);
        MmsLog.d(TAG, "delete obsolete threads and addresses end");
                ...
                if (wallpaperPath.exists()) {
                    ...
                    if (i > 0) {
                        for (int j = 0 ; j < i ; j++) {
                            if (threadWallpaperName.equals(oldFile[j])) {
                                boolean d = new File(WALLPAPER_PATH, oldFile[j]).delete();
                                MmsLog.d(TAG, "wallpaper " + threadWallpaperName + "isDelete " + d);
                            }
                        }
                    }
                }
        ...
        MmsLog.d(TAG, "updateMultiThreads end");
    }
    /// @}
}
