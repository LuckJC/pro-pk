...
public class DateSortedExpandableListAdapter implements ExpandableListAdapter {
    ...
    /* package */ boolean moveCursorToChildPosition(int groupPosition,
        ...
        /// M: add to fix 1092487, check if cursor window is null @{
        if (mCursor instanceof CursorWrapper) {
            CursorWrapper curWrapper = (CursorWrapper) mCursor;
            Cursor cursorTemp = curWrapper.getWrappedCursor();
            if (cursorTemp instanceof CrossProcessCursorWrapper) {
                CrossProcessCursorWrapper crossCursorWrapper = (CrossProcessCursorWrapper) cursorTemp;
                if (crossCursorWrapper.getWindow() == null) {
                    Xlog.d(XTAG_ENHANCE,  "DateSortedExpandableListAdapter:moveCursorToChildPosition(), " +
                             "mCursor's window is null, return false");
                    return false;
                }
           }
        }
