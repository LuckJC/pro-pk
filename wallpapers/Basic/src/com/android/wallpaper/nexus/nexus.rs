...
/// M: define plus size/half plus size for different dpi. @{
int gPluseSize;
int gHalfPluseSize;
/// @}
...
static void initPulse(struct pulse_s * pulse, int pulseType) {
    ...
    if (rsRand(1.f) > 0.5f) {
        pulse->originX = rsRand(gWidth * 2 / gPluseSize) * gPluseSize;
        pulse->dx = 0;
        if (rsRand(1.f) > 0.5f) {
            // Top
            pulse->originY = 0;
        ...
        pulse->originY = rsRand(gHeight / gPluseSize) * gPluseSize;
        pulse->dy = 0;
        if (rsRand(1.f) > 0.5f) {
            // Left
            pulse->originX = 0;
    ...
    for (int i=0; i<setSize; i++) {
        ...
        if (p->active != 0 && delta >= 0) {
           ...
           if (p->dx < 0) {
               ...
               float xx = x + (TRAIL_SIZE * gPluseSize);
               if (xx <= 0) {
                   initPulse(p, p->pulseType);
               } else {
                   setColor(p->color);
                   rsgBindTexture(gPFTexture, 0, gTPulse);
                   rsgDrawRect(x, y, xx, y + gPluseSize, 0.0f);
                   rsgBindTexture(gPFTexture, 0, gTGlow);
                   rsgDrawRect(x + gHalfPluseSize - HALF_GLOW_SIZE,
                       y + gHalfPluseSize - HALF_GLOW_SIZE,
                       x + gHalfPluseSize + HALF_GLOW_SIZE,
                       y + gHalfPluseSize + HALF_GLOW_SIZE,
                       0.0f);
               }
           } else if (p->dx > 0) {
               x += gPluseSize; // need to start on the other side of this cell
              ...
              if (xx >= gWidth * 2) {
                  initPulse(p, p->pulseType);
                   ...
                   setColor(p->color);
                   rsgBindTexture(gPFTexture, 0, gTPulse);
                   rsgDrawRect(xx, y, x, y + gPluseSize, 0.0f);
                   rsgBindTexture(gPFTexture, 0, gTGlow);
                   rsgDrawRect(x - gHalfPluseSize - HALF_GLOW_SIZE,
                       y + gHalfPluseSize - HALF_GLOW_SIZE,
                       x - gHalfPluseSize + HALF_GLOW_SIZE,
               ...
               /// @}
               rsgProgramVertexLoadTextureMatrix(&matrix);
               float yy = y + (TRAIL_SIZE * gPluseSize);
               if (yy <= 0) {
                  initPulse(p, p->pulseType);
               } else {
                   setColor(p->color);
                   rsgBindTexture(gPFTexture, 0, gTPulse);
                   rsgDrawRect(x, y, x + gPluseSize, yy, 0.0f);
                   rsgBindTexture(gPFTexture, 0, gTGlow);
                   rsgDrawRect(x + gHalfPluseSize - HALF_GLOW_SIZE,
                       y + gHalfPluseSize - HALF_GLOW_SIZE,
                       x + gHalfPluseSize + HALF_GLOW_SIZE,
            ...
            y += gPluseSize; // need to start on the other side of this cell
               rsMatrixRotate(&matrix, 90.0f, 0.0f, 0.0f, 1.0f);
               /// M: move texture matirx to correct position to remove artifacts. @{
               ...
               if (yy >= gHeight) {
                  initPulse(p, p->pulseType);
                   ...
                   setColor(p->color);
                   rsgBindTexture(gPFTexture, 0, gTPulse);
                   rsgDrawRect(x, yy, x + gPluseSize, y, 0.0f);
                   rsgBindTexture(gPFTexture, 0, gTGlow);
                   rsgDrawRect(x + gHalfPluseSize - HALF_GLOW_SIZE,
                       y - gHalfPluseSize - HALF_GLOW_SIZE,
                       x + gHalfPluseSize + HALF_GLOW_SIZE,
...
void addTap(int x, int y) {
    ...
    x = (x / gPluseSize) * gPluseSize;
    y = (y / gPluseSize) * gPluseSize;
...
int root() {
    ...
    // /M: ALPS01013637 enhance the performance to smooth the page switch
    // return 45;
    return 25;
}
