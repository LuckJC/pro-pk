...
public abstract class PhotoSource {
    ...
    /// M: clear queue and recycle bitmap when daydream exit @{
    protected void clearQueue() {
        log(TAG, "clear queue");
        mImageQueue.clear();
        Object[] keys = mImageMap.keySet().toArray();
        for (Object key : keys) {
            recycle((Bitmap) key);
        }
    }
            ...
            if (rawLongSide != -1 && rawShortSide != -1) {
                ...
                if (image != null && rawLongSide != -1 && rawShortSide != -1) {
                    ...
                    if (Math.abs(ratio - 1.0f) > 0.001) {
                        ...
                        /// M: ALPS00926152, options outWidth and outHeight can't equal 0 @{
                        options.outWidth = (int) Math.ceil(ratio * options.outWidth);
                        options.outHeight = (int) Math.ceil(ratio * options.outHeight);
                        /// @}
    ...
    protected int pickRandomStart(int total, int max) {
        if (max >= total) {
            ...
            return (Math.abs(mRNG.nextInt()) % (total - max)) - 1;
        }
    }
