...
public class ChargeTest extends TestBase implements Handler.Callback {
	...
	//private String BATTERYAVERAGECURRENT = "cat /sys/devices/platform/battery/charging_current";
	//private String BATTERYAVERAGECURRENT = "cat /sys/devices/platform/smapi/BAT0/current_now";
	...
	private String getChargerInfo(String location) {
		...
		Log.e("leihai","lo " + location);
		try {
			ret = CITShellExe.execCommand(cmd);
			Log.e("leihai","ret " + ret);
			if (ret != 0) {
		...
		Log.e("leihai","CITShellExe.getOutput() " + CITShellExe.getOutput());
	...
	private void getBatteryInfo(Intent intent, String s) {
		...
		if ("android.intent.action.BATTERY_CHANGED".equals(s)) {
			...
			if (j == 2) {
				if (i > 0) {
							tv_chargerinfo.setVisibility(View.VISIBLE);
					if (i == 1)
				...
				tv_chargerinfo.setVisibility(View.GONE);
