...
//		ct.initButton(btn_success);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ITelephonyEx teleMgr = ITelephonyEx.Stub.asInterface(ServiceManager.getService("phoneEx"));
		this.setContentView(R.layout.test_version);
		initAllControl();
		System.out.println("ddd->"+this.TELEPHONY_SERVICE);
...
*/
		...
		//s += s_citFlag; 
		if(sn.contains("_")){
			tv_CitFlag.setText(s+"0001");
		}else{
			tv_CitFlag.setText(s+"0000");
		}

		String invalid = getString(R.string.IMEIinfo);
		s = getString(R.string.IMEI1);
		imeiStr = ((TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE))
				.getDeviceId();///PhoneConstants.GEMINI_SIM_1
		if (imeiStr != null)
			s += imeiStr;
		else
			s += invalid;
		...
		imeiStr2 = ((TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE))
				.getDeviceId();//PhoneConstants.GEMINI_SIM_2
		if (imeiStr != null)
			s += imeiStr2;
		else
			s += invalid;
