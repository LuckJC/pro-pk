...
public class LightSenor extends TestBase implements Handler.Callback {
	...
	protected void onResume() {
		...
		flag = mSensorManager.registerListener(mLightSensorListener,sensor,
				Sensor.TYPE_LIGHT);
...

	{
		...
		public void onSensorChanged(SensorEvent arg0) {
			if (arg0.sensor.getType() == Sensor.TYPE_LIGHT) {
				float newX = Math.abs(arg0.values[SensorManager.DATA_X]);
				if (newX >= 120) {
					ct.setBrightness(255);					
				}else{
					ct.setBrightness(20);					
				}
				mtv_Light_info.setText(strLightinfo +"\n"+ newX);
			}
			...
			/* TextView textview = mtv_Light_info; StringBuilder stringbuilder =
			 ...
			 String s1 = stringbuilder1.append(f).toString(); textview.setText(s1);
