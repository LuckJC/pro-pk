...
public class RunInTest extends Activity implements OnItemSelectedListener,
	...
	private void showDialog() {
		...
		String sAgeFormat = getResources().getString(R.string.run_in_test_result_detail);  
		String detail = String.format(sAgeFormat, testdiruction);  
		builder.setMessage(detail);
		//ilder.setMessage("RUN IN 测试完成，测试时间 :\n" + testdiruction + "小时");
	...
	private void showDialogFail() {
		...
		AlertDialog.Builder builder = new Builder(this);
		builder.setMessage(R.string.run_in_test_result);
		builder.setTitle("RUN IN");
		// builder.setPositiveButton(
		builder.setPositiveButton(R.string.alert_dialog_ok,
				new android.content.DialogInterface.OnClickListener() {
	...
	public void onClick(View v) {
		...
		if (id == R.id.btn_begin) { // must
			...
			ct.cpFileToSD("test.mp3", is, RUNIN);
			...
			if (!ct.checkFile("test.mp3", RUNIN)) {
				CITTools.LOGV(debugFlag, TAG, RUNIN);
