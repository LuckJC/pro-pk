...
#define ALSPS_SET_PS_CALI3  						_IOR(ALSPS, 0x0f, int)
...
int get_ps_thd(void)
{
    ...
    if ((err == ioctl(handle, ALSPS_GET_PS_THD, &ps_thd_temp))) {
         __android_log_print(4, LOG_TAG, "PHILIPS_ATLAS get_ps_thd readfile file ps_dat error");
        return err;
    }
 __android_log_print(4, LOG_TAG, "PHILIPS_ATLAS get_ps_thd   close %d  far_away%d valid%d",ps_thd_temp.close,ps_thd_temp.far_away,ps_thd_temp.valid);  
...
#else
    if ((err = ioctl(handle, ALSPS_GET_ALS_RAW_DATA2, &als_dat))) {
         __android_log_print(4, LOG_TAG, "get_als_data readfile file ps_dat error");
        return err;
    }
#endif
