...
public class DateFormatter {
	...
	public CharSequence getDayName(LunarCalendar lc) {
		...
		int day = lc.getLunar(CompeterCalendar.LUNAR_DAY);
		if (day < 11) {
	...
	public CharSequence[] getFullDateInfo(LunarCalendar lc) {
		...
		// 取年柱,以春分为分界点
		int st_spring = CompeterCalendar.getSolarTerm(year, 3); // 立春
		...
		// 月柱,月柱以节令为界
		int st_monthFirst = (month == 1 ? st_spring : CompeterCalendar
				.getSolarTerm(year, month * 2 + 1));
		lMonth = (st_monthFirst > day ? -1 : 0);
		lMonth = (year - 1900) * 12 + +month + lMonth + 13;

		// 日柱,单纯的日循环
		lDay = (int) ((lc.getTimeInMillis() - CompeterCalendar.LUNAR_BASE_MILLIS) / CompeterCalendar.DAY_MILLIS) + 40;

		// 节气
		if (st_monthFirst == day) {
			solarTerm = month * 2;
		} else if (day > 15) {
			int st2 = CompeterCalendar.getSolarTerm(year, month * 2 + 2);
			if (st2 == day) {
				solarTerm = month * 2 + 1;
			}
		}
	...
	public CharSequence getMonthName(LunarCalendar lc) {
		StringBuilder result = new StringBuilder();
		if (lc.getLunar(CompeterCalendar.LUNAR_IS_LEAP) == 1) {
			result.append(getArrayString(R.array.chinesePrefix, 6));
		}
		int month = lc.getLunar(CompeterCalendar.LUNAR_MONTH);
		switch (month) {
		case 1:
	...
	public CharSequence getYearName(LunarCalendar lc) {
		StringBuilder result = new StringBuilder();
		int year = lc.getLunar(CompeterCalendar.LUNAR_YEAR);
