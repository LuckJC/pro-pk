...
//		Log.d(TAG, "handleSecretCode puyong 000 input: " +  input);
		//add by puyong@20140912 for  客户指定的指令字符串不符合规范
		String s1 = "*#66635*#";	
		if(len == 9 && s1.equals(input))
		{
			Intent intent = new Intent(TelephonyIntents.SECRET_CODE_ACTION, Uri.parse("android_secret_code://66635"));
			context.sendBroadcast(intent);
			
//			Log.d(TAG, "handleSecretCode puyong 111");
			
			return true;
		}

//		Log.d(TAG, "handleSecretCode puyong 001 input: " +  input);
		//add by puyong@20140912 for 客户指定的指令字符串不查看SAR值
		String s2 = "*#07#";	
		if(len == 5 && s2.equals(input))
		{
			Intent intent = new Intent(TelephonyIntents.SECRET_CODE_ACTION, Uri.parse("android_secret_code://07"));
			context.sendBroadcast(intent);
			
//			Log.d(TAG, "handleSecretCode puyong 122");
