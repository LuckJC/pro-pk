...
#ifdef MY_DUMP_IMG
        //dump buffer
        sprintf(saveFilename, "%s/%sTK%02d.bin", dumpImgFolder, prefixName, index+1);
...
#ifdef MY_DUMP_IMG
                 //dump buffer
                sprintf(fileName, "%s/%s%02d.bin", dumpImgFolder, prefixName, cand_idx);
...
bool MotionTrack::jpgDecode(char const *fileName, uint8_t *dstBuffer){
    ...
    #endif
    //decode one image
    if (!jpgToYV12(file_buffer, file_size, dstBuffer, outImgWidth, outImgHeight)) {
