...
public class ThreadPool {
    ...
    public static final int PARALLEL_THREAD_NUM = (CPU_CORES_NUM <= 2 ? CPU_CORES_NUM : CPU_CORES_NUM / 2);
    ...
    private static class ResourceCounter {
        ...
        public ResourceCounter(int v) {
            Log.i(TAG, "ResourceCounter max thread num " + v + " CPU Core " + CPU_CORES_NUM);
            value = v;
        }
    }
    ...
    private class Worker<T> implements Runnable, Future<T>, JobContext {
        ...
        public void run() {
            ...
            if (setMode(MODE_CPU)) {
                try {
                    Log.d(TAG, "ThreadPool job begin:"+mJob);
                    result = mJob.run(this);
                    Log.d(TAG, "ThreadPool job end:"+mJob);
                } catch (Throwable ex) {
