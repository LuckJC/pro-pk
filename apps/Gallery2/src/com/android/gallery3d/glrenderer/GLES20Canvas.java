...
public class GLES20Canvas implements GLCanvas {
    ...
    private static final int VERTEX_STRIDE_3 = COORDS_PER_VERTEX_3 * FLOAT_SIZE;
    // @]
    ...
    // M: 6592 panorama add @{
    private static final String MESH_VERTEX_3D_SHADER = ""
        + "uniform mat4 " + MATRIX_UNIFORM + ";\n"
        + "attribute vec3 " + POSITION_ATTRIBUTE + ";\n"
        + "attribute vec2 " + TEXTURE_COORD_ATTRIBUTE + ";\n"
        + "varying vec2 vTextureCoord;\n"
        + "void main() {\n"
        + "  vec4 pos = vec4(" + POSITION_ATTRIBUTE + ", 1.0);\n"
        + "  gl_Position = " + MATRIX_UNIFORM + " * pos;\n"
        + "  vTextureCoord = " + TEXTURE_COORD_ATTRIBUTE + ";\n"
        + "}\n";
    // @}
    ...
    public GLES20Canvas() {
        ...
        // M: 6592 panorama add @{
        int meshVertex3DShader = loadShader(GLES20.GL_VERTEX_SHADER, MESH_VERTEX_3D_SHADER);
        mMesh3DProgram = assembleProgram(meshVertex3DShader, textureFragmentShader, mMeshParameters);
        // @}
    ...
    public void setSize(int x, int y, int width, int height,
        ...
        int bottom = viewHeight - y - height;
        GLES20.glViewport(x, bottom, mWidth, mHeight);
        checkError();
        Matrix.setIdentityM(mMatrices, mCurrentMatrixIndex);
        Matrix.orthoM(mProjectionMatrix, 0, x, x + width, bottom, bottom + height, -1, 1);
        if (getTargetTexture() == null) {
            mScreenWidth = viewWidth;
            mScreenHeight = viewHeight;
            Matrix.translateM(mMatrices, mCurrentMatrixIndex, 0, tempHeight, 0);
            Matrix.scaleM(mMatrices, mCurrentMatrixIndex, 1, -1, 1);
        }
        GLES20.glScissor(x, bottom, width, height);
    }
    ...
    @Override
    public void saveScissorBox() {
        GLES20.glGetIntegerv(GLES20.GL_SCISSOR_BOX, mTempScissorBox, 0);
    }
    
    @Override
    public void restoreScissorBox() {
        GLES20.glScissor(mTempScissorBox[0], mTempScissorBox[1], 
    ...
    @Override
    public void drawMesh(BasicTexture tex, float degree, float x, float y, float z, int xyzBuffer,
            int uvBuffer, int count) {
        prepareTexture(tex, mMesh3DProgram, mMeshParameters);

        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, xyzBuffer);
        checkError();
        int positionHandle = mMeshParameters[INDEX_POSITION].handle;
        GLES20.glVertexAttribPointer(positionHandle, COORDS_PER_VERTEX_3, GLES20.GL_FLOAT, false,
                VERTEX_STRIDE_3, 0);
        checkError();
        ...
        rotate(degree, x, y, z);
        setMatrix(mMeshParameters, 0, 0, 1, 1);
    ...
    public void readPixels(int x, int y, int width, int height, int type, int format, Buffer buffer) {
        GLES20.glReadPixels(x, y, width, height, type, format, buffer);
    }
    @Override
