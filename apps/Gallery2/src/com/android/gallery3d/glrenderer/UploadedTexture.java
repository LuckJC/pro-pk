...
public abstract class UploadedTexture extends BasicTexture {
    ...
    public void updateContent(GLCanvas canvas) {
        if (!isLoaded()) {
            ...
            Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>UploadedTexture-uploadToCanvas");
        ...
        if (bitmap != null) {
            try {
                ...
                if (bWidth == texWidth && bHeight == texHeight) {
                    Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>UploadedTexture-initializeTexture");
                    ...
                    Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>UploadedTexture-texSubImage2D");
                    canvas.texSubImage2D(this, mBorder, mBorder, bitmap, format, type);
                    Trace.traceEnd(Trace.TRACE_TAG_APP);
                ...
                // M: It's possible that mBitmap has been set as null in recycle() when uploadToCanvas running,
                // so we need to check if mBitmap == null before freeBitmap, and synchronize it with recycle()
                synchronized (this) {
                    if (mBitmap != null) freeBitmap();
                }
            }
