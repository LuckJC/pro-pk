...
class BucketHelper {
    ...
    public static BucketEntry[] loadBucketEntries(
            JobContext jc, ContentResolver resolver, int type, Path path, String stereoAlbumName) {
        if (ApiHelper.HAS_MEDIA_PROVIDER_FILES_TABLE) {
            return loadBucketEntriesFromFilesTable(jc, resolver, type, path, stereoAlbumName);
        } else {
    ...
    private static BucketEntry[] loadBucketEntriesFromFilesTable(
        ...
        if (IS_STEREO_DISPLAY_SUPPORTED) {
            ///M: for build pass
            getAllStereoAlbum(buffer, resolver, type, path, stereoAlbumName);
        }
    ...
    private static void getAllStereoAlbum(ArrayList<BucketEntry> buffer, ContentResolver resolver,
            int type, Path path, String stereoAlbumName) {
        if (!IS_STEREO_DISPLAY_SUPPORTED) {
            return;
        }
        ...
        String whereGroup = MediatekFeature.getOnlyStereoWhereClause(
            MediatekFeature.ALL_DRM_MEDIA & path.getMtkInclusion());
        Log.i(TAG,"getAllStereoAlbum:whereGroup=" + whereGroup);

        Cursor cursor = resolver.query(getFilesContentUri(),
                PROJECTION_BUCKET, whereGroup, null, null);
