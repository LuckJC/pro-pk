        ...
        switch (mMatcher.match(path)) {
            case SNAIL_ALBUM:
                ...
                if (IS_DRM_SUPPORTED || IS_STEREO_DISPLAY_SUPPORTED) {
                    item = (MediaItem) dataManager.getMediaObject(itemPath, mtkInclusion);
                } else {
                    item = (MediaItem) dataManager.getMediaObject(itemPath);
                }
                return new SnailAlbum(path, (SnailItem)item);
            case SNAIL_ITEM: {
