...
import com.mediatek.gallery3d.videothumbnail.AbstractVideoGenerator;
...
public abstract class LocalMediaItem extends MediaItem {
    ...
    /// M: vtsp @{
    // thumbnail play type constants
    public static int TPT_UNKNOWN = 0;
    public static int TPT_NO_PLAY =1;
    public static int TPT_DIRECT_PLAY = 2;
    public static int TPT_GENERATE_PLAY = 3;
    public int thumbnailPlayType;
    
    public AbstractVideoGenerator mVideoGenerator;
    // cause generator is often not initialized at a proper time
    // I try to include this interface to init player in vstp control system
    // just a trial ...
    public abstract AbstractVideoPlayer getVideoPlayer();
    protected abstract void initThumbnailPlayType();
    protected abstract void initVideoGenerator();

    public void prepareThumbnailPlay() {
        if (TPT_UNKNOWN == thumbnailPlayType) {
            // compatible with existed generator gaining way
            if (null == mVideoGenerator) {
                initVideoGenerator();
            }
            initThumbnailPlayType();
        }
    }

    public void invalidateThumbnailPlayType() {
        thumbnailPlayType = TPT_UNKNOWN;
    }
    /// @}
}
