...
public class MenuExecutor {
    ...
    private void stopTaskAndDismissDialog() {
        ...
        if (mTask != null) {
            ...
            if (mDialog != null && mDialog.isShowing() && !mActivity.isDestroyed()) mDialog.dismiss();
            mDialog = null;
            mTask = null;
        }
    }
    ...
    public static void updateMenuOperation(Menu menu, int supported) {
        ...
        /// M: masked for KK, replace with KK print feature
        /*
        ...
        */
        ...
        boolean supportAutoConvOn = (supported & MediaObject.SUPPORT_AUTO_CONV_ON) != 0;
        ...
        */
        ...
        setMenuItemVisible(menu, R.id.action_adjust_convergence, supportConvTuning);
        if (supportStereoMode) {
            MenuItem item = menu.findItem(R.id.action_switch_auto_convergence);
            if (null != item) item.setChecked(supportAutoConvOn);
        }
        //only true stereo image can show ac switch
        boolean showACMenu = supportStereoMode && (supported & MediaObject.SUPPORT_CONVERT_TO_3D) == 0 && supportAutoConv;
        setMenuItemVisible(menu, R.id.action_switch_auto_convergence, showACMenu);
    ...
    ///M: add public for ConShots
    public static void setMenuItemVisible(Menu menu, int itemId, boolean visible) {
    ...
    public void onMenuClicked(int action, ProgressListener listener,
        ...
        switch (action) {
            ...
            case R.id.action_edit: {
                ...
                if(ContainerHelper.isConShotImage(obj)){
    ...
    public void startAction(int action, int title, ProgressListener listener,
        ...
        /// M: added for ConShots
        ArrayList<Path> ids;
        if(action == R.id.action_best_shots){
    ...
    private class MediaOperation implements Job<Void> {
        ...
        public Void run(JobContext jc) {
            ...
            try {
               ...
               MtkLog.i("Gallery2PerformanceTestCase2",
                       "[Performance Auto Test][Gallery_011] Delete 100 photos test end ["
    ...
    public static void updateSupportedMenuEnabled(Menu menu, int supported, boolean enabled) {
        ...
        // added for stereo 3D display
        if (supportStereoMode) {
            setMenuItemEnable(menu, R.id.action_switch_stereo_mode, enabled);
        }
        if (supportConvTuning) {
            setMenuItemEnable(menu, R.id.action_adjust_convergence, enabled);
        }
    }
