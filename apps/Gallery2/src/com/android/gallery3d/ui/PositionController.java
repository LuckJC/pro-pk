...
public class PositionController {
    ...
    /// M: when move to Camera, this flag will be set as true, 
    // then skip snapback animation to fix Camera performance issue. 
    ...
    public void scrollPage(int dx, int dy) {
        ...
        if ((!mImageSwitchable || !mHasPrev) && x > mBoundRight) {
            int pixels = x - mBoundRight;
            mListener.onPull(pixels, EdgeView.LEFT);
            x = mBoundRight;
        } else if ((!mImageSwitchable || !mHasNext) && x < mBoundLeft) {
    ...
    private class Box extends Animatable {
        ...
        public boolean startSnapback() {
            ...
            if (mCurrentY != y || mCurrentScale != scale) {
                ///M:camera preview don't need do animation @{
                if(mIsCamera) {
                    mCurrentY = y;
                    mCurrentScale = scale;
                    return true;
    ...
    private float getInitRate() {
        float initRate = 0.0f;
        Box b = mBoxes.get(0);
        if (b.mImageW > 0 && b.mImageH > 0 && 
            mViewW > 0 && mViewH > 0) {
            initRate = Math.min((float) mViewW / b.mImageW, 
                                (float) mViewH / b.mImageH);
        }
        return initRate;
    }

    public boolean isAtInitRate() {
        Box b = mBoxes.get(0);
        return b.mCurrentScale <= getInitRate() + 0.001f;
    }
    ...
    public float largerInitRate() {
        ...
        if (b.mImageW > 0 && b.mImageH > 0 && 
            ...
            initRate = Math.max(initRate,
                          Math.min((float) mViewW / b.mImageH, 
                                   (float) mViewH / b.mImageW));
    ...
    public boolean willScaleOutOf3DMode(float scale) {
        Box b = mBoxes.get(0);
        if (getTargetScale(b) * scale > getSuggestLargerZoomRate()) {
            return true;
        } else {
            return false;
        }
    ...
    public float getSuggestLargerZoomRate() {
        ...
        if (b.mImageW > 0 && b.mImageH > 0 && 
            mViewW > 0 && mViewH > 0) {
            int maxLength = b.mImageW > b.mImageH ? b.mImageW : b.mImageH;
            int minLength = b.mImageW < b.mImageH ? b.mImageW : b.mImageH;
            ...
            if (isPan) {
                ...
                //zoom rate is 4.0.
                maxRate = Math.max((float) mViewW / b.mImageW, 
                                   (float) mViewH / b.mImageH);
                maxRate = Math.max(maxRate,
                              Math.max((float) mViewW / b.mImageH, 
                                       (float) mViewH / b.mImageW));
            } else {
    ...
    public float getSuggestZoomRate() {
        ...
        if (b.mImageW > 0 && b.mImageH > 0 && 
            ...
            if (isPan) {
                rate = Math.max((float) mViewW / b.mImageW, 
                                   (float) mViewH / b.mImageH);
                rate = Math.min(rate,
                              Math.max((float) mViewW / b.mImageH, 
                                       (float) mViewH / b.mImageW));
            } else {
                rate = (float) mViewH / b.mImageH;
    ...
    public boolean isInStereoRate() {
        Box b = mBoxes.get(0);
        return b.mCurrentScale <= getSuggestLargerZoomRate();
    }
    ...
    public boolean isAtMaxStereoScale() {
        Box b = mBoxes.get(0);
        return isAlmostEquals(b.mCurrentScale, getSuggestLargerZoomRate());
    }
    ...
    public void resetToSuggestedScale() {
        ...
        Log.v(TAG,"getSuggestZoomRate:isPan="+isPan);
        if (isPan) {
            startAnimation(mPlatform.mDefaultX, 0, getSuggestZoomRate(), ANIM_KIND_ZOOM);
        } else {
            resetToFullView();
        }
    }

    // this variable indicate that weather user can switch image
    // it is a suppliment for mHasPrev and mHasNext
    private boolean mImageSwitchable = true;

    public void setImageSwitchable(boolean switchable) {
        mImageSwitchable = switchable;
    }
    ...
    //record whether current status is camera preview
