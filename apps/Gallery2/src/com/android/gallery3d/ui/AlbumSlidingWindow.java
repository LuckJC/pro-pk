...
public class AlbumSlidingWindow implements AlbumDataLoader.DataListener, VideoThumbnailSourceWindow {
    @SuppressWarnings("unused")
    private static final String TAG = "Gallery2/AlbumSlidingWindow";

    private static final int MSG_UPDATE_ENTRY = 0;
    // M: Dynamic control thread number according to CPU Cores from constant number
    ...
    public static class AlbumEntry implements VideoThumbnailSourceWindow.DataEntry {
    ...
    private void prepareSlotContent(int slotIndex) {
        ...
        if (MediatekFeature.isPanorama3DSupported()) {
            entry.isPanorama = entry.item != null ? entry.item.isPanorama() : false;
        } else {
        // @}
