...
public class GLView {
    ...
    protected void dispatchGenericMotionEvent(MotionEvent event) {
        for (int i = getComponentCount() - 1; i >= 0; --i) {
          GLView component = getComponent(i);
          if (component.getVisibility() != GLView.VISIBLE) continue;
          if (component != null) {
              component.onGenericMotionEvent(event);
          }
        }
    }
   ...
   /// }@
    ...
    // M: Added for stereo display feature to notify current and
    // children view that stereo layout has changed.
    public void changeStereoLayout(int toStereoLayout) {
        onStereoLayoutChanged(toStereoLayout);
        for (int i = 0, n = getComponentCount(); i < n; ++i) {
            getComponent(i).onStereoLayoutChanged(toStereoLayout);
        }
    }

    // M: Added for stereo display feature.
    // GLView may do some operations when stereo layout change
