...
import com.mediatek.gallery3d.stereo.StereoPassHelper.StereoPass;
...
public class GLRootView extends GLSurfaceView
    ...
    /// M: add for performance test case 2
    ...
    private final boolean DBG_LAUNCH_TIME = MtkLog.DBG_LAUNCH_TIME;
    ...
    private boolean supportSW3D;

    private long mLastDrawFinishTime;
    private boolean mInDownState = false;
    private boolean mFirstDraw = true;

    // M: avoid NE after surfaceDestroy
    ...
    public GLRootView(Context context, AttributeSet attrs) {
        ...
        //M: this is for emulator which OpenGL version should be selected.
        //ro.kernel.qemu.gles = true, means Use host GPU enable.
        //supportSW3D = true, means SW3D support OpenGL 2.0.
        if (FeatureOption.MTK_EMULATOR_SUPPORT) {
            ...
            supportSW3D = version.startsWith("2");
            Log.d(TAG, "ro.kernel.qemu.gles = " + "1".equals(SystemProperties.get("ro.kernel.qemu.gles")) + ", supportSW3D = " + supportSW3D);
            setEGLContextClientVersion((ApiHelper.HAS_GLES20_REQUIRED && (supportSW3D || "1".equals(SystemProperties.get("ro.kernel.qemu.gles")))) ? 2 : 1);
        }else {
    ...
    @Override
    public void addGLContextRequiredOnGLIdleListener(OnGLIdleListener listener) {
        synchronized (mGLContextRequiredIdleListeners) {
            mGLContextRequiredIdleListeners.addLast(listener);
            requestRender();
        }
    }
    ...
    public void onSurfaceCreated(GL10 gl1, EGLConfig config) {
        ...
        try {
            ...
            //supportSW3D = true, means SW3D support OpenGL 2.0.
            if (FeatureOption.MTK_EMULATOR_SUPPORT) {
                mCanvas = (ApiHelper.HAS_GLES20_REQUIRED && (supportSW3D || "1".equals(SystemProperties.get("ro.kernel.qemu.gles")))) ? new GLES20Canvas() : new GLES11Canvas(gl);
            }else {
    ...
    private void outputFps() {
        ...
        } else if (DEBUG_FPS && (now - mFrameCountingStart) > 1000000000) {
            Log.d(TAG, "fps: " + (double) mFrameCount * 1000000000 / (now - mFrameCountingStart));
            mFrameCountingStart = now;
            mFrameCount = 0;
        } else if (FPS_PERFORMANCE && mFrameCount != 0) {
            Log.i("Gallery2PerformanceTestCase2", "[Performance Auto Test] Gallery render fps = " + (int) mFrameCount
    ...
    // M: add for launch performance @{
    public static boolean sVideoThumbnailPlaying = false;
    // @}

    private void onDrawFrameLocked(GL10 gl) {
        if (DEBUG_FPS || FPS_PERFORMANCE) outputFps();
        ...
        /// M: queue OnGLIdleListener that require a GL context (to be refactored) @{
        synchronized (mGLContextRequiredIdleListeners) {
            if (!mGLContextRequiredIdleListeners.isEmpty()) mGLContextRequiredIdleRunner.enable();
        }
        /// @}
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        /// M: virtual device
        GLRootView.DThumbClean = (event.getDeviceId() == 0);
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        ...
        try {
            if (mContentView != null) {
                mContentView.dispatchGenericMotionEvent(event);
            }
        } finally {
            mRenderLock.unlock();
        }
...
/// }@
    ...
    private int mTargetStereoLayout = StereoHelper.STEREO_TYPE_2D;
    private int mCurrentStereoLayout = StereoHelper.STEREO_TYPE_2D;
    ...
    public void setStereoLayout(int stereoLayout) {
        if (IS_STEREO_DISPLAY_SUPPORTED) {
            if (mTargetStereoLayout == stereoLayout) {
                return;
            }
            mTargetStereoLayout = stereoLayout;
            Log.d(TAG, "setStereoLayout: " + stereoLayout);
        }
    }

    @Override
    public int getStereoLayout() {
        return mCurrentStereoLayout;
    }
    ...
    private void setSurfaceStereoMode(int stereoLayout) {
        // set SurfaceView stereo mode
        StereoHelper.setSfStereoLayout(this, stereoLayout);

        if (mContentView != null) {
           // to view tree to prepare for stereo mode changes
           mContentView.changeStereoLayout(stereoLayout);
        }
    }

    private void prepareForFirstPath() {
        //set current pass id
        mStereoPassId = StereoHelper.STEREO_DISPLAY_FIRST_PASS;
    }

    private void prepareForSecondPath() {
        //set current pass id
        mStereoPassId = StereoHelper.STEREO_DISPLAY_SECOND_PASS;
    }

    private void resetStereoPath() {
        //reset canvas mode to normal mode
        mCanvas.setSize(getWidth(), getHeight());
        //set current pass id to be normal
        mStereoPassId = StereoHelper.STEREO_DISPLAY_NORMAL_PASS;
    }
    ...
    private boolean prepareForStereo() {
        ...
        if (mCurrentStereoLayout != mTargetStereoLayout) {
            mCurrentStereoLayout = mTargetStereoLayout;
        ...
        //now, left
        if (StereoHelper.isStereo(mCurrentStereoLayout)) {
            prepareForFirstPath();
        }
    ...
    private void finishForStereo(GL10 gl, boolean updateStereoMode) {
        ...
        if (updateStereoMode) {
            setSurfaceStereoMode(mCurrentStereoLayout);
        }
    }

    private void transformForStereo() {
        if (IS_STEREO_DISPLAY_SUPPORTED &&
            StereoHelper.isStereo(mCurrentStereoLayout)) {
            StereoPassHelper.prepareForStereoPass(mCanvas, mStereoPassId,
                    mCurrentStereoLayout, getWidth(), getHeight());
        }
    }
    ...
    /// M: add OnGLIdleListener that require a GL context (to be refactored) @{
    private final ArrayDeque<OnGLIdleListener> mGLContextRequiredIdleListeners =
        new ArrayDeque<OnGLIdleListener>();

    private final GLContextRequiredIdleRunner mGLContextRequiredIdleRunner = new GLContextRequiredIdleRunner();

    private class GLContextRequiredIdleRunner implements Runnable {
        // true if the idle runner is in the queue
        private boolean mActive = false;
        ...
        public void run() {
            ...
            synchronized (mGLContextRequiredIdleListeners) {
                mActive = false;
                if (mGLContextRequiredIdleListeners.isEmpty()) return;
                listener = mGLContextRequiredIdleListeners.removeFirst();
            }
            mRenderLock.lock();
            boolean keepInQueue;
            try {
            ...
            synchronized (mGLContextRequiredIdleListeners) {
                if (keepInQueue) mGLContextRequiredIdleListeners.addLast(listener);
                if (!mRenderRequested && !mGLContextRequiredIdleListeners.isEmpty()) enable();
            }
        }
    ...
    /// M: virtual device
    public static boolean DThumbClean = false;
    /// @}
}
