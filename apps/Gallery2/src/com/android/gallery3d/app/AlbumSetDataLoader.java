...
public class AlbumSetDataLoader {
    ...
    /// M: add for hide camera button when loading @{
    private static final int MSG_LOAD_CONTENT_NOT_EMPTY = 4;
    /// @}
    ...
    public interface AlbumSetLoadingListener extends LoadingListener {
        public void onLoadingContentNotEmpty();
    }
    /// @}
            ...
            public void handleMessage(Message message) {
                switch (message.what) {
                    ...
                    case MSG_LOAD_FINISH:
                        ...
                        return;
                    /// M: add for hide camera button when loading @{
                    case MSG_LOAD_CONTENT_NOT_EMPTY:
                        if (mLoadingListener != null) mLoadingListener.onLoadingContentNotEmpty();
                        return;
                    /// @}
                }
            }
    ...
    public void resume() {
        MtkUtils.traceStart("AlbumSetDataLoader:resume");
        mSource.addContentListener(mSourceListener);
        /// M: refresh cluster name when switch system language
        mSource.reloadResources();
        mReloadTask = new ReloadTask();
        mReloadTask.start();
        MtkUtils.traceEnd("AlbumSetDataLoader:resume");
    }
    ...
    private class GetUpdateInfo implements Callable<UpdateInfo> {
        ...
        public UpdateInfo call() throws Exception {
            Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>AlbumSetDataLoader-GetUpdateInfo.run");
    ...
    private class UpdateContent implements Callable<Void> {
        ...
        public Void call() {
            ...
            Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>AlbumSetDataLoader-UpdateContent.run");
    ...
    private class ReloadTask extends Thread {
        ...
        public void run() {
            ...
            while (mActive) {
                ...
                Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>AlbumSetDataLoader-reload");
                long version = mSource.reload();
                Trace.traceEnd(Trace.TRACE_TAG_APP);
                MtkUtils.traceStart("AlbumSetDataLoader:GetUpdateInfo");
                UpdateInfo info = executeAndWait(new GetUpdateInfo(version));
                MtkUtils.traceEnd("AlbumSetDataLoader:GetUpdateInfo");
                ...
                if (info.version != version) {
                    ...
                    /// M: add for hide camera button when loading @{
                    if (info.size > 0) {
                        mMainHandler.sendEmptyMessage(MSG_LOAD_CONTENT_NOT_EMPTY);
                    }
                    /// @}
                ...
                if (info.index != INDEX_NONE) {
                    ...
                    MtkUtils.traceStart("AlbumSetDataLoader:getCoverMediaItem");
                    Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>AlbumSetDataLoader-getCoverMediaItem");
                    info.cover = info.item.getCoverMediaItem();
                    Trace.traceEnd(Trace.TRACE_TAG_APP);
                    MtkUtils.traceEnd("AlbumSetDataLoader:getCoverMediaItem");
                    MtkUtils.traceStart("AlbumSetDataLoader:getTotalMediaItemCount");
                    Trace.traceBegin(Trace.TRACE_TAG_APP, ">>>>AlbumSetDataLoader-getTotalMediaItemCount");
                    info.totalCount = info.item.getTotalMediaItemCount();
                    Trace.traceEnd(Trace.TRACE_TAG_APP);
                    MtkUtils.traceEnd("AlbumSetDataLoader:getTotalMediaItemCount");
                }
                MtkUtils.traceStart("AlbumSetDataLoader:UpdateContent");
                executeAndWait(new UpdateContent(info));
                MtkUtils.traceEnd("AlbumSetDataLoader:UpdateContent");
            }
            updateLoading(false);
        }
