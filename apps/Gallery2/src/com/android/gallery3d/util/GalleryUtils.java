...
public class GalleryUtils {
    ...
    ///M: added for WFD. @{
    private static AlertDialog mWfdDialog = null;
    private static boolean mIsShowWfdDialog = true;
    /// @}
    ///M: added for HDMI.
    private static AlertDialog mHdmiDialog = null;
    private static boolean mIsShowHdmiDialog = true;
    /// M: Because ReDecode from origin image to improve image quality in full image display, 
    /// so we need set display real resolution max size to thumbnail size
    public static int REAL_RESOLUTION_MAX_SIZE;
    //M: redefine high resolution size 
    private static final int HIGH_RESOLUTION_SIZE = 1600;
    ///M: add low Ram device feature option
    public static boolean Is_Low_Ram_Device;
    ...
    public static void initialize(Context context) {
        ...
        // M: 6592 panorama add @{
        initializePanoramaScreenNailSize(metrics);
        // @}
        // M: Get real resolution size @{
        DisplayMetrics reMetrics = new DisplayMetrics();
        wm.getDefaultDisplay().getRealMetrics(reMetrics);
        REAL_RESOLUTION_MAX_SIZE = Math.max(reMetrics.widthPixels, reMetrics.heightPixels);
        // @}
        // M: 6592 FHD add
        initializeTiledTxtureSize(metrics);
        Is_Low_Ram_Device = ((ActivityManager) context.getSystemService("activity")).isLowRamDevice();
        Log.i(TAG, "initialize Is_Low_Ram_Device " + Is_Low_Ram_Device);
    }
  ...
  // M: 6592 panorama add @{
    private static void initializePanoramaScreenNailSize(DisplayMetrics metrics) {
        PanoramaHelper.setPanoramaScreenNailSize(metrics.widthPixels, metrics.heightPixels);
    }
    // @}
  ...
  /// M: added for delete Items. @{
    private static ArrayList<String> getItemList (ContentResolver contentResolver,
            Uri srcContentUri,String mWhereClauseForDelete, String[] mWhereClauseArgsForDelete) {
        ArrayList<String> list = new ArrayList<String>();
        Cursor cursor = null;
        try {
            cursor = contentResolver.query(
                    srcContentUri, new String[] {
                            FileColumns.DATA}, mWhereClauseForDelete,
                    mWhereClauseArgsForDelete, null);
            if (cursor != null) {
    ...
    public static void deleteItems(ContentResolver contentResolver,
            Uri srcContentUri,String mWhereClauseForDelete, String[] mWhereClauseArgsForDelete) {
        ArrayList<String> list = getItemList(contentResolver, srcContentUri, mWhereClauseForDelete, mWhereClauseArgsForDelete);
        if (list != null) {
            int size = list.size();
            try {
                for (int i = 0; i <= size; i++) {
                    Log.d(TAG," filepath="+list.get(i));
                    File fileC = new File(list.get(i));
                    fileC.delete();
                }
            } catch (Exception e) {
                e.printStackTrace();
