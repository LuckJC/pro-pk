...
public class FilterShowActivity extends FragmentActivity implements OnItemClickListener,
    ...
    private static String TAG = "Gallery2/FilterShowActivity";
    ...
    private void fillBorders() {
        ...
        */
        ...
        for (FilterRepresentation representation : borders) {
            /// M: names seems unreasonable, back to JB design @{
            /*
            if (representation.getTextId() != 0) {
                representation.setName(getString(representation.getTextId()));
            }
    ...
    private class LoadHighresBitmapTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            if (master.supportsHighRes()) {
                ...
                /// M: considering height when deciding highresPreviewSize @{
    ...
    void resetHistory() {
        ///M:reset mirror and rotate data@{
        FiltersManager filtersManager = FiltersManager.getManager();
        ArrayList<FilterRepresentation> filtersRepresentations = filtersManager.getTools();
        for (FilterRepresentation representation : filtersRepresentations) {
            if (representation instanceof FilterRotateRepresentation) {
    ...
    /// M: exit if file to edit has been deleted
    private boolean needCheckInvalid = true;
    private boolean finishIfInvalidUri() {
        if (!needCheckInvalid) return false;
        Uri uri = null;
        Intent intent = getIntent();
        if (intent != null) {
            uri = intent.getData();
        ...
        if (uri != null) {
            ...
            try {
                ...
                Log.e(LOGTAG, "Exception at finishIfInvalidUri()", e);
            } finally {
                if (cur != null) {
                    cur.close();
                }
    ...
    /// M: restrict frequency of adding version @{
