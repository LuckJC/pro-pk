...
public class WfdPowerSaving {
    ...
    private boolean mIsExtensionEnable = false;
    ...
    public WfdPowerSaving(final View rootView, final MovieActivity movieActivity, 
                                final MTKVideoView videoview, final Handler handler) {

        mPowerManager = (PowerManager)movieActivity.getSystemService(Context.POWER_SERVICE);
        
        mWindow = movieActivity.getWindow();
        WindowManager.LayoutParams lp = mWindow.getAttributes();
        mScreenBrightness = lp.screenBrightness;

        // Get the media router service.        
        mMediaRouter = (MediaRouter)movieActivity.getSystemService(Context.MEDIA_ROUTER_SERVICE);
        ...
        if(isExtensionEnable()) {
            ((ViewGroup)mRootView).removeView(mVideoView);
            updatePresentation();
    ...
    public void release() {
        ...
        if(isExtensionEnable()) {
          Toast.makeText(mMovieActivity.getApplicationContext(),
                  mMovieActivity.getString(R.string.wfd_disconnected),
    ...
    private int getPowerSavingMode() {
       ...
       MtkLog.v(TAG, "getPowerSavingMode mode = " + mode);
        if ((mode >= EXTENSION_MODE_LIST_START)
           ...
           mIsExtensionEnable = true;
       } else {
           mIsExtensionEnable = false;
       }
       return mode;
    }
    ...
    public void refreshPowerSavingPara() {
        ...
        if(isExtensionEnable()) {
          ...
          if(mPresentation != null) {
                ...
                mPresentation = null;
                ((ViewGroup)mRootView).addView(mVideoView,0);
                unregisterOnSystemUiVisibilityChangeListener();
          }
      }
    }
       ...
       private BroadcastReceiver mWfdReceiver = new BroadcastReceiver() {
           public void onReceive(Context context, Intent intent) {
               ...
               if (action.equals(DisplayManager.ACTION_WIFI_DISPLAY_STATUS_CHANGED)) {
                   ...
                   MtkLog.v(TAG, "mWfdReceiver onReceive action: " + status.getActiveDisplayState());
                   if(status.getActiveDisplayState() == WifiDisplayStatus.DISPLAY_STATE_NOT_CONNECTED) {
                       stopPowerSaving();
                   }
               }
           };
...
//         updatePresentation();
    ...
    */
    private final  class WfdPresentation extends Presentation {
     ...
     public WfdPresentation(Context context, Display display, SurfaceView surfaceview) {
         super(context, display);
     ...
     protected void onCreate(Bundle savedInstanceState) {
         ...
         mRoot.addView(mSurfaceView,wrapContent);
     }
     ...
     public void removeSurfaceView() {
        ((ViewGroup)mRoot).removeView(mSurfaceView);
     }
    }
}
