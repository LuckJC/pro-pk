...
public class SubTitleView extends TextView {
	...
	public void InitFirst(Context context, ImageView subTitleViewBm) {
		...
		mSubTitleViewBm.setVisibility(View.VISIBLE);
		mContext = context;
		SharedPreferences sp = mContext.getSharedPreferences("com.android.gallery3d.SubtitleViewSetInfoSP",
				Context.MODE_PRIVATE);
	 ...
	 */
	public void setTextOrBitmap(TimedText text) {
		...
		try {
			if (mEncodingSelectIdx != 0) {
				mSubTitleViewBm.setImageBitmap(getBitmapFromAddr(TimedTextEx
						.getBitMapFd(text), TimedTextEx.getBitMapWidth(text),
						TimedTextEx.getBitMapHeight(text)));
				if (TimedTextEx.getTextByteChars(text) != null) {
					setText(new String(TimedTextEx.getTextByteChars(text),
							mEncodingSelect).trim());
				}
