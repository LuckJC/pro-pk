...
//    }
    ...
    protected void openVideo() {
        ...
        try {
            ...
            // M: added for stereo feature: tell codec not to auto detection
            if (!StereoHelper.needAutoFormatDection(mStereoLayout)) {
                if (LOG) Log.d(TAG, "openVideo:no video format auto detectoin");
                setParameter(StereoHelper.KEY_PARAMETER_3D_INFO, mStereoLayout);
            }
   ...
   //FOR MTK_SUBTITLE_SUPPORT
    ...
    public void setSlowMotionSpeed(int speed) {
        if(mMediaPlayer != null) {
            if (LOG) {
                MtkLog.v(TAG, "setSlowMotionSpeed(" + speed + ")");
            }
    ...
    public boolean setParameter(int key, int value) {
        if (LOG) MtkLog.i(TAG, "setParameter(key=" + key + ", value=" + value + ")");
        if (mMediaPlayer == null ) {
            MtkLog.w(TAG, "setParameter:why we got a null MediaPlayer!");
            return false;
        } else {
    ...
    protected int mStereoLayout = 0;
    public void setStereoLayout(int stereoLayout) {
        if (LOG) MtkLog.i(TAG, "setStereoLayout(stereoLayout=" + stereoLayout + ")");
 ...
 // : MTK_SUBTITLE_SUPPORT & MTK_AUDIO_CHANGE_SUPPORT
    ...
    public int selectTrack(int index){
    	try{
            ...
            Log.w(TAG, "AudioAndSubtitle selectTrack: Unable to selectTrack IllegalStateException: index  " + index, ex);
            return RETURN_ILL_STATE;
        } catch (final Exception ex) {
            Log.w(TAG, "AudioAndSubtitle selectTrack: Unable to selectTrack Exception: index  " + index, ex);  
            return RETURN_ERROR;
        }
    	return RETURN_OK;
    }
    ...
    */
    public void deselectTrack(int index){
    	try{
			...
			Log.w(TAG, "AudioAndSubtitle deselectTrack: Unable to deselectTrack IllegalStateException: index = "+ index, ex);
			return;
		}
    }
    ...
    public void addExtTimedTextSource(String path, String mimeType){
    	try{
    		Log.d(TAG, "AudioAndSubtitle addExtTimedTextSource: before addExtTimedTextSource path = "+ path);   
    		mMediaPlayer.addTimedTextSource(path, mimeType);
    		Log.d(TAG, "AudioAndSubtitle addExtTimedTextSource: after addExtTimedTextSource path = "+ path); 
    	} catch (final IllegalStateException ex) {
            Log.w(TAG, "AudioAndSubtitle addExtTimedTextSource: Unable to addExtTimedTextSource IllegalStateException: path  " + path, ex);            
            return;
        } catch (final IOException ex) {
            Log.w(TAG, "AudioAndSubtitle addExtTimedTextSource: Unable to addExtTimedTextSource IOException: path  " + path, ex);            
            return;
        } catch (final IllegalArgumentException ex) {
            Log.w(TAG, "AudioAndSubtitle addExtTimedTextSource: Unable to addExtTimedTextSource IllegalArgumentException: path  " + path, ex);            
            return;
        } 
    }    
    ///@}
