...
public class BackgroundRenderer extends Thread {
    private static final String TAG = "Gallery2/BackgroundRenderer";
    ...
    public static synchronized BackgroundRenderer getInstance() {
        if (sInstance == null) {
            sInstance = new BackgroundRenderer("Thread-BackgroundRenderer",
                    ApiHelper.HAS_GLES20_REQUIRED && !FeatureOption.MTK_EMULATOR_SUPPORT ? 2 : 1);
            sInstance.start();
        }
        return sInstance;
    }
    ...
    private void initialize() {
        ...
        if (mVersion == 1) {
            mCanvas = new GLES11Canvas((GL11) mEglContext.getGL());
        } else {
