...
public class MediatekFeature {
    ...
    public static final boolean IS_HEAVY_FEATURE_SUPPORTED = (CPU_CORES_NUM >= 4);
    ...
    // M: 6592 panorama add @{
    private static final boolean supportPanorama3D = true && IS_HEAVY_FEATURE_SUPPORTED;
    // @}
    ...
    //Stereo Display
    private static final boolean supportStereoDisplay = MtkUtils.isSupport3d() && !FeatureOption.MTK_EMULATOR_SUPPORT;//true;
    ...
    //<S3D> mark for Stereo3DConvergence
    private static final boolean SUPPORT_STEREO_CONVERGENCE = supportStereoDisplay && true;
    ...
    private static final boolean supportConShotsImages = true && IS_HEAVY_FEATURE_SUPPORTED;
    private static final boolean supportMotionTrack = true && IS_HEAVY_FEATURE_SUPPORTED;
    /// M: add for live photo
    private static final boolean supportLivePhoto = FeatureOption.MTK_LIVE_PHOTO_SUPPORT && IS_HEAVY_FEATURE_SUPPORTED;
    /// M: add for slowmotion
    private static final boolean supportSlowMotion = FeatureOption.MTK_SLOW_MOTION_VIDEO_SUPPORT;
    /// M: add for 2k video feature
    private static final boolean support2kVideo = true;//FeatureOption.MTK_VIDEO_4KH264_SUPPORT;//true;
    private static boolean needHide2kVideo = false;
    /// M: added for HotKnot
    private static final boolean supportHotKnot = FeatureOption.MTK_HOTKNOT_SUPPORT;//true;
    ...
    /// M: kk migration
    // kk remove thememanager feature, so disable this feature.
    ...
    public static boolean isStereoConvergenceSupported() {
        return SUPPORT_STEREO_CONVERGENCE;
    }
    ...
    /* M: mark for KK print feature support.
    public static boolean isBluetoothPrintSupported() {
     ///M: Edit for Bluetooth print feature
       return true;//ConfigHelper.checkSupportedProfiles(ProfileConfig.PROFILE_ID_BPP);
...
//        prepareImageOptions(context);
    ...
    public static class Params {
        ...
        public static final int THUMBNAIL_TARGET_SIZE_AC = 1080;
        // M: 6592 panorama add @{
        ...
        public void info(String tag) {
            ...
            if (inStereoConvergence) {
                Log.v(tag,"Params:inStereoConvergence=true");
            }
            if (inGifDecoder)
            ...
            if (inInputDataBundle != null) {
                Log.v(tag,"Params:input DataBundle");
            }
            if (inMtk3D) {
                Log.v(tag, "Params:inMtk3D=" + inMtk3D);
            }
        }
        ...
        //added for stereo display feature, indicating whether we want the
        //stereo convergence data
        public boolean inStereoConvergence;
        ...
        //added to provide extra info
        public DataBundle inInputDataBundle;
        
        // add for 3D picture which captured by mtk camera
        public boolean inMtk3D;
    }
    ...
    public static class DataBundle {
        public DataBundle() {
            ...
            secondFullFrameAC = null;
            stereoConvergence = null;
            gifDecoder = null;
            mpoFrames = null;
            mpoTotalCount = 0;
        }
        public void recycle() {
            ...
            if (null != secondFrameAC) secondFrameAC.recycle();
            ...
            if (null != secondFullFrameAC) secondFullFrameAC.release();
            if (null != mpoFrames) {
        ...
        public void info(String tag) {
            ...
            //Log.d(tag,"secondFrameAC="+secondFrame);
            if (null != secondFrameAC) {
                Log.v(tag,"DataBundle:secondFrameAC[" + secondFrameAC.getWidth()
                          + "x" + secondFrameAC.getHeight() + "]");
            }
            ...
            //Log.d(tag,"DataBundle:secondFullFrameAC="+secondFullFrame);
            if (null != secondFullFrameAC) {
                Log.v(tag,"DataBundle:secondFullFrameAC[" + secondFullFrameAC.getWidth()
                          + "x" + secondFullFrameAC.getHeight() + "]");
            }
            if (null != stereoConvergence) {
                Log.v(tag,"DataBundle:returned stereoConvergence");
            }
    ...
    public static final int INCLUDE_ALL_STEREO_MEDIA = INCLUDE_MPO_3D | 
                                           INCLUDE_MPO_3D_PAN | 
                                           INCLUDE_STEREO_JPS | 
                                           INCLUDE_STEREO_PNS | 
                                           INCLUDE_STEREO_VIDEO;
    ...
    public static String getWhereClause(int mtkInclusion) {
        ...
        /// M: add for 2k video feature
        whereGroup = getMediatekWhereClause(whereGroup); 
    ...
    private static final boolean supportThumbnailMAV = true && IS_HEAVY_FEATURE_SUPPORTED;
    ...
    public static void renderSubTypeOverlay(Context context, GLCanvas canvas, int width,
        ...
        if (isMav) {
            ...
            /// M: move Icon to bottom left corner
            // mMavOverlay.draw(canvas, (width - side) / 2, (height - side) / 2, side, side);
            mMavOverlay.draw(canvas, side/2, height - side*3/2, side, side);
        }
        ...
        /// M: added for ConShots @{
        // render continuous shots icon
        if((subType & MediaObject.SUBTYPE_CONTAINER) != 0){
            if((subType & MediaObject.SUBTYPE_CONSHOT) != 0) {
                if (mConShotsPlay == null) {
                    mConShotsPlay = new ResourceTexture(context, R.drawable.ic_conshots_play);
                }
                int side = Math.min(width, height) / 5;
                mConShotsPlay.draw(canvas, side/2, height - side*3/2, side, side);
            }
            if((subType & MediaObject.SUBTYPE_MOTION) != 0) {
                if (mMotionPlay == null) {
                    mMotionPlay = new ResourceTexture(context, R.drawable.ic_motion_play);
                }
                int side = Math.min(width, height) / 5;
                mMotionPlay.draw(canvas, side/2, height - side*3/2, side, side);
            }
        }
        /// @}
        /// M: add for livephoto .@{
        if(supportLivePhoto && (subType & MediaObject.SUBTYPE_LIVEPHOTO) != 0){
            if (mLivePhotoOverlay == null) {
                mLivePhotoOverlay = new ResourceTexture(context, R.drawable.ic_livephoto);
            }
            int side = Math.min(width, height) / 5;
            mLivePhotoOverlay.draw(canvas, side/2, height - side*3/2, side, side);
            // mLivePhotoOverlay.draw(canvas, width - side*5/4, height - side*5/4, side, side);
        }
        /// @}
        /// M: add for slowmotion .@{
        if(supportSlowMotion && (subType & MediaObject.SUBTYPE_SLOWMOTION) != 0){
            if (mSlowMotionOverlay == null) {
                mSlowMotionOverlay = new ResourceTexture(context, R.drawable.ic_slowmotion_albumview);
            }
            int side = Math.min(width, height) / 5;
            mSlowMotionOverlay.draw(canvas, side/2, height - side*3/2, side, side);
        }
        /// @}
    }
	...
	// M: add for Change SubTitle and Audio Track When Playing video:
    //MTK_AUDIO_CHANGE_SUPPORT & MTK_SUBTITLE_SUPPORT
    ///@{
    /// M: Mark for 4.3 migration
    public static final boolean MTK_AUDIO_CHANGE_SUPPORT = FeatureOption.MTK_AUDIO_CHANGE_SUPPORT;
    public static final boolean MTK_SUBTITLE_SUPPORT = FeatureOption.MTK_SUBTITLE_SUPPORT;
    ///@}
    
    /// M: add for 2k video feature @{
    public static void hide2kVideo(boolean hide) {
        Log.d(TAG, "hide2kVideo:"+hide);
        needHide2kVideo = hide;
    }
    
    public static String get2kVideoWhereClause() {
        //Log.d(TAG, "get2kVideoWhereClause, support:"+support2kVideo+" need hide:"+needHide2kVideo);
        if (support2kVideo && needHide2kVideo) {
            return " AND ((" + VideoColumns.WIDTH + " <= 1920" + " AND " + VideoColumns.HEIGHT + " <= 1920) " +
                   " OR (" + VideoColumns.WIDTH + " IS NULL" + " OR " + VideoColumns.HEIGHT + " IS NULL) "+
                   "OR (" + VideoColumns.MIME_TYPE + " NOT LIKE 'video%'))";
        } else {
            return "";
        }
    }
    
    public static String getMediatekWhereClause(String whereClause) {
        String clause = new String(whereClause);
        clause = clause + get2kVideoWhereClause();
        //Log.d(TAG, "whereClause:" + clause);
        return clause;
    }
    // @}
}
