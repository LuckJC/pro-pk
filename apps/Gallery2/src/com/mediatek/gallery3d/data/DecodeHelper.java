...
public class DecodeHelper {
    ...
    public static final int MAX_PIXEL_COUNT = 640000;  // 400x1600

    public static final int TARGET_DISPLAY_WIDTH [] = {/*1920,*/ 1280, 1280, 960, 800, 640, 480};
    public static final int TARGET_DISPLAY_HEIGHT [] = {/*1080,*/ 800, 720, 540, 480, 480, 320};
    ...
    public static RegionDecoder getRegionDecoder(JobContext jc, Bitmap bitmap,
        ...
        Log.i(TAG, "getRegionDecoder:got byte Buffer size:"+array.length);
    ...
    public static RegionDecoder getRegionDecoder(JobContext jc, Params params,
                       MpoDecoderWrapper mpoDecoderWrapper, int frameIndex) {
        if (null == params || null == mpoDecoderWrapper || frameIndex < 0) {
...
Log.i(TAG,"safeDecodeImageRegion:decodeRegion(rect="+imageRect+"..)");
            ...
            Log.w(TAG,"safeDecodeImageRegion:out of memory when decoding:" + e1);
            return null;
    ...
    public static Bitmap safeDecodeImageRegion(JobContext jc, BitmapRegionDecoder
        ...
        try {
            ...
            for (int i = 0; i < maxTryNum; i++) {
                ...
                Log.d(TAG,"safeDecodeImageRegion:try for sample size " +
                          options.inSampleSize);
    ...
    public static Bitmap resizeBitmap(int targetWidth, int targetHeight, Bitmap source) {
        if (targetWidth <= 0 || targetHeight <= 0 || source == null) {
            Log.w(TAG, "resizeBitmap:got invalid parameters");
            return null;
        }
        try {
            ...
            Log.w(TAG,"resizeBitmap:out memory when resize bitmap");
            e.printStackTrace();
            return null;
        }
    }
