...
public class ItemOperationsFetchParser extends Parser {
    ...
    private void parseProperties() throws IOException {
        ...
        if (length > MimeUtility.NEED_COMPRESS_BODY_SIZE) {
            if (msg.mText != null) {
                cv.put(Body.TEXT_CONTENT, StringCompressor.compressToBytes(msg.mText));
            }
            if (msg.mHtml != null) {
                cv.put(Body.HTML_CONTENT, StringCompressor.compressToBytes(msg.mHtml));
            }
            isNeedCompress = true;
        ...
        ops.add(ContentProviderOperation.newUpdate(ContentUris.withAppendedId(
                ...
                .withValue(EmailContent.Message.FLAGS, msg.mFlags)
                .withValue(EmailContent.Message.FLAG_LOADED, msg.mFlagLoaded)
                .build());
        try {
