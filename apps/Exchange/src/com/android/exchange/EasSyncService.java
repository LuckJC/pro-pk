...
public class EasSyncService extends AbstractSyncService {
    ...
    /// M: Add a List to record GAL service which is running and set MAX running thread to avoid out of bind
...
//        }
    ...
    static public GalResult searchGal(Context context, long accountId, String filter, int limit) {
        ...
        if (acct != null) {
            ...
            /**
             * M:  If GAL search service count was more then GAL_SEARCH_LIMIT,
             * cancel the oldest.
            ...
            synchronized(sGalServiceList) {
                ...
                svc.userLog("Running GalService count " + size);
                sGalServiceList.add(svc);
            }
            if (needStopGalService != null) {
                svc.userLog("Cancel the oldest GalService");
            ...
            try {
                ...
                try {
                    int code = resp.getStatus();
                    svc.userLog("searchGal return status: " + code);
                    if (code == HttpStatus.SC_OK) {
                        InputStream is = resp.getInputStream();
                        try {
                    ...
                    resp.close();
                    /// M: Remove object from static list to avoid object leak for a while
                    synchronized(sGalServiceList) {
                        if (sGalServiceList.contains(svc)) {
