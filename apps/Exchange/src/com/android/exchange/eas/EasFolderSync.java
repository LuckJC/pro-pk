...
public class EasFolderSync extends EasOperation {
    ...
    /// M: Set the max retry count of provision to 3.
    private static final int TRY_PROVISION_MAX_RETRY_COUNT = 3;
    ...
    public int doFolderSync(final SyncResult syncResult) {
        ...
        /** M: If the securitySyncKey is null, try provision firstly.
         *  Because the first folderSync command will spend much time.
         *  So we send provision command to server directly. @{ */
        LogUtils.d(LOG_TAG, "Account: " + mAccount.mEmailAddress + " start doFolerSync");

        /// M: Use mPolicyKey to determine if we have been asked to try provision during validate process
        if (mAccount.mPolicyKey > 0
                && (TextUtils.isEmpty(mAccount.mSecuritySyncKey) || "0"
                        .equals(mAccount.mSecuritySyncKey))) {
            int retryCount = 1;
            while (retryCount <= TRY_PROVISION_MAX_RETRY_COUNT) {
                LogUtils.d(LOG_TAG, "tryProvision");
                boolean isSuccess = super.tryProvision(syncResult, mAccountId);
                if (isSuccess) {
                    LogUtils.d(LOG_TAG, "provision success");
                    break;
                } else {
                    LogUtils.d(LOG_TAG, "provision error!");
                    if (retryCount == TRY_PROVISION_MAX_RETRY_COUNT) {
                        LogUtils.d(LOG_TAG, "provision error count > 3, return");
