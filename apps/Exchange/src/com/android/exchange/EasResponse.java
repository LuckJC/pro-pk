...
public class EasResponse {
    ...
    public static EasResponse fromHttpRequest(
        ...
        /** M: The ConnectionManager of this client may be shutdown by other thread due to
         *  its lifetime > MAX_LIFETIME(10 minutes). The client.execute(request) will throw
         *  a IllegalStateException in this case. We can treat it as an IOException so that
         *  it can be handled by the upper logic. @{ */
        final HttpResponse response;
        try {
            response = client.execute(request);
        } catch (IllegalStateException ex) {
            LogUtils.w(Eas.LOG_TAG, "EasResponse fromHttpRequest got exception: %s", ex);
    ...
    public InputStream getInputStream() {
        ...
        mInputStream = is;
        /// M: Return a BufferedInputStream for more quickly parsing.
