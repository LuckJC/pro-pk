...
public class ExchangeDirectoryProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        LogUtils.logFeature(LogTag.SEARCH_TAG, "ExchangeDirectoryProvider: query: %s", uri.toString());
        ...
        switch (match) {
            ...
            case GAL_EMAIL_FILTER: {
                ...
                try {
                    ...
                    LogUtils.logFeature(LogTag.SEARCH_TAG, "searchGal with limit: %d", limit);
                    // Get results from the Exchange account
                    final GalResult galResult = EasSyncService.searchGal(getContext(), accountId,
                            filter, limit);
                    if (galResult != null) {
                        LogUtils.logFeature(LogTag.SEARCH_TAG, "searchGal result total: %d", galResult.total);
                        return buildGalResultCursor(
                                projection, galResult, match == GAL_PHONE_FILTER, sortOrder);
                    } else {
                        LogUtils.logFeature(LogTag.SEARCH_TAG, "searchGal contacts got nothing");
                    }
                } finally {
