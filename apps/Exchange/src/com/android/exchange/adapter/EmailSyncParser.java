...
public class EmailSyncParser extends AbstractSyncParser {
    ...
    protected void wipe() {
        ...
        We just ignore all the local changes before the recovery @{ */
        mContentResolver.delete(MessageMove.CONTENT_URI,
                MessageMove.SRC_FOLDER_KEY + "=" + mMailbox.mId, null);
...
//                Eas.EXCHANGE_ACCOUNT_MANAGER_TYPE), mMailbox.mId);
    ...
    private void wipeMessageStateChanges() {
        Cursor c = mContentResolver.query(Message.CONTENT_URI, new String[] {MessageColumns.MESSAGE_ID},
                MessageColumns.MAILBOX_KEY + "=?", new String[] {String.valueOf(mMailbox.mId)}, null);
        if (c == null) {
        ...
        try {
            ...
            while (c.moveToNext()) {
                ops.add(ContentProviderOperation.newDelete(MessageStateChange.CONTENT_URI)
                        .withSelection(MessageChangeLogTable.MESSAGE_KEY + "=?",
                                new String[] {c.getString(0)}).build());
        ...
        if (mMailbox.mId == Exchange.sBadSyncKeyMailboxId
            ...
            try {
                if (c != null) {
                    ...
                    LogUtils.i(Eas.BSK_TAG, "For current window, found "
            ...
            LogUtils.i(Eas.BSK_TAG, "There are " + newEmails.size()
