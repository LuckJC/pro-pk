...
public abstract class EasSyncHandler extends EasServerConnection {
    ...
    public final boolean performSync(SyncResult syncResult) {
        ...
        /// M: The window count that BSK recovery would sync
        int windowCount = 0;
        /// M: Whether the window count has been gotten
        boolean gotWindowCount = false;
        while (result == SYNC_RESULT_MORE_AVAILABLE) {
            /** M: The status machine for "Bad Sync Key" recovery, currently
            just for Email folder. If initial sync, we can not do "getEstimate",
            we start the recovery from the real 1st sync @{ */
            if (mMailbox.mId == Exchange.sBadSyncKeyMailboxId && !"0".equals(key)) {
                EasMailboxSyncHandler handler = (EasMailboxSyncHandler)this;
                if (!gotWindowCount) {
                    gotWindowCount = true;
                    windowCount = handler.getWindowCount();
                    LogUtils.i(Eas.BSK_TAG, "window count: " + windowCount);
                    // If getEstimate failed, we cannot calculate the window count,
                    // or if we just need sync one window, we sync as "NORMAL". Otherwise,
                    // start to sync use 0 truncation size for saving network.
                    if (windowCount > 1) {
                        handler.mBskSyncAsNormal = false;
                    }
                    LogUtils.i(Eas.BSK_TAG, "About to sync window: " + windowCount);
                    windowCount--;
                } else if (windowCount-- > 1) {
                    LogUtils.i(Eas.BSK_TAG, "About to sync window: " + (windowCount + 1));
                } else {
                    LogUtils.i(Eas.BSK_TAG, "About to sync the last window");
                    ((EasMailboxSyncHandler)this).mBskSyncAsNormal = true;
                }
            }
            /** @} */
