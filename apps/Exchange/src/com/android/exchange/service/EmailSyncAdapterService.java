...
import com.mediatek.exchange.outofoffice.EasOutOfOfficeSetting;
...
public class EmailSyncAdapterService extends AbstractSyncAdapterService {
    ...
    public class SyncHandlerSynchronizer {
        ...
        public synchronized void modifyPing(final boolean lastSyncHadError,
            ...
            if (isRunningSync(account.mId)) {
                LogUtils.d(TAG, "Start or restart a ping while syning on account: " + account.mId);
                return;
            }
            ...
            LogUtils.d(TAG, "Start or restart a ping with syncinterval: " + account.mSyncInterval);
            ...
            if (pushNeeded) {
                ...
                if (pingSyncHandler != null) {
                    ...
                    LogUtils.d(TAG, "ModifyPing: Ready to start a new ping");
                    // Start a new ping.
    ...
    private final IEmailService.Stub mBinder = new IEmailService.Stub() {
        ...
        public void fetchMessage(long messageId) {
            LogUtils.d(TAG, "IEmailService.fetchMessage: %d", messageId);
            /// M: add for partial download
            EasItemOperations.fetchMessage(EmailSyncAdapterService.this, messageId);
        }
        ...
        public OofParams syncOof(long accountId, OofParams oofParams, boolean isGet) {
            return EasOutOfOfficeSetting.doSyncOof(EmailSyncAdapterService.this, accountId, oofParams, isGet);
        }
    ...
    private class SyncAdapterImpl extends AbstractThreadedSyncAdapter {
        ...
        public void onPerformSync(final android.accounts.Account acct, final Bundle extras,
            ...
            /// M: When we are doing initial sync, we gotta do a full sync, cause security hold flag
            // is set in email process asynchronously, first try of initial sync may fail, so we permit
            // an initial sync, however, in that case, mailboxType may have changed to Calendar/Contacts,
            // which won't leat to a full sync
            final boolean isFullSync = isInitialSync || (!pushOnly && !accountOnly && mailboxIds == null
            ...
            if ((account.mFlags & Account.FLAGS_SECURITY_HOLD) == 0) {
                /// M: Only do folder sync when account has released security hold, otherwise we may
                // encounter bad circumstances: folder sync successfully while we don't do mailbox
                // sync, so make foldersync/mailboxsync act in same condition @{
        ...
        private boolean syncMailbox(final Context context, final ContentResolver cr,
            ...
            /// M: The Search mailbox is also can not sync with server
            if (mailbox.mType == Mailbox.TYPE_DRAFTS || mailbox.mType == Mailbox.TYPE_SEARCH) {
                ...
                LogUtils.d(TAG, "Skipping sync of DRAFTS and SEARCH folder");
                return true;
            }
