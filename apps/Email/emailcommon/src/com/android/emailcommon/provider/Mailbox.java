...
public class Mailbox extends EmailContent implements MailboxColumns, Parcelable {
    ...
    /**M: display unread message count on applist @{ */
    ...
    public long mLastNotifiedMsgId;
    ...
    ///M:repeat alert when mark unread flag on another device
    public static final int CONTENT_LAST_NOTIFIED_MESSAGE_KEY_COLUMN = 21;
    ...
    /// M: can be used to identify all messages have be downloaded.
    public static final int FLAG_ALL_MESSAGES_DOWNLOADED = 1<<7;
    ...
    public static Mailbox newSystemMailbox(Context context, long accountId, int mailboxType) {
        ...
        switch (mailboxType) {
            ...
            case TYPE_TRASH:
                ...
                /** M: For pop3 account, we do not sync the Trash and Sent. @{ */
                if (HostAuth.LEGACY_SCHEME_POP3.equals(Account.getProtocol(context, accountId))) {
    ...
    public void restore(Cursor cursor) {
        ...
        ///M:repeat alert when mark unread flag on another device
        mLastNotifiedMsgId = cursor.getLong(CONTENT_LAST_NOTIFIED_MESSAGE_KEY_COLUMN);
    }
    ...
    /**
     * M: Update mailbox flag witch identified whether load all messages or not
     * @param context the Context
     * @param allMessagesLoaded true(when all messages has downloaded or false
     */
    //Todo: For common type mailbox , we also need update the flag to decide the visibility of "LoadMore"
    public void updateAllMessageDownloadFlag(Context context, boolean allMessagesLoaded) {
        if (allMessagesLoaded) {
            mFlags |= Mailbox.FLAG_ALL_MESSAGES_DOWNLOADED;
        } else {
            mFlags &= ~Mailbox.FLAG_ALL_MESSAGES_DOWNLOADED;
        }
        ContentValues values = new ContentValues();
