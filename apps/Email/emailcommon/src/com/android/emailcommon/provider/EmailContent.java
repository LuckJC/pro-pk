...
public abstract class EmailContent {
    ...
    public Uri save(Context context) {
        ...
        /// M: Add a null value judgment, res is null said that database operation is fail.
        if (res == null) {
            LogUtils.w( Logging.LOG_TAG,
                          "The return uri of insert operation is null, this database operation maybe fail!");
            return null;
        }
    ...
    public static final class Body extends EmailContent implements BodyColumns {
        ...
        public static Uri CONTENT_URI;
        /// M: Add to avoid too large body operation issue
    ...
    public static final class Message extends EmailContent implements SyncColumns, MessageColumns {
        ...
        public static final int CONTENT_DIRTY_COLUMN = 26;

        public static final int CONTENT_MAIN_MAILBOX_KEY_COLUMN = 27;
        ...
        public static final String[] CONTENT_PROJECTION = new String[] {
            ...
            MessageColumns.MAIN_MAILBOX_KEY
        };
         ...
         */
        /// M: Add Message.FLAG_LOADED_ENVELOPE for IMAP partial download.
        ...
        public static final int FLAG_LOADED_ENVELOPE = 5;
        ...
        public static final int FLAG_BODY_COMPRESSED = 1 << 23;
        ...
        /** M: Flags used to indicate partial download message status @{ */
        public static final int FLAG_LOAD_STATUS_NONE = 1 << 25;
        public static final int FLAG_LOAD_STATUS_LOADING = 1 << 26;
        public static final int FLAG_LOAD_STATUS_SUCCESS = 1 << 27;
        public static final int FLAG_LOAD_STATUS_FAILED = 1 << 28;
        public static final int LOAD_SATATUS_MASK = 0xF << 25;
        /** @} */
        /// M: Flag used to indicate the message is in sending state(in outbox) @{
        public static final int FLAG_STATUS_SENDING = 1 << 29;
        /// @}
        ...
        @Override
        public ContentValues toContentValues() {
            ///M: Generate text if the text is null for local search.
            ...
            /// M: save FLAG_BODY_COMPRESSED for compressed body
            if (isBodyNeedCompress()) {
                mFlags |= FLAG_BODY_COMPRESSED;
            }
        ...
        public Uri save(Context context) {
            ...
            if (mText == null && mHtml == null && mTextReply == null && mHtmlReply == null &&
                ...
                if (doSave) {
                    /// M: log receive a new message. @{
                    Uri uri = super.save(context);
                    logMessageReceived(context, new Message[] { this });
                    return uri;
                    /// @}
                } else {
        ...
        public void addSaveOps(ArrayList<ContentProviderOperation> ops) {
            ...
            if (isNew) {
                ...
                /**
                 * M: Append messageId to BaseUri and let it to notify a
            ...
            if (isBodyNeedCompress()) {
                if (mText != null) {
                    cv.put(Body.TEXT_CONTENT, StringCompressor.compressToString(mText));
                }
                if (mHtml != null) {
                    cv.put(Body.HTML_CONTENT, StringCompressor.compressToString(mHtml));
                }
                LogUtils.d(Logging.LOG_TAG, "Message addSaveOps body compressed");
            } else {
                if (mText != null) {
        ...
        public boolean isFlaggedBodyCompressed() {
            return (mFlags & FLAG_BODY_COMPRESSED) == FLAG_BODY_COMPRESSED;
        }

        /**
         * M: Is the message body large and need to compress
         */
        private boolean isBodyNeedCompress() {
            int length = 0;
            if (mText != null) {
                length += mText.length();
            }
            ...
            return length > MimeUtility.NEED_COMPRESS_BODY_SIZE;
        }

        /**
         * M: Generate text from html content if the text is null.
        ...
        /** M: Set message loaded state flags */
        public int changeMessageStateFlags(int flagLoadStatus) {
            // Generally only failures are handled here
            mFlags &= ~Message.LOAD_SATATUS_MASK;
            mFlags |= flagLoadStatus;
            return mFlags;
        }
        ...
        public static String buildMessageListSelection(Context context, long mailboxId) {
            ...
            if (isVirtualMailbox(mailboxId)) {
                ...
                // 1.1 VIP is an exception, for it get result by filtered cursor,
                // not through sql. append account selection when not a combined account,
                // do nothing for combined account.
                if (mailType == Mailbox.TYPE_VIP) {
                    if (!isCombinedMailbox(mailboxId)) {
                        selection.append(MessageColumns.ACCOUNT_KEY)
                ...
                if (mailType == Mailbox.TYPE_INBOX) {
                    ...
                    selection.append(MessageColumns.MAILBOX_KEY).append('=')
                            .append(mailboxId);
                }
                ...
                if (!isCombinedMailbox(mailboxId)) {
                    selection
                            .append(" AND " + MessageColumns.ACCOUNT_KEY)
                            .append("= ")
                            .append(accountId);
                }
            } else {
            ...
            if (queryField.contains(SearchParams.SEARCH_FIELD_BODY) || isAll) {
                ...
                try {
                    c = context.getContentResolver().query(
                            ...
                            Message.buildMessageListSelection(context,
                                    mailboxId), null, null);
                    ...
                    LogUtils.d(LOG_TAG, e, "buildLocalSearchSelection query exception: ");
                } finally {
                    if (c != null) {
                        c.close();
                    }
                ...
                try {
                    /** M: Find the messages with compressed body @{ */
                    String largeMessgeSelection = "(" + MessageColumns.FLAGS + "&" + Message.FLAG_BODY_COMPRESSED +
                            ") !=0 AND " + MessageColumns.ID + " IN (" + messageIdRange.toString();
                    largeMessgeCursor = context.getContentResolver().query(Message.CONTENT_URI, new String[]{
                            MessageColumns.ID}, largeMessgeSelection, null, null);
                    ...
                    while (c.moveToNext()) {
                        if (!TextUtils.isEmpty(messageContent = c.getString(1))) {
                            /** M: Find the compressed body content and depress it. @{ */
                            if (largeMessgeIds.contains(c.getLong(0))) {
                    ...
                    /// M: Close the large message cursor
                    if (largeMessgeCursor != null) {
        ...
        /**
         * M: log a received mail, query mailbox information from db.
         * @param context
         * @param messages
         */
        public static void logMessageReceived(Context context, Message[] messages) {
            if (LogUtils.isLoggable(LogTag.RECEIVEMAIL_TAG, LogUtils.DEBUG)) {
                for (Message message : messages) {
                    LogUtils.logFeature(LogTag.RECEIVEMAIL_TAG, " Message received [%s][%s], mailboxkey[%d]",
                            new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()),
                            message.toString(), message.mMailboxKey);
                }
            }
        }
    }
    ...
    public static final class Attachment extends EmailContent
        ...
        /**
         * M: For clear old internal attachments @{
         */
