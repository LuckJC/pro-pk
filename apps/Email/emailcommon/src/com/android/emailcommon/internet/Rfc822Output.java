...
public class Rfc822Output {
    ...
    /*package*/ static String[] buildBodyText(Body body, boolean useSmartReply) {
        ...
        if (useSmartReply && pos > 0) {
            /** M: Modified for getting the correct html or text content. @{ */
            if (messageBody[INDEX_BODY_HTML] != null) {
                if (pos < messageBody[INDEX_BODY_HTML].length()) {
                    messageBody[INDEX_BODY_HTML] = messageBody[INDEX_BODY_HTML].substring(0, pos);
                }
                messageBody[INDEX_BODY_TEXT] = HtmlConverter
                        .htmlToText(messageBody[INDEX_BODY_HTML]);
            }
            /** @} */
        }
        return messageBody;
    ...
    public static void writeTo(Context context, Message message, OutputStream out,
        ...
        /// M: Decompress the body if it was compressed
        if (message.isFlaggedBodyCompressed()) {
            body.mTextContent = StringCompressor.decompressFromString(body.mTextContent);
            body.mHtmlContent = StringCompressor.decompressFromString(body.mHtmlContent);
        }
    ...
    private static void writeOneAttachment(Context context, Writer writer, OutputStream out,
        ...
        writeHeader(writer, "Content-Type",
                /// M: if the filename contains non-printable ascii code , encode it
    ...
    private static void writeTextWithHeaders(Writer writer, OutputStream out, String[] bodyText)
        ...
        /// M: if we have text and html content, we use multipart/alternative as content-type @{
