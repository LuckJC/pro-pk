...
public class ObjectCursorLoader<T> extends AsyncTaskLoader<ObjectCursor<T>> {
    ...
    /// M: Add for performance @{
    private boolean mPerformance = false;
    private String mLoaderName = "";
    public ObjectCursorLoader(Context context, Uri uri, String[] projection,
            CursorCreator<T> factory, String loader) {
        this(context, uri, projection, factory);
        this.mLoaderName = loader;
        mPerformance = true;
    }
    /// @}
    ...
    public ObjectCursor<T> loadInBackground() {
        if (mPerformance) {
            PDebug.EndAndStart(mLoaderName + ".onCreateLoader->loadInBg", mLoaderName + ".loadInBg");
            PDebug.Start(mLoaderName + ".loadInBg.query");
        }
        final Cursor inner = getContext().getContentResolver().query(mUri, mProjection,
                mSelection, mSelectionArgs, mSortOrder);
        if (mPerformance) {
            PDebug.End(mLoaderName + ".loadInBg.query");
        }
        if (inner == null) {
            // If there's no underlying cursor, there's nothing to do.
            if (mPerformance) {
                PDebug.EndAndStart(mLoaderName + ".loadInBg", mLoaderName + ".loadInBg->onLoadFinished");
            }
            return null;
        }
