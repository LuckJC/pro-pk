...
public class MimeType {
    ...
    /**
     * M : If the default content type could not be viewed, we try the content type inferred by MediaFile again.
     */
    public static boolean isViewable(Context context, Attachment attachment) {
        if (isViewable(context, attachment.contentUri, attachment.getContentType())) {
            return true;
        } else {
            // try other type again.
            String defaultContentType = attachment.getContentType();
            String reinferredContentType = getAttachmentExtensionMimeType(attachment.getName());
            if (isViewable(context, attachment.contentUri, reinferredContentType)) {
                attachment.setInferredContentType(reinferredContentType);
                LogUtils.d(LOG_TAG, "MimeType : change attachment: %s  MIME type from: %s to: %s ",
                        attachment.getName(), defaultContentType, reinferredContentType);
                return true;
            }
            return false;
        }
    ...
    public static String inferMimeType(String name, String mimeType) {
        ...
        if (TextUtils.isEmpty(extension)) {
            ...
            if (isGenericType || TextUtils.isEmpty(mimeType)) {
                ///M: Try to get a mime type by file name for Email
                type = getAttachmentExtensionMimeType(name);
    ...
    /**
     * M: get mime type by system class MediaFile's api.
     * @param fileName
     * @return
     */
