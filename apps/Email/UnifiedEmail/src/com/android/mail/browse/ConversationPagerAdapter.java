...
public class ConversationPagerAdapter extends FragmentStatePagerAdapter2
    ...
    /**
     * M: Returns true if singleton mode is still needed by something.
     * For example, it always should be singleton mode in {@link UIProvider.FolderType.UNREAD} folder
     * @return true if need to, false otherwise
     */
    public boolean isNeedSingletonMode() {
        LogUtils.d(LOG_TAG, "isNeedSingletonMode with mFolder: %s", mFolder);
        return mFolder.isType(UIProvider.FolderType.UNREAD);
    }
    ...
    /// M: We should inherit this function to do additional things
        ...
        if (mController != null && !mDetachedMode && mPager != null) {
            ...
            if (pos == POSITION_NONE && cursor != null && currConversation != null) {
                ...
                if (frag != null && cursor.moveToPosition(pos) && frag.isUserVisible()
                        /** M: Do not calculate position again when paging disabled, it's
                         * unnecessary and make wrong position @{
                         */
                        && !isPagingDisabled(cursor)) {
                        /** M: @} */
