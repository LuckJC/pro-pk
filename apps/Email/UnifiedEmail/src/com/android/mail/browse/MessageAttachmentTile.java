...
public class MessageAttachmentTile extends AttachmentTile implements OnClickListener,
    ...
    private boolean onClick() {
        /// M: Only download what we want
        showAndDownload();
        return true;
    }

    /**
     * M: Download this attachment
     */
    private void showAndDownload() {
        // download with additional priority and no delay
        if (downloadAttachment(1, false)) {
            viewAttachment();
        }
    }
    ...
    /**
     * M: Return true if we can(start/already) download(ed), return false if we can't
     * @param additionalPriority
     * @param delayDownload
     * @return
     */
    public boolean downloadAttachment(int additionalPriority, boolean delayDownload) {
        ...
        if (ImageUtils.isImageMimeType(mime)) {
            MailPhotoViewActivity
                    .startMailPhotoViewActivity(getContext(), mAttachmentsListUri, mPhotoIndex,
                            /* M: ViewOnlyMode */mIsViewOnlyMode);
            return;
        }
    ...
    public void thumbnailLoadFailed() {
        ...
        /// M: We prefer disable it now, maybe we could enhance this design in future.
