...
public class SendersView {
    ...
    public static void handlePriority(Context context, int maxChars, String messageInfoString,
        ...
        for (int i = 0; i < conversationInfo.messageInfos.size(); i++) {
            ...
            /** M: Usually an Address without a address has no name either.
             * The right sequence should be:
             * SenderAddress > Sender > Account("Me") @{ */
            nameString = TextUtils.isEmpty(currentMessage.senderEmail) ?
                    getMe(context) :
                        TextUtils.isEmpty(currentMessage.sender) ?
                                currentMessage.senderEmail : currentMessage.sender;
...
//            }
            ...
            if (shouldAddPhotos) {
                /** M: Usually an Address without a address has no name either.
                *  If no senderEmail address we use account directly @{ */
                String senderEmail = TextUtils.isEmpty(currentMessage.senderEmail) ?
