...
public class EmlMessageViewFragment extends Fragment
    ...
    private class MessageLoadCallbacks
        ...
        public void onLoadFinished(Loader<ConversationMessage> loader, ConversationMessage data) {
            /** M: Nothing useful to do if we have no valid data,just show toast and finish @{*/
            if (data == null) {
                if (getActivity() != null) {
                    Toast.makeText(getActivity(), R.string.attachment_info_deleted, Toast.LENGTH_LONG)
                         .show();
                    ((EmlViewerActivity) getActivity()).finish();
                }
                return;
            }
            /** @} */
    ...
    private class FilenameLoadCallbacks implements LoaderManager.LoaderCallbacks<Cursor> {
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
            /// M: getActivity() may return null in background, so need check too.
            if (data == null || !data.moveToFirst() || getActivity() == null) {
                return;
            }
