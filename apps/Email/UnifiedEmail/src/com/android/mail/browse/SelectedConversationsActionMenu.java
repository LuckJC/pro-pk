...
public class SelectedConversationsActionMenu implements ActionMode.Callback,
    ...
    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
        ...
        if (itemId == R.id.delete) {
            ...
            /// M: After mark conversation read, return to conversation list.
            clearSelection();
        } else if (itemId == R.id.unread) {
            markConversationsRead(false);
            /// M: After mark conversation unread, return to conversation list.
            clearSelection();
        } else if (itemId == R.id.star) {
            starConversations(true);
            /// M: After mark conversation star, return to conversation list.
            clearSelection();
            ...
            /// M: After remove star, return to conversation list.
            clearSelection();
    ...
    public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
        ...
        /// M:If the folder is trash folder, then do not show the menu star and remove_star
        boolean isTrash = mFolder != null && mFolder.isType(UIProvider.FolderType.TRASH);
        final MenuItem star = menu.findItem(R.id.star);
        star.setVisible(!isTrash && showStar);
        final MenuItem unstar = menu.findItem(R.id.remove_star);
        unstar.setVisible(!isTrash && !showStar);
    ...
    public void onSetChanged(ConversationSelectionSet set) {
        ...
        updateCount();
        /// M: We also need to refresh action menu.
        updateSelection();
    }
