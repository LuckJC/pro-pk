...
public class Folder implements Parcelable, Comparable<Folder> {
    ...
    public long lastNotifiedMsgKey;
    ...
    public static final class Builder {
        ...
        public Folder build() {
            return new Folder(mId, mPersistentId, mUri, mName, mCapabilities,
                    ...
                    mLastMessageTimestamp, mUnreadSenders, /*M: for local search*/mLocalSearchUri);
        }
    ...
    public Folder(Cursor cursor) {
        ...
        ///M:repeat alert when mark unread flag on another device
        lastNotifiedMsgKey = cursor.getLong(UIProvider.FOLDER_LAST_NOTIFIED_MESSAGE_KEY_COLUMN);

        /// M: add for local search
        String localsearch = cursor.getString(UIProvider.FOLDER_LOCALSEARCH_LIST_URI_COLUMN);
        localSearchUri = !TextUtils.isEmpty(localsearch) ? Uri.parse(localsearch) : null;
    }
    ...
    public Folder(Parcel in, ClassLoader loader) {
        ...
        ///M: for local search
        localSearchUri = in.readParcelable(loader);
     }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        ///M: for local search
        dest.writeParcelable(localSearchUri, 0);
    }
    ...
    public static ObjectCursorLoader<Folder> forSearchResults(Account account, String query,
            /*M: add search field for remote search*/String field, Context context) {
        if (account.searchUri != null) {
            ...
            searchBuilder.appendQueryParameter(SearchParams.BUNDLE_QUERY_FIELD, field);
            /// @}
    ...
    /// M: create local search Uri.
    public Uri buildLocalSearchUri(String queryTerm, String queryField) {
        if (localSearchUri == null || TextUtils.isEmpty(queryTerm)
                || TextUtils.isEmpty(queryField)) {
            LogUtils.logFeature(LogTag.SEARCH_TAG, "buildLocalSearchUri failed for some param is null");
            return null;
        }
        Uri result = localSearchUri
                .buildUpon()
                .appendQueryParameter(SearchParams.BUNDLE_QUERY_TERM, queryTerm)
                .appendQueryParameter(SearchParams.BUNDLE_QUERY_FIELD,
                        queryField).build();
        return result;
    }
}
