...
public class ConversationInfo implements Parcelable {
    ...
    public boolean markRead(boolean read) {
        ...
        /**
         * M: Change read or unread state,the snippet will be changed. In order
         * to keep the snippet is not null, add the adjust situation of null.@{
         */
        if (read) {
            if (!TextUtils.isEmpty(lastSnippet)) {
                firstSnippet = lastSnippet;
            }
        } else {
            if (!TextUtils.isEmpty(firstUnreadSnippet)) {
