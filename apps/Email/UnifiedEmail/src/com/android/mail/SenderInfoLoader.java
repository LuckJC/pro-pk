...
public class SenderInfoLoader extends AsyncTaskLoader<ImmutableMap<String, ContactInfo>> {
    ...
    public SenderInfoLoader(Context context, Set<String> senders) {
        /* M: use customer Thread Pool @} */
        super(context, AsyncTask.THREAD_POOL_EXECUTOR);
        /* @{ */
        mSenders = senders;
    }
    ...
    public static ImmutableMap<String, ContactInfo> loadContactPhotos(
        ...
        /// M: one contact maybe has several email address, so should use multimap to record
        Multimap<Long, Pair<String, ContactInfo>> photoIdMap = ArrayListMultimap.create();
        ArrayList<String> photoIdsAsStrings = new ArrayList<String>();
        /** M: make all address to query to lower case. @{ */
        ArrayList<String> senders = new ArrayList<String>();
        for (String string : getTruncatedQueryParams(senderSet)) {
            senders.add(string.toLowerCase());
        }
        /** @} */
        ...
        StringBuilder query = new StringBuilder()
                ...
                .append("' AND Lower(").append(Email.DATA).append(")").append(" IN (");
        ...
        try {
            ...
            while (cursor.moveToPosition(++i)) {
                ...
                /// M: save result ignore case, means one address belong to different
                //  contacts will have one result, the last put one.
                results.put(email.toLowerCase(), result);
            }
            cursor.close();
            ...
            while (cursor.moveToPosition(++i)) {
                ...
                /// M: one photoId may map to several Emailaddress, take the photo to the
                //  address if it has a photoId,also put to result ignore case.
                Collection<Pair<String, ContactInfo>> prevs = photoIdMap.get(photoId);
                for (Pair<String, ContactInfo> prev : prevs) {
