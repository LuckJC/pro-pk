...
public abstract class PhotoManager implements ComponentCallbacks2, Callback {
    ...
    private void processLoadedImages() {
        ...
        /**
         * M: The variable "mPendingRequests" may be modified by different
         * threads simultaneously, for example, the observer for contact
         * provider would call the "clear" function on other thread to result in
         * some problem. So add lock for mPendingRequests to make sure the
         * operations on UI thread work well.
         * @{
         */
        synchronized (mPendingRequests) {
            Set<Integer> keyHashSet = mPendingRequests.keySet();
            for (final Integer hash : keyHashSet) {
