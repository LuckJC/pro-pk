...
public class ConversationCursorLoader extends AsyncTaskLoader<ConversationCursor> {
    ...
    public ConversationCursorLoader(Activity activity, Account account, Uri uri, String name) {
        ...
        super(activity, AsyncTask.THREAD_POOL_EXECUTOR);
        /* @{ */
        mUri = uri;
        mName = name;
    ...
    public ConversationCursor loadInBackground() {
        PDebug.EndAndStart("LOADER_CONVERSATION_LIST.onCreateLoader->loadInBg",
                "LOADER_CONVERSATION_LIST.loadInBg");
        if (!mInit) {
            mConversationCursor.load();
            mInit = true;
        }
        PDebug.EndAndStart("LOADER_CONVERSATION_LIST.loadInBg",
                "LOADER_CONVERSATION_LIST.loadInBg->onLoadFinished");
