...
public class NestedFolderTeaserView extends LinearLayout implements ConversationSpecialItemView {
    ...
    private void updateViews(final FolderHolder folderHolder) {
        ...
        /** M: Set the visibility of SendersTextView. So, the folder name can be shown
         *  at the vertical center while the sendersText is null. @{ */
        if (TextUtils.isEmpty(sendersText)) {
            folderHolder.getSendersTextView().setVisibility(GONE);
        } else {
            folderHolder.getSendersTextView().setVisibility(VISIBLE);
        }
        /** @} */
    }
