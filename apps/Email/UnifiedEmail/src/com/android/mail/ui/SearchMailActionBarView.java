...
public class SearchMailActionBarView extends MailActionBarView {
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        /// M: disable refresh menu when we are do remote searching.
        Utils.setMenuItemVisibility(menu, R.id.refresh, false);
        switch (getMode()) {
    ...
    /** M: the search view's text will roll back after clicking the Optionmenu during remote search @{ */
    private boolean mSearchMenuExpanding = false;

    @Override
    public boolean onMenuItemActionExpand(MenuItem item) {
        ///M: use to determine if it's in expanding process
        if (item.getItemId() == R.id.search) {
            mSearchMenuExpanding = true;
        }
        return true;
    }
    ...
    public boolean onQueryTextChange(String newText) {
        if (!(TextUtils.isEmpty(newText) && mSearchMenuExpanding)) {
            mActivity.getIntent().putExtra(
                    ConversationListContext.EXTRA_SEARCH_QUERY, newText);
        }
        mSearchMenuExpanding = false;
        return true;
    }
    /** @} */
    ...
    private void setSearchQueryTerm() {
        final MenuItem search = getSearch();
        if (search != null) {
            ///M: only do expand when the actionview hasn't expand. ExapandActionView will result in empty text in searchview
