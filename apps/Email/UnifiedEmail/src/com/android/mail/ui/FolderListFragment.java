...
public class FolderListFragment extends ListFragment implements
    ...
    /** M: VIP folder @{ */
    // For save the state of mail count of VIP folder
    private static final String BUNDLE_VIP_MAIL_COUNT = "FolderListFragment.state.VipMailCount";
    private FolderItemView mVipFolderItemView;
    ...
    public void onActivityCreated(Bundle savedState) {
        ...
        /**
         * M: register the Vip observer. When the notification arrived , new
         * task will be start to update the VIP account
         * Use throttle observer to avoid duplicate changed notification. @{
         */
        if (mVipMemberObserver == null) {
            mVipMemberObserver = new ThrottleContentObserver(new Handler(), activity, new Runnable() {
                @Override
                public void run() {
                    // Execute in background thread, check if current account is available.
                    if (null == mCurrentAccount || null == mCurrentAccount.uri) {
                        return;
                    }
                    long accountId = (Long.parseLong(mCurrentAccount.uri.getLastPathSegment()));
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (savedState != null && savedState.containsKey(BUNDLE_VIP_MAIL_COUNT)) {
            mVipMailCount = savedState.getInt(BUNDLE_VIP_MAIL_COUNT);
            updateVipMessageCount(mVipMailCount);
        }
        /** @} */
    ...
    private void viewFolderOrChangeAccount(int position) {
        ...
        if (item instanceof DrawerItem) {
            ...
            if (itemType == DrawerItem.VIEW_ACCOUNT) {
                ...
                /// M: Correct a coding defect that mSelectedFolderUri is not a Java Uri object,
                 // thus do not write Uri.equals(mSelectedFolderUri)
    ...
    public Loader<ObjectCursor<Folder>> onCreateLoader(int id, Bundle args) {
        ...
        if (id == FOLDER_LIST_LOADER_ID) {
            PDebug.EndAndStart("FOLDER_LIST_LOADER_ID.initLoader->onCreateLoader",
                    "FOLDER_LIST_LOADER_ID.onCreateLoader->loadInBg");
            loaderName = "FOLDER_LIST_LOADER_ID";
            ...
            PDebug.EndAndStart("ALL_FOLDER_LIST_LOADER_ID.initLoader->onCreateLoader",
                    "ALL_FOLDER_LIST_LOADER_ID.onCreateLoader->loadInBg");
            loaderName = "ALL_FOLDER_LIST_LOADER_ID";
        ...
        return new ObjectCursorLoader<Folder>(mActivity.getActivityContext(), folderListUri,
                UIProvider.FOLDERS_PROJECTION, Folder.FACTORY, loaderName);
        /** @} */
    }
    ...
    public void onLoadFinished(Loader<ObjectCursor<Folder>> loader, ObjectCursor<Folder> data) {
        if (mCursorAdapter != null) {
            if (loader.getId() == FOLDER_LIST_LOADER_ID) {
                PDebug.EndAndStart("FOLDER_LIST_LOADER_ID.loadInBg->onLoadFinished",
                        "FOLDER_LIST_LOADER_ID.onLoadFinished");
                ...
                PDebug.End("FOLDER_LIST_LOADER_ID.onLoadFinished");
            } else if (loader.getId() == ALL_FOLDER_LIST_LOADER_ID) {
                PDebug.EndAndStart("ALL_FOLDER_LIST_LOADER_ID.loadInBg->onLoadFinished",
                        "ALL_FOLDER_LIST_LOADER_ID.onLoadFinished");
                mCursorAdapter.setAllFolderListCursor(data);
                PDebug.End("ALL_FOLDER_LIST_LOADER_ID.onLoadFinished");
            }
        }
    }
    ...
    private class FolderListAdapter extends BaseAdapter implements FolderListFragmentCursorAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            /** M: we should not use mCurrentFolderForUnreadCheck to override the loader's data @ {
            if (type == DrawerItem.VIEW_FOLDER
                    && isSelected
            ...
            /** M: VIP folder item view @{ */
            if (type == DrawerItem.VIEW_FOLDER && item.mFolder.id == Mailbox.TYPE_VIP) {
                mVipFolderItemView = ((FolderItemView) view);
                mVipFolderItemView.overrideUnreadCount(mVipMailCount);
            }
            /** @} */
    ...
    private void setSelectedAccount(Account account){
        ...
        if (changed) {
            ...
            /**
             * M: [Launch Performance] The default case is starting the folder
             * list loaders here. But if the conversation list was not shown, we
             * would start them latter when conversation list load finished. @{
             */
     ...
     * M: VIP member observer to observe the VIP member changes
     */
    private ThrottleContentObserver mVipMemberObserver = null;

    /**
     * M: [Launch Performance] Start the folder list loaders here.
     */
    public void startLoaders() {
        final LoaderManager manager = getLoaderManager();
        manager.destroyLoader(FOLDER_LIST_LOADER_ID);
        PDebug.Start("FOLDER_LIST_LOADER_ID.initLoader->onCreateLoader");
        manager.restartLoader(FOLDER_LIST_LOADER_ID, Bundle.EMPTY, this);
        manager.destroyLoader(ALL_FOLDER_LIST_LOADER_ID);
        PDebug.Start("ALL_FOLDER_LIST_LOADER_ID.initLoader->onCreateLoader");
