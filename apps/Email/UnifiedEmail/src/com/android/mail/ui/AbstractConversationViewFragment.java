...
public abstract class AbstractConversationViewFragment extends Fragment implements
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (activity == null
                /** M: let it go ahead if activity was finishing,  that's ok. || activity.isFinishing()*/) {
            // Activity is finishing, just bail.
            LogUtils.wtf(LOG_TAG, "AbstractConversationViewFragment onActivityCreated : activity == null");
            return;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (!isUserVisible()) {
            ...
            if (LogUtils.isLoggable(LOG_TAG, LogUtils.DEBUG)) {
                //M: the dump has '%' chars in it. so use %s to avoid JE 
                LogUtils.e(LOG_TAG, "dumpFragment(%s)", Utils.dumpFragment(this));
            }
            return false;
        }
    ...
    public void onDestroyView() {
        ...
        /// M: Reset Controller of MessageCursor and itself @{
        if (mCursor != null) {
            mCursor.setController(null);
            mCursor = null;
        }
        /// M: @}
