...
import com.android.mail.utils.EmailConnectivityManager.EmailConntectvityStateChangeListener;
...
public final class ConversationListFragment extends ListFragment implements
        /// M: Implement to observer connectivity state changes if necessary
        EmailConntectvityStateChangeListener,
        OnItemLongClickListener, ModeChangeListener, ListItemSwipedListener, OnTouchListener {
    ...
    void configureSearchResultHeader() {
        ...
        if (showHeader) {
            ...
            /// M: Initially reset the count
            updateRemoteSearchCount(mActivity.getAccountController(), 0);
        }
        mSearchStatusView.setVisibility(showHeader ? View.VISIBLE : View.GONE);
        /// M: Do not need to margin for search result, we will show it on actionBar @{
...
//        mListView.setLayoutParams(layoutParams);
    ...
    private void updateSearchResultHeader(int count) {
        ...
        // Only show the header if the context is for a search result
//        final Resources res = getResources(); ///M: Remove it due to no more used
        final boolean showHeader = ConversationListContext.isSearchResult(mViewContext);
        if (showHeader) {
            /// M: Update remote search result and disable searchView @{
            mSearchStatusView.setVisibility(View.GONE);
            updateRemoteSearchCount(mActivity.getAccountController(), count);
...
//                    .setText(res.getString(R.string.search_results_loaded, count));
    ...
    private void initializeUiForFirstDisplay() {
        ...
        /** M: add logs for smartbook plug-in/out NPE exception @{*/
        if (mSearchStatusView == null) {
            LogUtils.e(LOG_TAG, "CLF.mSearchStatusView is null");
            mActivity.getWindow().getDecorView().debug();
        }
        /** @} */
    }
    ...
    public void onActivityCreated(Bundle savedState) {
        PDebug.Start("CLF.onActivityCreated");
        ...
        if (mActivity.isFinishing()) {
            ...
            /**
             * M: If the activity is finishing, it should un-register the
             * observer for conversation list, because the mFolder for the
             * AnimatedAdapter hadn't initialized at this time, once it changed,
             * it would make the list view to re-draw resulted the JE. @{
             */
            ...
            LogUtils.d(LOG_TAG, "activity=%s is finishing, fragment=%s", mActivity, this);
            /** @} */
            return;
        }
        ...
        /// M: Get default choice mode
        int choice = getDefaultChoiceMode();
        if (savedState != null) {
            ...
            /// M: Don't save choice mode, cause we set it by device sw-dp with portrait/land-scape mode
//            choice = savedState.getInt(CHOICE_MODE_KEY, choice);
        ...
        /// M: Add EmailConnectivityStateChangeListener after Activity been created
        mActivity.getConnMgr().addListener(this);
        /// M: register touch listener to hide keybord in search mode.
        mListView.setOnTouchListener(this);
        PDebug.End("CLF.onActivityCreated");
    }

    /**
     * Returns the default choice mode for the list based on whether the list is displayed on tablet
     * or not.
     * M: In some case, {@link TwoPaneLayout} also display like {@link OnePaneLayout}, eg. tablet is
     * in portrait mode, we should keep the choice mode consistent with phone layout, which is NONE.
     * Mark as un-static function.
     * @return
     */
    private final int getDefaultChoiceMode() {
        return ((AbstractActivityController) mActivity.getFragmentLauncher()).isConversationListCollapsable()
                ? ListView.CHOICE_MODE_NONE : ListView.CHOICE_MODE_SINGLE;
    }
    ...
    public void onDestroyView() {
        ...
        /// M: Remove EmailConnectivityStateChangeListener before destroyed
        mActivity.getConnMgr().removeListener(this);
    ...
    public void onResume() {
        ...
        /// M: Remove sending notification if open the related outbox.Add the situation that mFolder != null.
        if (mFolder != null && mFolder.type == UIProvider.FolderType.OUTBOX) {
            LogUtils.d(LOG_TAG, "remove failed sending notification");
            SendNotificationProxy.getInstance(mActivity.getActivityContext())
                .suspendSendFailedNotification(Long.parseLong(mAccount.uri.getLastPathSegment()));
        }
    }
...
//            outState.putInt(CHOICE_MODE_KEY, mListView.getChoiceMode());
    ...
    public boolean isAnimating() {
        ...
        boolean result = (adapter != null && adapter.isAnimating()) ||
                (mListView != null && mListView.isScrolling());
        /// M: Log for debugging. @ {
        if (DEBUG) {
            LogUtils.d(LOG_TAG, "ConvesationListFragment : isAnimating : %s adapter : %s mListView : %s",
                    result , (adapter == null ? "null" : adapter.toString()),
                    (mListView == null ? "null" : mListView.isScrolling()));
        }
        /// @}
        return result;
    }
    ...
    private void viewConversation(final int position) {
        ...
        /*
         * M: sometimes, the item is not a real conversation, it should skip
         * those case, just return.
         */
        ...
        /// M: log for performance content show time test.
        LogUtils.d(LOG_TAG, "[Performance test][Email][Email_008][%d] Content show time start [%s]",
                conv.id, System.currentTimeMillis());
        /*
    ...
    public void requestDelete(int actionId, final Collection<Conversation> conversations,
        ...
        if (listView.getSwipeAction() == actionId && /* M: do not swipe when id was 0*/ actionId != 0) {
    ...
    private void onFolderStatusUpdated() {
        ...
        if (errorStatus == UIProvider.LastSyncResult.SUCCESS
            ...
            if (folderCount == 0
                    /* M: mFolder's totalCount is not updated, use queryed cusor count. @{ */
    ...
    private void setSwipeAction() {
        ...
        if (swipeSetting == Swipe.DISABLED
                ...
                ///M: If folder is draft, it also can not be swiped.
     ...
     */
    public void clear() {
        /// M: We can't setAdapter(null), cause this may cause no list items displaying problem,
        // since mListAdapter is useless anymore in that case, we should swapCursur(null) instead. @{
    ...
    public void onConnectivityStateChanged(NetworkInfo networkInfo) {
        /**
         * M: Handling when connectivity state changes. It's just a demo here
         * without actually be used
         * TODO: We would do something here when network state changed
         */
        LogUtils.d(LOG_TAG, "onConnectivityStateChanged connected: %s, %s",
                String.valueOf((networkInfo != null ? networkInfo.isConnected()
                        : null)), String.valueOf(networkInfo != null ? networkInfo
                                .getDetailedState() : null));
    }

    /// M: observer touch event, and hide keyboard in search mode when read search result.
    @Override
    ...
    /**
     * M: Update remote search result count.
    ...
    /**
     * M: update local search result count.
     */
