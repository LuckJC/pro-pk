...
public class MimeMessage extends Message {
    ...
    /// M: Using received date for Date field if it was null in some MimeMessage text.
    ...
    /// M: add some date format to parse not standard format date.
    ...
    public Date getReceivedDate() throws MessagingException {
        /// M: add implementation for this function. @{
        if (mReceivedDate == null) {
            String receivedString = getFirstHeader("Received");
            if (receivedString == null) {
                return null;
            }
            String[] parts = MimeUtility.unfold(receivedString).split(";");
            parts = MimeUtility.mergeIncorrectlySplit(parts, ";");
            if (parts.length > 1) {
                // Cause the field "Received" contain date(but not match standard parameter format),
                ...
                try {
                    ...
                    mReceivedDate = externalDateFormatParse(receivedDateString);
                }
            }
        }
        /// @}
        Log.d(TAG, " getReceivedDate : parse result = " + mReceivedDate);
    ...
    public Date getSentDate() throws MessagingException {
        ...
        if (mSentDate == null) {
            dateString = getFirstHeader("Date");
            if (dateString != null) {
                try {
                    DateTimeField field = (DateTimeField) Field.parse("Date: "
                    ...
                    mSentDate = externalDateFormatParse(dateString);
                }
            }
        }
        if (mSentDate == null) {
            dateString = getFirstHeader("Delivery-date");
            if (dateString != null) {
                try {
                    DateTimeField field = (DateTimeField) Field.parse("Date: "
                            + MimeUtility.unfoldAndDecode(dateString));
    ...
    /**
     * M: try to using other date format parse it.
     * @param dateString
     * @return
     */
    private Date externalDateFormatParse(String dateString) {
        Date date = null;
        try {
            date = SIMPLE_DATE_FORMAT_1.parse(dateString);
            return date;
        } catch (ParseException e) {
            Log.d(TAG, "round 1 parse date string:" + dateString + " failed");
        }
        try {
            date = SIMPLE_DATE_FORMAT_2.parse(dateString);
            return date;
        } catch (ParseException e) {
            Log.d(TAG, "round 2 parse date string:" + dateString + " failed");
        }
        try {
            date = SIMPLE_DATE_FORMAT_3.parse(dateString);
        } catch (ParseException e) {
            Log.d(TAG, "round 3 parse date string:" + dateString + " failed");
        }
        return date;
    }
