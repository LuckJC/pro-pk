...
public class EmailInfoDataCollector {
    ...
    public static String[] getBodyHtmlText(Context context, long messageId){
        ...
        Cursor c = null;
        try {
            c = resolver.query(BODY_CONTENT_URI,
                               BODY_CONTENT_PROJECTION,
                               MESSAGE_KEY + " = ? ",
                               new String[]{String.valueOf(messageId)},
                               null);
            if (null == c || c.getCount() == 0){
                Log.i(TAG, "Get body text and html failed.");
                return null;
            }
            info = new String[2];
            while (c.moveToNext()) {
                info[BODY_TEXT_CONTENT_COLUMN] = c.getString(BODY_TEXT_CONTENT_COLUMN);
                info[BODY_HTML_CONTENT_COLUMN] = c.getString(BODY_HTML_CONTENT_COLUMN);
            }
        } catch (Exception e) {
