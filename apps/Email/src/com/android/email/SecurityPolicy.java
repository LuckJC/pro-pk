...
public class SecurityPolicy {
    ...
    /// M: Projection used for getting email address for an account
    ...
    /*package*/ void deleteSecuredAccounts(Context context) {
        ...
        Cursor c = cr.query(Account.CONTENT_URI, ACCOUNT_EMAIL_PROJECTION,
                Account.SECURITY_NONZERO_SELECTION, null, null);
        /** M: AccountManager should be the unified account deletion entrance.
         * Hence, delete AccountManager account instead of EmailProvider's, in
         * order to guarantee the account reconciliation. @{ */
        if (c == null) {
            LogUtils.e(TAG, "Failed to get cursor in deleteSecuredAccounts");
            return;
        }
        List<android.accounts.Account> amAccounts = AccountReconciler.getAllAmAccounts(mContext);
        try {
            LogUtils.w(TAG, "Email administration disabled; deleting " + c.getCount() +
                    " secured account(s)");
            while (c.moveToNext()) {
                String address = c.getString(0);
                for (android.accounts.Account acct : amAccounts) {
                    if (address != null && address.equalsIgnoreCase(acct.name)) {
                        AccountManager.get(mContext).removeAccount(acct, null, null);
                        break;
                    }
                }
            }
