...
public class AccountReconciler {
     ...
     */
    public static List<android.accounts.Account> getAllAmAccounts(final Context context) {
        ...
        /// M: In strings.xml, pop3 type and legacy imap type are the same "com.android.email".
         // This is weird and lead to POP3 type can not be shown in system account settings UI.
         // TODO: It's a bug that POP3 account be shown in "IMAP" category, but set it aside at present.
         // Here just avoid to mistakenly retrieve 2 accounts from the one and the only account.
        String legacyImapType = context.getString(R.string.account_manager_type_legacy_imap);
        String pop3Type = context.getString(R.string.account_manager_type_pop3);
        builder.addAll(Arrays.asList(am.getAccountsByType(legacyImapType)));
        if (legacyImapType != null && !legacyImapType.equals(pop3Type)) {
            builder.addAll(Arrays.asList(am.getAccountsByType(pop3Type)));
        }
    ...
    private static boolean reconcileAccountsInternal(
        ...
        ///M: Disable Accounts Reconcile during TestCase
        ...
        if (accountDeleted) {
            ...
            /*
             * M: reconcile the accounts cache in MailAppProvider with which in
             * AccountMannager.
             */
