...
public final class DBHelper {
    ...
    /// M: Version 121: Add dirty flag field to message (for Bad sync key recovery)
    /// M: Version 122: Add smartpush table
    /// M: Version 123: Add the VIP Member table
    // Version 124: Add mainMailboxKey, which will be set for messages that are in the fake
    ...
    // Version 125: Need to update Message_Updates and Message_Deletes to match previous.
    // Version 126: Changed the duplicateMesage deletion trigger to ignore accounts that aren't
    //              exchange accounts.
    ...
    static void createQuickResponseTable(SQLiteDatabase db) {
     ...
     /// M: Create Smart push habit data table
    static void createSmartPushTable(SQLiteDatabase db) {
        String s = " (" + EmailContent.RECORD_ID + " integer primary key autoincrement, "
                ...
                + ");";
        db.execSQL("create table if not exists " + SmartPush.TABLE_NAME + s);
    }
    ...
    /** M: Support VIP Feature. If the VIP member table was not exist, create it. @{ */
    ...
    protected static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        @SuppressWarnings("deprecation")
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            LogUtils.d(TAG, "DBHelper upgrading EmailProvider.db from " + oldVersion + " to " + newVersion);
            ...
            /// M: MTK JB2 Email latest database version is 44
             // TODO: If customer's version is more than 44(they developed other features)
             // how can we do?
            if (oldVersion <= 44) {
                upgradeToEmail2(db);
            }

            if (oldVersion > 42 && oldVersion <= 44) {
                upgradeJB2SmartPush(db);
            }
            ...
            /// M: For bad sync key recovery, add "dirty" field in Message table
            if (oldVersion <= 120) {
                try {
                    db.execSQL("alter table " + Message.TABLE_NAME
                            + " add column " + MessageColumns.DIRTY + " text;");
                } catch (SQLException e) {
                    /// M: May reach here if upgrade from MTK JB2 Email because "dirty" is
                     // existed already
            ...
            if (oldVersion <= 121) {
                try {
                    ...
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 121 to 122 ", e);
                }
            }
            /** @} */

            /** M: Add vip member table to support VIP feature @{ */
            if (oldVersion <= 122) {
                try {
                    createVipMemberTable(db);
                } catch (SQLException e) {
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 122 to 123 ", e);
                }
            }
            /** @} */
    ...
    /**
     * M: For smart push account in MTK JB2 Email, the account sync interval is
     * "-5" and the mailboxes(inbox, calendar, contacts) sync interval are "-2" or "-3".
     * Adjust them to KK new values.
     * @param db
     */
    private static void upgradeJB2SmartPush(SQLiteDatabase db) {
        try {
            // JB2 Email sync interval definitions
            final String smartPushInterval = "-5";
            final int pushInterval = -2;
            // Loop through accounts
            Cursor accountCursor = db.query(Account.TABLE_NAME, new String[]{
                    Account.ID, Account.FLAGS, Account.SYNC_INTERVAL}, null, null, null, null, null);
            if (accountCursor == null) {
                LogUtils.w(TAG, "Failed to get account cursor in upgrading " +
                    "EmailProvider.db from MTK JB2");
                return;
            }
            ...
            try {
                ContentValues cv = new ContentValues();
                while (accountCursor.moveToNext()) {
                    int flags = accountCursor.getInt(1);
                    if (smartPushInterval.equalsIgnoreCase(accountCursor.getString(2))) {
                        mailboxCursor = db.query(Mailbox.TABLE_NAME, new String[]{
                                Mailbox.SYNC_INTERVAL}, Mailbox.ACCOUNT_KEY + "=? AND " + Mailbox.TYPE + "=?",
                                    new String[]{String.valueOf(accountCursor.getInt(0)),
                                        String.valueOf(Mailbox.TYPE_INBOX)}, null, null, null);
                        if (mailboxCursor == null) {
                            LogUtils.w(TAG, "Failed to get mailbox cursor in upgrading " +
                                "EmailProvider.db from MTK JB2");
                            return;
                        }
                        if (mailboxCursor.moveToFirst()) {
                            int interval = mailboxCursor.getInt(0);
                            // KK Email only need to set account interval. If JB2 Email inbox
                            // interval is push/ping like.
                            if (interval <= pushInterval) {
                                interval = Account.CHECK_INTERVAL_PUSH;
                            }
                            cv.put(AccountColumns.SYNC_INTERVAL, interval);
                            flags |= Account.FLAGS_SMART_PUSH;
            ...
            // Shouldn't be needed unless we're debugging and interrupt the process
            LogUtils.w(TAG, "Exception upgrading EmailProvider.db from MTK JB2" + e);
        }
    }
}
