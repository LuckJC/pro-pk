...
public class EmailProvider extends ContentProvider {
    ...
    /// M: local search
    private static final int UI_LOCAL_SEARCH = UI_BASE + 19;
   ...
   // @}
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        try {
            ...
            if (c == null) {
                ...
                LogUtils.w(TAG,
                        "Query returning null for uri: %s, selection: %s", uri,
                        selection);
            }
        }
    ...
    private boolean isUpdatingAccountSyncOption(SQLiteDatabase db, ContentValues values, String accountId) {
        Integer newSyncInterval = values.getAsInteger(Account.SYNC_INTERVAL);
        Integer newLookback = values.getAsInteger(Account.SYNC_LOOKBACK);
        if (newSyncInterval == null && newLookback == null) {
            return false;
        }
                ...
                case BODY_LARGE:
                    ...
                    if (match == BODY_LARGE) {
                        ...
                        if (compressedBody != null) {
                            values.put(Body.TEXT_CONTENT, StringCompressor.stringFromBytes(compressedBody));
                        }
                        compressedBody = values.getAsByteArray(Body.HTML_CONTENT);
                        if (compressedBody != null) {
                            values.put(Body.HTML_CONTENT, StringCompressor.stringFromBytes(compressedBody));
                        }
                    }
                    /** M: @} */
    ...
    private static final String MESSAGE_SENDING_STATUS =
        "CASE WHEN (" + MessageColumns.FLAGS + "&" + Message.FLAG_STATUS_SENDING +
            ") !=0 THEN " + UIProvider.MessageSendingState.SENDING +
            " ELSE " + UIProvider.MessageSendingState.NOT_SENDING + " END";
    ...
    private static ProjectionMap getMessageViewMap() {
        if (sMessageViewMap == null) {
            sMessageViewMap = ProjectionMap.builder()
                ...
                .add(UIProvider.MessageColumns.IS_SENDING, MESSAGE_SENDING_STATUS)
                /// M: The extend flags of message
...
//                    }
                    ...
                    uiAtts.add(uiAtt);
                    LogUtils.d(TAG, "GenQueryViewMessage Attachment: %d, State: %d, attUri: %s, ContentUri: %s"
                            , att.mId, att.mUiState, uiAtt.uri, att.getContentUri());
                    /// M: @}
                }
    ...
    private static void appendConversationInfoColumns(final StringBuilder stringBuilder) {
        ...
        stringBuilder.append(',').append(MessageColumns.DISPLAY_NAME)
                ...
                .append(',').append(MessageColumns.REPLY_TO_LIST);
    }
    ...
    private static String genQueryMailboxMessages(String[] uiProjection, final boolean unseenOnly, String selection) {
        ...
        sb.append("AND " + Message.MAILBOX_KEY + "=? ");
    ...
    private void addCombinedAccountRow(MatrixCursor mc) {
        ...
        if (colPosMap.containsKey(UIProvider.AccountColumns.SettingsColumns.AUTO_ADVANCE)) {
            ...
            values[colPosMap.get(UIProvider.AccountColumns.SettingsColumns.AUTO_ADVANCE)] =
                    Integer.toString(autoAdvanceToUiValue(prefs.getAutoAdvanceDirection()));
        }
    ...
    private Object[] getVirtualMailboxRow(long accountId, int mailboxType) {
        ...
        values[UIProvider.FOLDER_LOCALSEARCH_LIST_URI_COLUMN] = combinedUriString("uilocalsearch",
                idString);
        /// M: Add type into folder for further usage
        values[UIProvider.FOLDER_TYPE_COLUMN] = getFolderTypeFromMailboxType(mailboxType);
        ...
        switch (mailboxType) {
            ...
            case Mailbox.TYPE_UNREAD: {
                ...
                final int starredCount = EmailContent.count(getContext(), Message.CONTENT_URI,
                        ...
                        /** M: get the real unread number here and mail is not from trash @} */
                        + " AND " + MessageColumns.FLAG_READ + "=0"
                        + " AND " + MessageColumns.MAILBOX_KEY + " NOT IN (SELECT " + MailboxColumns.ID
                        + " FROM " + Mailbox.TABLE_NAME + " WHERE " + MailboxColumns.TYPE + " = "
                        + Mailbox.TYPE_TRASH + ")", whereArgs);
                        /** @} */
            ...
            /** M: Support VIP folder @{ */
                ...
                values[UIProvider.FOLDER_UNREAD_COUNT_COLUMN] = 0;
                // Add the icon
                values[UIProvider.FOLDER_ICON_RES_ID_COLUMN] = R.drawable.ic_folder_vip_holo_light;
                break;
            /** @} */
            /// M: add for local search. @{
            } default:
    ...
    private Cursor uiVirtualMailboxes(final String id, final String[] uiProjection) {
        ...
        if (id.equals(COMBINED_ACCOUNT_ID_STRING)) {
            ...
            mc.addRow(getVirtualMailboxRow(COMBINED_ACCOUNT_ID, Mailbox.TYPE_VIP));
        } else {
            ...
            ///M: Add the VIP folder
            mc.addRow(getVirtualMailboxRow(acctId, Mailbox.TYPE_VIP));
        }
    ...
    static class EmailConversationCursor extends CursorWrapper {
        ...
        public EmailConversationCursor(final Context context, final Cursor cursor,
            ...
            if (mailbox != null) {
                ...
                /** M: Recoder the value. @{ */
                if ((mailbox.mFlags & Mailbox.FLAG_ALL_MESSAGES_DOWNLOADED) == Mailbox.FLAG_ALL_MESSAGES_DOWNLOADED) {
                    mExtras.putBoolean(UIProvider.CursorExtraKeys.EXTRA_MESSAGES_LOAD_FINISH, true);
                } else {
                    mExtras.putBoolean(UIProvider.CursorExtraKeys.EXTRA_MESSAGES_LOAD_FINISH, false);
                }
                /** @} */
                /// M: print mailbox syncing status related fields for debug.
                LogUtils.d(Logging.LOG_TAG, " EmailConversationCursor : mailbox.mUiSyncStatus : "
                        + mailbox.mUiSyncStatus
                        + " System.currentTimeMillis() - mailbox.mSyncTime : "
                        + (System.currentTimeMillis() - mailbox.mSyncTime) + " mailbox.mSyncInterval : "
                        + mailbox.mSyncInterval);
                 if (mailbox.mUiSyncStatus == EmailContent.SYNC_STATUS_BACKGROUND
                    ...
                    mExtras.putInt(UIProvider.CursorExtraKeys.EXTRA_STATUS,
                     ...
                     if (mailbox.mSyncInterval == 0
                             ...
                             /// M: If the mUiLastSyncResult is STORAGE_ERROR, should reset the
        ...
        /**M: for vip message , we need filter the cursor firstly @{ */
        protected String filterMessageForVip(Cursor cursor) {
        ...
        public Bundle respond(Bundle params) {
            ...
            if (params.containsKey(setVisibilityKey)) {
                ...
                if (visible) {
                    ...
                    /**
                     * M: Modified for VIP notification.Enter VIP folder from
                     * notification, set message as seen.@{
                     */
                    if (params.getBoolean(UIProvider.ConversationCursorCommand.COMMAND_KEY_FILTER_VIP_MSG)) {
                        LogUtils.d(TAG, "ECC set vip msgs read %s", this);
                        resolver.update(uri, contentValues, MessageColumns.ID
                                + " IN (" + filterMessageForVip(this) + ")",
                                null);
                        /** @} */
                    } else {
            ...
            /** M: Use to check if it's vip mailbox */
            final String isVipMailbox =
                    UIProvider.ConversationCursorCommand.COMMAND_KEY_IS_VIP_MAILBOX;
            if (params.containsKey(isVipMailbox)) {
                if (getVirtualMailboxTypeById(mMailboxId) == Mailbox.TYPE_VIP) {
                    response.putBoolean(isVipMailbox, true);
                }
                return response;
            }
            /** @} */
        ...
        private ConversationInfo generateConversationInfo() {
            ...
            String fromString = getString(getColumnIndex(MessageColumns.FROM_LIST));
            final String email;
            ...
            if (!TextUtils.isEmpty(fromString)) {
                final Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(Address.unpackToString(fromString));
                if (tokens.length > 0) {
                    email = tokens[0].getAddress();
                    senderString = tokens[0].getName();
                } else {
                    LogUtils.d(TAG, "Couldn't parse email address");
                    email = fromString;
                    senderString = getString(getColumnIndex(MessageColumns.DISPLAY_NAME));
                }
            } else {
                /**
                 * ConversationInfo will show on ConverationList and ConversationView
                 * From will be null sometimes which make "Account" or "Me" displayed as Sender
                 * From == null:
                 *      Reply-To != null:
                 *          Reply-To as Sender Info
                 *      else:
                 *          Sender Info null and use "Me" instead
                 * Reply-To will be show as Sender when From is null, which is much better for UX
                 */
                fromString = getString(getColumnIndex(MessageColumns.REPLY_TO_LIST));
                Address address = Address.unpackFirst(fromString);
                if (address != null) {
                    email = address.getAddress();
                    senderString = address.getPersonal();
                } else {
                    email = null;
                    senderString = null;
                }
    ...
    public static com.android.mail.providers.Account getAccount(Context context, long accountId) {
        ...
        try {
            if (ac.moveToFirst()) {
                account = new com.android.mail.providers.Account(ac);
            }
        } finally {
            ac.close();
        }
    ...
    private Uri uiSaveMessage(Message msg, Mailbox mailbox, Bundle extras) {
        ...
        synchronized (AttachmentUtilities.SYNCHRONIZE_LOCK_FOR_FORWARD_ATTACHMENT) {
            for (com.android.mail.providers.Attachment uiAtt : uiAtts) {
                ...
                LogUtils.d(TAG, "Save draft with Attachment: %s", (attUri != null ? attUri.toSafeString() : "Null"));
                    ...
                    if (att != null) {
                        if (att.mMessageKey == msg.mId) {
                            ...
                            if (!Utility.attachmentExists(context, attClone)) {
                                LogUtils.d(TAG, "Attachment %s dosen't exist, need to be downloaded!", attClone.mId);
            ...
            if (!msg.isSaved()) {
                ...
                String delUselessAttSelection = " ( " + AttachmentColumns.ID + " NOT IN (" + arg + ") )";
        ...
        if (msg.mAttachments != null && DraftType.NOT_A_DRAFT != draftType) {
            ...
            for (Attachment att : attachments) {
                ...
                if (AttachmentUtilities.CACHE_FAIL == result) {
                    ...
                    LogUtils.d(TAG, "No need to cach Attachment %s", att.getContentUri());
                }
            }
    ...
    private Uri uiSaveDraftMessage(final long accountId, final Bundle extras) {
        ...
        if (extras.containsKey(BaseColumns._ID)) {
            ...
            if (msg == null) {
                LogUtils.w(TAG, "uiSaveDraftMessage failed, for message [%d] not found.", messageId);
                return null;
            }
            /// @}
        } else {
    ...
    private void notifyUIFolder(final String folderId, final long accountId) {
        ...
        /// M: some virtual mailbox need updated too.
    ...
    private void updateAccountSyncInterval(final long accountId, final ContentValues values) {
        ...
        } else if (syncInterval != AccountSetupOptions.NEVER_MENU_ITEM_VALUE) {
    ...
    private static void startSyncBackgroundForMailbox(final android.accounts.Account account,
        ...
        LogUtils.i(TAG, "requestSync EmailProvider startSyncBackgroundForMailbox %s, %s",
    ...
    private static void startSyncBackground(final android.accounts.Account account) {
        ...
        ContentResolver.requestSync(account, EmailContent.AUTHORITY, extras);
        LogUtils.i(TAG, "requestSync EmailProvider startSyncBackground %s, %s", account.toString(),
                extras.toString());
    }
    ...
    private void notifiyVirtualMailbox(long accountId, int type) {
        ...
        if (type == Mailbox.TYPE_VIP) {
            notifyUI(VipMember.UIPROVIDER_VIPMEMBER_NOTIFIER, null);
        } else {
            notifyUI(UIPROVIDER_CONVERSATION_NOTIFIER,
