...
public class AccountSetupOptions extends AccountSetupActivity implements OnClickListener {
    ...
    /** M: This two string is used to save the shown state of smartpush dialog mainly
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //M: Change the WIFI download attachment default to false
        ...
        /// M: Determine the shown state of smart push dialog based on the SavedInstanceState.
        ...
        if (mServiceInfo.protocol.equalsIgnoreCase(HostAuth.LEGACY_SCHEME_EAS)) {
            mCheckFrequencyView.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                @Override
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    /// M: The following code guarantees the dialog can be shown as expected
                    // after rotating the screen or changing the system language.
    ...
    private void onDone() {
        ...
        Integer frequency = (Integer)((SpinnerOption)mCheckFrequencyView
                .getSelectedItem()).value;
        /// M: If user selected smart push, set account sync interval as push.
         // The sync interval would be dynamically changed by SmartPushService
        if (frequency == SMART_PUSH_MENU_ITEM_VALUE) {
            account.setSyncInterval(PUSH_MENU_ITEM_VALUE);
            account.mFlags |= Account.FLAGS_SMART_PUSH;
        } else {
            account.setSyncInterval(frequency);
        }
        ...
        /// M: It's possible to create 2 same exchange accounts(e.g. simultaneously create 1
         // from email UI and create another from Contacts). We need to check duplicated account
         // again at this very end.
        new DuplicateCheckTask(this, account.getEmailAddress())
            .executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }

    /**
     * M: Async task to check for duplicate account and
     * then either alerts the user, or continues.
     */
    private class DuplicateCheckTask extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String duplicateAccountName) {
            ...
            if (!AccountSetupOptions.this.isResumed()) {
                return;
            }
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            .setPositiveButton(getString(android.R.string.ok), new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    ((AccountSetupOptions)getActivity()).mSmartPushDialogShowing = false;
