...
public class AccountSettingsFragment extends PreferenceFragment
    ...
    /// M: ListPreference sync_window named string.
    private static final String PREFERENCE_SYNC_WINDOW = "sync_window";
    ...
    /** M: record/store account protocol.@{ */
    private static final String BUNDLE_KEY_PROTOCOL = "key_account_protocol";
    private static final String EXCHANGE_PROTOCOL = "eas";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAccountDirty = false;
        /** M: update check frequency entries if the saved account protocol is "eas".@{*/
        mCheckFrequency = (ListPreference) findPreference(PREFERENCE_FREQUENCY);
        if (savedInstanceState != null) {
            String protocol = savedInstanceState.getString(BUNDLE_KEY_PROTOCOL);

            if (EXCHANGE_PROTOCOL.equals(protocol)) {
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        if (mCheckFrequency.getDialog() != null) {
            outState.putString(BUNDLE_KEY_PROTOCOL, mProtocol);
        }
    ...
    private void loadSettings() {
        ...
        if (info.syncContacts || info.syncCalendar) {
            ...
            if ((mAccount.mFlags & Account.FLAGS_SMART_PUSH) != 0) {
                mCheckFrequency.setValue(String.valueOf(AccountSetupOptions.SMART_PUSH_MENU_ITEM_VALUE));
            } else {
        ...
        /// M: Get this ListPreference from the fragment's layout.
...
*/
    ...
    private void saveSettings() {
        ...
        if (info.syncContacts || info.syncCalendar) {
            ...
            // The "sync frequency" preference really just needs to control the frequency value
            // in our database.
            Integer interval = Integer.parseInt(mCheckFrequency.getValue());
            ...
            if (interval == AccountSetupOptions.SMART_PUSH_MENU_ITEM_VALUE) {
                interval = AccountSetupOptions.PUSH_MENU_ITEM_VALUE;
                newFlags |= Account.FLAGS_SMART_PUSH;
            } else {
                newFlags &= ~Account.FLAGS_SMART_PUSH;
            }
            mAccount.setSyncInterval(interval);
        } else {
            // This account only syncs email (not contacts or calendar), which means that we will
            // hide the preference to turn syncing on and off. In this case, we want the sync
            // frequency preference to also control whether or not syncing is enabled at all. If
