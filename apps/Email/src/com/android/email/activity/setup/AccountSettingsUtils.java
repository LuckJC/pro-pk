...
public class AccountSettingsUtils {
    ...
    /**
     * M: Auto-set the config info for an email account
     ...
     * @param protocol The protocol that user selected.
     * @return suitable server name definition, or null if no match found.
     */
    public static String findManualInfoForDomain(Context context, String domain, String protocol) {
        ...
        try {
            ...
            while ((xmlEventType = xml.next()) != XmlResourceParser.END_DOCUMENT) {
                if (xmlEventType == XmlResourceParser.START_TAG
                    ...
                    try {
                        ...
                        Logging.w(Logging.LOG_TAG, "providers line: " + xml.getLineNumber() +
                                "; Domain contains multiple globals");
                    }
                } else if ((xmlEventType == XmlResourceParser.START_TAG
                        && "pop3support".equals(xml.getName()) && isMatched && HostAuth.LEGACY_SCHEME_POP3
                            .equals(protocol))
                        || (xmlEventType == XmlResourceParser.START_TAG
                                && "imapsupport".equals(xml.getName()) && isMatched && HostAuth.LEGACY_SCHEME_IMAP
                                    .equals(protocol))
                        || (xmlEventType == XmlResourceParser.START_TAG
                                && "eassupport".equals(xml.getName()) && isMatched && HostAuth.LEGACY_SCHEME_EAS
                                    .equals(protocol))
                        || (xmlEventType == XmlResourceParser.START_TAG
                                && "smtpsupport".equals(xml.getName()) && isMatched && HostAuth.LEGACY_SCHEME_SMTP
                                    .equals(protocol))) {
            ...
            Logging.e(Logging.LOG_TAG, "Error while trying to load manual_setup_helper settings.",
                    e);
    ...
    public static Provider findProviderForDomain(Context context, String domain) {
        ...
        /** M: add for extension provider. @{ */
        if (p == null) {
            IServerProviderPlugin extension = OPExtensionFactory.getProviderExtension(context);
            if (extension.isSupportProviderList()) {
                ...
                p = findProviderForDomain(pluginContext, domain, extensionProviderXml);
            }
        }
        /** @} */
        return p;
    ...
    /** M: Add for auto match emailAddress when set up account.@{ */
    ...
    public static ArrayList<String> collectAutoSetupDomain(Context context) {
        ...
        try {
            ...
            LogUtils.e(Logging.LOG_TAG,
                    "Catch XmlPullParserException while trying to load manual_setup_helper settings.", ex);
