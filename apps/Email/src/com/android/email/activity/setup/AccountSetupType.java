...
public class AccountSetupType extends AccountSetupActivity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /** M: if support CT provider plugin and user selected exchange, skip this screen
         * and "click" EAS. @{ */
        boolean selectedExchangefromChooseESP = AccountSetupChooseESP
                .isSelectedExchange(mExtension);
        if (selectedExchangefromChooseESP) {
            onSelectProtocol(HostAuth.LEGACY_SCHEME_EAS);
            return;
        }
        ...
        for (EmailServiceInfo info: EmailServiceUtils.getServiceInfoList(this)) {
            if (EmailServiceUtils.isServiceAvailable(this, info.protocol)) {
                ...
                /// M: if support CT provider plugin, skip exchange button, just user selected "exchange"
                // can create exchange account, and will not go to here.
                if (HostAuth.LEGACY_SCHEME_EAS.equalsIgnoreCase(info.protocol)
                        && mExtension.isSupportProviderList()) {
                    continue;
                }
    ...
    /**
     * M: set mLogin before we replace the server address
     */
    ...
    /**
     * M: setup different information according to different types of protocol
     * @param protocol
     */
    private void onSelectProtocol(String protocol) {
        final Account account = mSetupData.getAccount();
        final EmailServiceInfo info = EmailServiceUtils.getServiceInfo(this, protocol);
        final HostAuth recvAuth = account.getOrCreateHostAuthRecv(this);
        recvAuth.setConnection(protocol, recvAuth.mAddress, recvAuth.mPort,
                (HostAuth.LEGACY_SCHEME_EAS.equals(protocol)) ? recvAuth.mFlags | HostAuth.FLAG_SSL
                        : recvAuth.mFlags);
        ...
        if (serverName != null) {
            ...
            //M: pop3 and imap account, set Email address as the login name.
            recvAuth.mLogin = recvAuth.mLogin + "@" + recvAuth.mAddress;
            recvAuth.mAddress = AccountSettingsUtils.inferServerName(this, recvAuth.mAddress,
                    info.inferPrefix, null);
        }
        ...
        switch (v.getId()) {
            ...
            default:
                if (!mButtonPressed) {
                    mButtonPressed = true;
                    /// M: Auto config setup info @{
                    onSelectProtocol((String)v.getTag());
                    /// @}
                }
                break;
        }
