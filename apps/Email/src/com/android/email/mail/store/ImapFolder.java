...
public class ImapFolder extends Folder {
    ...
    /// M: Add this variable for partial download.
    ...
    /**
     * M: Get remote message from server according mesageId.
     */
    public Message getRemoteMessage(String messageId) throws MessagingException {
        String searchString = "HEADER Message-Id \"" + messageId + "\"";
        String[] newIdArray = searchForUids(searchString);
        Message remoteMessage = null;
        if (newIdArray.length > 0) {
            remoteMessage = getMessage(newIdArray[0]);
        }
    ...
    public Message[] getMessages(SearchParams params, MessageRetrievalListener listener)
        ...
        if (field.equalsIgnoreCase(SearchParams.SEARCH_FIELD_ALL)) {
            // Break the command up into pieces ending with the string literal
            // length
        ...
        checkOpen();
        LogUtils.logFeature(LogTag.SEARCH_TAG, " imap remote search, command %s",
                commands);
    ...
    // CHECKSTYLE:OFF
    /**
     * M: Supports FetchProfile.Item.BODY and FetchProfile.Item.STRUCTURE with EmailSize
     */
    @Override
    public void fetch(Message[] messages, FetchProfile fp, MessageRetrievalListener listener, int fetchSize)
            throws MessagingException {
        try {
        ...
        if (fetchPart != null) {
            ...
            if (partIds != null) {
                fetchFields.add(ImapConstants.FETCH_FIELD_BODY_PEEK_BARE
                        + "[" + partIds[0] + "]" + (mFetchSize > 0 ? String.format("<0.%d>", mFetchSize) : ""));
            }
        }
     ...
     */
    private Body decodeBody(ImapString imapString, String contentTransferEncoding, int size,
        ...
        try {
            ...
            if (listener != null) {
                ...
                listener.loadAttachmentProgress(count > 0 ? 100 : 0);
            }
            /**@}*/
...
//                        + " not yet supported.");
                LogUtils.w(LogUtils.TAG,"BODYSTRUCTURE " + MimeUtility.MIME_TYPE_RFC822
