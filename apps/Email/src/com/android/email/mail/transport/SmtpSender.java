...
public class SmtpSender extends Sender {
    ...
    public void sendMessage(long messageId) throws MessagingException {
        LogUtils.logFeature(LogTag.SENDMAIL_TAG, " >>> Smtp send message start id [%s]", messageId);
        close();
        open();
        ...
        try {
            ...
            LogUtils.logFeature(LogTag.SENDMAIL_TAG, " <<< Smtp send message success [%s]", message);
        } catch (IOException ioe) {
