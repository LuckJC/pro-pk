...
public class EmailBroadcastProcessorService extends IntentService {
    ...
    ///M: for Testcase Only
    public static final String ACTION_SWITCH_TESTCASE = "com.android.email.action.SWITCH_TESTCASE";
    private static final String STATUS_TESTCASE_SWITCH_ON = "testcase.switch.on";
    ...
    public static void processBroadcastIntent(Context context, Intent broadcastIntent) {
        LogUtils.d(Logging.LOG_TAG, "processBroadcastIntent");
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        LogUtils.d(Logging.LOG_TAG, "onHandleIntent start:action = %s", action);
        if (ACTION_BROADCAST.equals(action)) {
            ...
            LogUtils.d(Logging.LOG_TAG, "onHandleIntent:broadcastAction = %s", broadcastAction);
            ///M: The number of unread Email is also displayed after you clear data.
            ...
            /** M: The number of unread Email is also displayed after you clear data. @{*/
                ...
                onSettingsPackageClearData();
            /** @} */
            /** M: process storage low/ok intents @{ */
            ...
            onAppUpgrade();
        ///M: for Testcase only !!!
        } else if (ACTION_SWITCH_TESTCASE.equals(action)) {
            boolean isOpen = intent.getBooleanExtra(STATUS_TESTCASE_SWITCH_ON,
                    false);
            LogUtils.d(Logging.LOG_TAG, "ACTION_SWITCH_TESTCASE " + isOpen);
            if (isOpen) {
        ...
        LogUtils.d(Logging.LOG_TAG, "onHandleIntent end = %s", action);
    }
    ...
    private void onAppUpgrade() {
        ...
        /// M:(Migrate MR1 change)When upgrading to a version that changes the protocol strings, we need to essentially
    ...
    private void reconcileAndStartServices() {
        /** M: after ACTION_UPGRADE_BROADCAST is received, onAppUpgrade will be called once, which
         * will disable the EmailUpgradeBroadcastReceiver component, and this will make onAppUpgrade
         * useless, see {@link #onAppUpgrade()}, so don't call it here @{
         */
        /**
    ...
    /** M: @{The number of unread Email is also displayed after you clear data. */
    private void onSettingsPackageClearData() {
        Intent i = new Intent();
        i.setAction(Intent.MTK_ACTION_UNREAD_CHANGED);
        i.putExtra(Intent.MTK_EXTRA_UNREAD_NUMBER, 0);
        ...
        LogUtils.d(Logging.LOG_TAG,
                "!!!onSettingsPackageClearData: send broadcastto laucher : unreadCount = " + 0);

        android.provider.Settings.System.putInt(this.getContentResolver(),
                "com_android_email_mtk_unread", 0);
    }
    /**@}*/
