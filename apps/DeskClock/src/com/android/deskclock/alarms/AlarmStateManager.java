...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public static void updateNextAlarm(Context context) {
        ...
        /// M: Enable power-off alarm when device Unencrypted and tablet multi-user owner @{
        if (nextAlarm != null && PowerOffAlarm.deviceUnencrypted() && !isNotTabletOwner()) {
            ...
            /// M: Power off alarm should use the different pendingIntent to normal alarm
            PendingIntent pendingIntent = PendingIntent.getBroadcast(context, nextAlarm.hashCode(),
                    stateChangeIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
            Log.v("Set for PowerOffAlarm alarmType = 8, time = "
            ...
            /// M: Add for power_off_alarm to backup the external ringtone
    ...
    public static Intent createStateChangeIntent(Context context, String tag,
        ...
        Log.v("createStateChangeIntent GlobalIntentId = " + getGlobalIntentId(context));
        if (state != null) {
    ...
    public static void setMissedState(Context context, AlarmInstance instance) {
        ...
        if (PowerOffAlarm.bootFromPoweroffAlarm()) {
            ...
            Log.v("PowerOffAlarm setMissedState hasTimeout = " + hasTimeout);
            if (hasTimeout) {
                Log.v("PowerOffAlarm timeout and set the alarm to snoozed state");
    ...
    private void handleIntent(Context context, Intent intent) {
        ...
        if (instance == null) {
            Log.e("Can't change state for unknown instance: " + uri);
            return;
        }
        if (CHANGE_STATE_ACTION.equals(action)) {
            ...
            Log.v("IntentId: " + intentId + " GlobalId: " + globalId + " AlarmState: " + alarmState);
    ...
    /** M: Used to get if the tablet multi-user owner @{ */
    private static boolean isNotTabletOwner() {
        ...
        if (isTablet && (UserHandle.myUserId() != UserHandle.USER_OWNER)) {
            Log.v("It is tablet MultiUser, but not the Owner");
            return true;
        }
        return false;
    }
