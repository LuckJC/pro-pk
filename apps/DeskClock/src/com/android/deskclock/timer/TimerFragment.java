...
public class TimerFragment extends DeskClockFragment
    ...
    public void setLabel(TimerObj timer, String label) {
        ///M: If the timer was deleted by user, then can't set the label @{
        try {
            int timerId = mAdapter.findTimerPositionById(timer.mTimerId);
            mAdapter.getItem(timerId).mLabel = label;
        } catch (ArrayIndexOutOfBoundsException ex) {
            ex.printStackTrace();
            Log.e(TAG, "AlarmClock setLabel Can't find the timer");
            return;
        }
        /// @}
