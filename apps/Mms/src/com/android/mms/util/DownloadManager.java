...
public class DownloadManager {
            ...
            if (TelephonyIntents.ACTION_SERVICE_STATE_CHANGED.equals(intent.getAction())) {
                ...
                synchronized (sInstance) {
                    ...
                    if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                        ...
                        EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(mContext, intent.getIntExtra(EncapsulatedPhone.GEMINI_SIM_ID_KEY, -1));
                        if (null == si) {
    ...
    public void markState(final Uri uri, int state) {
        ...
        /// M: When canceled the download, shield the original toast. @{
        IMmsCancelDownload sCancelDownloadPlugin = (IMmsCancelDownload) MmsPluginManager
                .getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_CANCEL_DOWNLOAD);
        /// @}
        // Notify user if downloading permanently failed.
        if (state == STATE_PERMANENT_FAILURE && !sCancelDownloadPlugin.getCancelToastEnabled()) {
            mHandler.post(new Runnable() {
                public void run() {
                    try {
    ...
    private boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming, int simId) {
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            autoDownload = prefs.getBoolean(Integer.toString(simId) + "_" +
            MmsPreferenceActivity.AUTO_RETRIEVAL, true);
        } else {
      	    autoDownload = prefs.getBoolean(MmsPreferenceActivity.AUTO_RETRIEVAL, true);
        }
    ...
    private boolean isRoaming(int simId) {
        ...
        int slotId = EncapsulatedSimInfoManager.getSlotById(mContext, simId);
