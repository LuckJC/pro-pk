...
import static com.google.android.mms.pdu.PduHeaders.STATUS_UNREACHABLE;
import static com.google.android.mms.pdu.PduHeaders.RETRIEVE_STATUS_ERROR_PERMANENT_MESSAGE_NOT_FOUND;
...
public class NotificationTransaction extends Transaction implements Runnable {
    ...
    public void processNotification() {
        new Thread() {
            public void run() {
                notifyWithoutAutoDownload(DownloadManager.getInstance());
            }
        }.start();
    }
    ...
    public boolean allowAutoDownload() {
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            int datastate = teleManager.getDataState(EncapsulatedSimInfoManager.getSlotById(mContext, mSimId));
    ...
    private void notifyWithoutAutoDownload(DownloadManager downloadManager) {
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(mContext, 0);
            if (si == null) {
        ...
        */
        ...
        try {
            ...
            if (!autoDownload) {
                ...
                notifyWithoutAutoDownload(downloadManager);
            ...
            if (sCancelDownloadPlugin.isEnableCancelDownload() && mIsCancelling) {
                MmsLog.d(MmsApp.TXN_TAG, "***Canceling download in processing!(NotificationTransaction)");
                ...
                mIsCancelling = false;
                final Uri trxnUri = getNotTrxnUri();
            ...
            try {
                ...
                if (sCancelDownloadPlugin.isEnableCancelDownload()) {
                    if (sCancelDownloadPlugin.getStateExt(mUri) == IMmsCancelDownload.STATE_ABORTED) {
                        if (sMmsFailedNotifyPlugin.getFailedNotificationEnabled()) {
                            sMmsFailedNotifyPlugin.popupToast(IMmsFailedNotify.CANCEL_DOWNLOAD, null);
                        }
                        sCancelDownloadPlugin.markStateExt(mUri, IMmsCancelDownload.STATE_COMPLETE);
            ...
            if (retrieveConfData != null) {
                ...
                if ((pdu == null) || (pdu.getMessageType() != MESSAGE_TYPE_RETRIEVE_CONF)) {
                ...
                } else if(mmsTransactionPlugin.isEnableRetrieveStatusErrorCheck() &&
                       (((RetrieveConf)pdu).getRetrieveStatus() == RETRIEVE_STATUS_ERROR_PERMANENT_MESSAGE_NOT_FOUND)) {
                    /// M: check for the Retrieve status error code
                    Log.e(TAG, "Invalid M-RETRIEVE.CONF PDU. retrieve status 0xE2");
                    // Mark this transaction as failed since MMS is not present on Server.
                    mTransactionState.setState(FAILED);
                    ...
                    MmsLog.d(MmsApp.TXN_TAG, " NotificationTransaction. subject is: " + (subject == null ? "":subject.getString()));
            ...
            /// @}
            /// M: do not send the M-NotifyResp.ind to server for the error case 
    ...
    private void sendNotifyRespInd(int status) throws MmsException, IOException {
        ...
        if (MmsConfig.getDeliveryReportAllowed()) {
            ...
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                reportAllowed = prefs.getBoolean(MmsPreferenceActivity.MMS_ENABLE_TO_SEND_DELIVERY_REPORT, false);
            }
    ...
    public NotificationTransaction(
        ...
        try {
            mUri = PduPersister.getPduPersister(context).persist(
                        ind, Inbox.CONTENT_URI, true, false, null);
        } catch (MmsException e) {
            Log.e(TAG, "Failed to save NotificationInd in constructor.", e);
            throw new IllegalArgumentException();
        }
