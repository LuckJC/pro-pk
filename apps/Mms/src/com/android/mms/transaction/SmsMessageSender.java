...
import com.mediatek.mms.ext.MmsSmsMessageSenderImpl;
...
public class SmsMessageSender implements MessageSender {
    ...
    /// M: OP09 plug-in
    private IMmsSmsMessageSender mMmsSmsMessageSenderPlugin = null;
    ...
    private boolean queueMessage(long token) throws MmsException {
        ...
        for (int i = 0; i < mNumberOfDests; i++) {
            try {
                ...
                /// M: For OP09 feature, the mass text message merged into one message. @{
                Uri smsUri = null;
                if (mNumberOfDests > 1) {
                    smsUri = mMmsSmsMessageSenderPlugin.addMessageToUri(
                            mContext.getContentResolver(),
                            Uri.parse("content://sms/queued"), mDests[i],
                            mMessageText, null, mTimestamp,
                            true, requestDeliveryReport,
                            mThreadId, mSimId, -timeStamp);
                }
                /// @}
    ...
    /**
     * M: init plug in manager
     * @param context
     */
    private void initPlugin(Context context){
        try {
            mMmsSmsMessageSenderPlugin = (IMmsSmsMessageSender)EncapsulatedPluginManager.createPluginObject(context,
                IMmsSmsMessageSender.class.getName());
            MmsLog.d(TAG, "operator mMmsSmsMessageSenderPlugin = " + mMmsSmsMessageSenderPlugin);
        } catch (AndroidException e) {
            mMmsSmsMessageSenderPlugin = new MmsSmsMessageSenderImpl(context);
            MmsLog.d(TAG, "default mMmsSmsMessageSenderPlugin = " + mMmsSmsMessageSenderPlugin);
        }
    }
}
