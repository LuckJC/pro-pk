...
public class SimFullReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (!MmsConfig.isSmsEnabled(context)) {
            Log.d("SimFullReceiver", "receive SIM_FULL_ACTION, but sms is not default!");
            return;
        }
        if (EncapsulatedSettings.Global.getInt(context.getContentResolver(),
            ...
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                EncapsulatedSimInfoManager simInfo = EncapsulatedSimInfoManager.getSimInfoBySlot(context, slotId);
                if (simInfo != null) {
            ...
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT
                    && mStringReplacementPlugin.isEnableStringReplacement()
                    && ctString != null && MessageUtils.isUSimType(slotId)) {
                notification.tickerText = ctString;
            } else {
                notification.tickerText = context.getString(R.string.sim_full_title);
            } /// @}

            notification.defaults = Notification.DEFAULT_ALL;
            ///M: auto cancel the notification when clicked.
