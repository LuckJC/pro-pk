...
import com.mediatek.encapsulation.android.net.EncapsulatedConnectivityManager;
import com.mediatek.encapsulation.android.net.EncapsulatedNetworkInfo;
...
public class TransactionService extends Service implements Observer, IMmsCancelDownloadHost{
    ...
    /// M: New memeber for OP09 plug-in.
    private static IMmsCancelDownload sCancelDownloadPlugin;
    private static IMmsFailedNotify sMmsFailedNotifyPlugin;
    ...
    private int mBackUpSlotId;///M: add for ALPS00908472
    ...
    public void onCreate() {
        ...
        sMmsFailedNotifyPlugin = (IMmsFailedNotify) MmsPluginManager
...
//        }
    ...
    private void launchTransaction(int serviceId, TransactionBundle txnBundle, boolean noNetwork) {
        ...
        if (noNetwork && (txnBundle.getTransactionType() != Transaction.NOTIFICATION_TRANSACTION)
                && (txnBundle.getTransactionType() != Transaction.RETRIEVE_TRANSACTION)) {
    ...
    public void onDestroy() {
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            /// @}
            /// M: For OP09
            sCancelDownloadPlugin.setWaitingDataCnxn(false);
            ...
            if (EncapsulatedPhone.GEMINI_SIM_NUM >= 2) {
            ...
            if (EncapsulatedPhone.GEMINI_SIM_NUM >= 3) {
            ...
            if (EncapsulatedPhone.GEMINI_SIM_NUM >= 4) {
                ...
                case TransactionState.SUCCESS:
                    ...
                    switch (transaction.getType()) {
                        ...
                        case Transaction.RETRIEVE_TRANSACTION:
                            ...
                            */
                            /** @} */
                            /// M: Update the complete status_ext, for OP09 feature. @{
                            if (sCancelDownloadPlugin.isEnableCancelDownload()
                                    && transaction.getType() == Transaction.RETRIEVE_TRANSACTION) {
                                sCancelDownloadPlugin.markStateExt(((RetrieveTransaction)transaction).getRtrTrxnUri(),
                                        IMmsCancelDownload.STATE_COMPLETE);
                            }
                            /// @}
                            break;
    ...
    private final class ServiceHandler extends Handler {
            ...
            switch (msg.what) {
                ...
                case EVENT_CONTINUE_MMS_CONNECTIVITY:
                    ...
                    try {
                        ...
                        int result = 0;
                        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                            EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(getApplicationContext(), msg.arg2);
                            if (null == si) {
                ...
                case EVENT_DATA_STATE_CHANGED:
                    ...
                    if (mServiceHandler.hasMessages(EVENT_PENDING_TIME_OUT)) {
                        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                            ...
                            if (trxn == null) {
                                ...
                                int slotId = EncapsulatedSimInfoManager.getSlotById(getApplicationContext(), trxn.mSimId);
                                if (slotId == msg.arg2) {
                                    mServiceHandler.removeMessages(EVENT_PENDING_TIME_OUT);
                    ...
                    if (sCancelDownloadPlugin.isEnableCancelDownload() && sCancelDownloadPlugin.getWaitingDataCnxn()) {
                        synchronized (mProcessing) {
                            int cancelCount = 0;
                            for (Transaction t : mPending) {
                                if (t.mIsCancelling) {
                                    mPending.remove(t);
                                    MmsLog.d(MmsApp.TXN_TAG, "***Canceling download after EVENT_DATA_STATE_CHANGED!");
                                    cancelTransaction(t);
                                    cancelCount++;
                                }
                            }
                            MmsLog.d(MmsApp.TXN_TAG, "Canceled transaction: cancelCount = " + cancelCount);
                            if (mPending.size() == 0) {
                                MmsLog.d(MmsApp.TXN_TAG, "mPending.size() == 0, Cancel all!");
                                return;
                                }
                        }
                    }
                    ...
                    EncapsulatedNetworkInfo encapsulatedInfo = new EncapsulatedNetworkInfo(info, msg.arg2);
                    int slotOfInfo =  encapsulatedInfo.getSimId();
                    ...
                    if (!info.isConnected()) {
                        ...
                        if (ConnectivityManager.TYPE_MOBILE_MMS == info.getType() 
                            ...
                            /// M: add for OP09, pop-up toast when transaction failed. @{
                            if (sMmsFailedNotifyPlugin.getFailedNotificationEnabled()
                                    && sCancelDownloadPlugin.getWaitingDataCnxn()) {
                                sMmsFailedNotifyPlugin.popupToast(IMmsFailedNotify.DATA_OCCUPIED, null);
                            }
                            /// @}
                            if (0 != pendingSize){
                                MmsLog.d(MmsApp.TXN_TAG,
                                ...
                                ///M: ALPS00846707, add for cmcc, when in call, set transaction fail, but don't increase retryIndex @{
                                Transaction trxn = removePending(0);
                                if (MmsConfig.isRetainRetryIndexWhenInCall()) {
                                    if (trxn instanceof SendTransaction
                                            || trxn instanceof RetrieveTransaction) {
                                        boolean incall = isDuringCall();
                                        MmsLog.d(MmsApp.TXN_TAG, "incall? " + incall);
                                        if (incall) {
                                            setTransactionFail(trxn, FAILE_TYPE_RESTAIN_RETRY_INDEX);
                                        } else {
                                            setTransactionFail(trxn, FAILE_TYPE_TEMPORARY);
                                        }
                    ...
                    if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                        int simId = 0;
                        List<EncapsulatedSimInfoManager> mListSimInfo;
                        mListSimInfo = EncapsulatedSimInfoManager.getInsertedSimInfoList(getApplicationContext());
                    ...
                    if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                        ...
                        for (slotId = 0; slotId < simNumber; slotId++) {
                            if (mListSimInfo.get(slotId).getSlot() == slotOfInfo) {
                                EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(getApplicationContext(),
                                        slotOfInfo);
                                if (null == si) {
                    ...
                    try {
                        ...
                        if (mmsc != null) {
                            ...
                            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                                ...
                                int slotId = EncapsulatedSimInfoManager.getSlotById(getApplicationContext(), msg.arg2);
                                transactionSettings = new TransactionSettings(
                        ...
                        switch (transactionType) {
                            ...
                            case Transaction.RETRIEVE_TRANSACTION:
                                ...
                                /// @}
                                /// M: Update the downloading status_ext, for OP09 feature. @{
                                if (sCancelDownloadPlugin.isEnableCancelDownload()) {
                                    sCancelDownloadPlugin.markStateExt(((RetrieveTransaction)transaction).getRtrTrxnUri(),
                                            IMmsCancelDownload.STATE_DOWNLOADING);
                                }
                                /// @}
                                break;
                            case Transaction.SEND_TRANSACTION:
                ...
                case EVENT_PENDING_TIME_OUT:
                    ...
                    if (0 != pendSize){
                      ...
                      /// M:Code analyze 010,add for new feature,this member is used to ignore status message sent by framework
                        ...
                        if (sMmsFailedNotifyPlugin.getFailedNotificationEnabled()) {
                            sMmsFailedNotifyPlugin.popupToast(IMmsFailedNotify.CONNECTION_FAILED, null);
                        } /// @}
                    }
                    return;
                    /// @}
                ...
                case EVENT_SCAN_PENDING_MMS:
                    ...
                    if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                        for (int slotId = 0; slotId < EncapsulatedPhone.GEMINI_SIM_NUM; slotId++) {
                            ...
                            EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(getApplicationContext(), slotId);
                            if (null != si) {
        ...
        private boolean processTransaction(Transaction transaction) throws IOException {
            ...
            synchronized (mProcessing) {
                ...
                if (mmsTransactionPlugin == null || !mmsTransactionPlugin.isGminiMultiTransactionEnabled()) {
                    ...
                    /* It is a first transaction, start directly*/
                    if (mProcessing.size() == 0 && !mIsWaitingConxn) {
                        MmsLog.d(MmsApp.TXN_TAG, "processing size=" + mProcessing.size()
                            + ", is waiting conxn=" + mIsWaitingConxn);
                    } else if (mIsWaitingConxn) { /* Waiting PDP success, add it to pending*/
                        MmsLog.d(MmsApp.TXN_TAG, "Wating PDP and add to pending");
                        mPending.add(transaction);
                        return true;
                    } else if (mProcessing.size() > 0) { /* PDP ok, and have processing, need check simId*/
                        MmsLog.d(MmsApp.TXN_TAG, "processing size=" + mProcessing.size());

                        Transaction transactionInProcess = mProcessing.get(0);
                        MmsLog.d(MmsApp.TXN_TAG, "pending(0).simId=" + transactionInProcess.mSimId +
                            ", this simId=" + transaction.mSimId);

                        /* This is another sim's mms, add to pending for handling later*/
                        if (transaction.mSimId != transactionInProcess.mSimId) {
                            mPending.add(transaction);
                            return true;
                        }
                    }
                ...
                */
                ...
                /// M: For OP09, check if cancel download requested. @{
                if (sCancelDownloadPlugin.isEnableCancelDownload() && transaction.mIsCancelling) {
                ...
                if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                    boolean isConnectivityVariable = false;
                    try {
                        isConnectivityVariable = EncapsulatedTelephonyService.getInstance().
                                    isDataConnectivityPossibleGemini(EncapsulatedSimInfoManager.
                                            getSlotById(TransactionService.this, transaction.mSimId));
                    } catch (android.os.RemoteException ex) {
                        MmsLog.e(MmsApp.TXN_TAG, "isDataConnectivityPossibleGemini has exception");
                        isConnectivityVariable = false;
                    /// M: add for ALPS00971549, NullPointer exception when sim card are pulled out@{
                    } catch (NullPointerException e) {
                        MmsLog.e(MmsApp.TXN_TAG, "isDataConnectivityPossibleGemini has NullPointer exception");
                        isConnectivityVariable = false;
                    }
                    /// @}
                    MmsLog.d(MmsApp.TXN_TAG, "isConnectivityVariable=" + isConnectivityVariable);
                    if (!isConnectivityVariable && isDuringCall()
                            &&(transaction instanceof ReadRecTransaction
                            || transaction instanceof SendTransaction
                            || transaction instanceof RetrieveTransaction)) {
                        connectivityResult = EncapsulatedPhone.APN_TYPE_NOT_AVAILABLE;
                    } else {
                ...
                if (connectivityResult == EncapsulatedPhone.APN_REQUEST_STARTED) {
                    ...
                    /// @}
                    /// M: For OP09
                    sCancelDownloadPlugin.setWaitingDataCnxn(true);
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                ...
                /// so set current transaction failed also,add for gemini and open @{ */
                    ...
                    /// M: add for OP09 feature @{
                    if (connectivityResult == EncapsulatedPhone.APN_REQUEST_FAILED) {
                        if (sMmsFailedNotifyPlugin.getFailedNotificationEnabled()) {
                            sMmsFailedNotifyPlugin.popupToast(IMmsFailedNotify.CONNECTION_FAILED, null);
                        }
                    } /// @}
            ...
            if (requestResult == EncapsulatedPhone.APN_ALREADY_ACTIVE) {
                ...
                if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                    int slotId = EncapsulatedSimInfoManager.getSlotById(getApplicationContext(), transaction.mSimId);
  ...
  ///M: modify for cmcc, when in call, set transaction fail, but don't increase retryIndex
    ...
    private void setTransactionFail(Transaction txn, int failType) {
        ...
        if (txn instanceof NotificationTransaction) {
            ...
            if (!autoDownload) {
                ((NotificationTransaction) txn).processNotification();
        ...
        if (cursor != null) {
            try {
                if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                    ...
                    /// M: ALPS01262315, Do not mark temporary send fail message as unread @{
...
//                                    uri, readValues, null, null);
    ...
    protected int beginMmsConnectivityGemini(int simId) throws IOException {
        ...
        /// @}
        // convert sim id to slot id
        int slotId = EncapsulatedSimInfoManager.getSlotById(getApplicationContext(), simId);

        ///M: add for ALPS00908472,plug out sim card when sending mms,then slotId is set to -1,
        /// ,then stopUsingNetworkFeatureGemini cannot be run successfully, so slotId should be storaged firstly @{
        mBackUpSlotId = slotId;
        MmsLog.d(MmsApp.TXN_TAG, "beginMmsConnectivityGemini: mBackUpSlotId=" + mBackUpSlotId);
        /// @}

        int result = new EncapsulatedConnectivityManager(mConnMgr).startUsingNetworkFeatureGemini(
    ...
    protected void endMmsConnectivityGemini(int simId) {
        try {
            ...
            ///M: add for ALPS00908472 @{
            if (-1 == slotId) {
                slotId = mBackUpSlotId;
                MmsLog.d(MmsApp.TXN_TAG, "endMmsConnectivityGemini:backup slot Id= " + slotId);
            }
            /// @
            ...
            mServiceHandler.removeMessages(EVENT_CONTINUE_MMS_CONNECTIVITY);
            if (mConnMgr != null) {
                new EncapsulatedConnectivityManager(mConnMgr).stopUsingNetworkFeatureGemini(
                                ConnectivityManager.TYPE_MOBILE,
    ...
    private Transaction removePending(int index) {
        ...
        synchronized (mProcessing) {
            if (mPending.size() > 0) {
                ...
                MmsLog.e(MmsApp.TXN_TAG, "removePending error, mPending size is already 0");
            }
        }
        return trxn;
    }
    ...
    private void cancelTransaction(Transaction transaction) {
        ...
        if (sMmsFailedNotifyPlugin != null) {
            sMmsFailedNotifyPlugin.popupToast(IMmsFailedNotify.CANCEL_DOWNLOAD, null);
        }
        ...
        if (transaction.getType() == Transaction.RETRIEVE_TRANSACTION) {
            trxnUri = ((RetrieveTransaction)transaction).getRtrTrxnUri();
        } else if (transaction.getType() == Transaction.NOTIFICATION_TRANSACTION) {
            trxnUri = ((NotificationTransaction)transaction).getNotTrxnUri();
        }
    ...
    /// M: Called by OP09 Plug-in.
    /// Set a label, according to which, we can know whether user need to cancel the transaction. @{
    public void setCancelDownloadState(Uri uri, boolean isCancelling) {
        ...
        Uri inboxUri = Uri.parse("content://" + uri.getAuthority() + "/inbox/" + uri.getLastPathSegment());
        Uri uriInList = null;
            ...
            synchronized (mProcessing) {
                for (Transaction t : mPending) {
                    ...
                    if (t.getType() == Transaction.RETRIEVE_TRANSACTION) {
                        uriInList = ((RetrieveTransaction)t).getRtrTrxnUri();
                        MmsLog.d(MmsApp.TXN_TAG, "    uriInList == " + uriInList);
                    } else if (t.getType() == Transaction.NOTIFICATION_TRANSACTION) {
                        uriInList = ((NotificationTransaction)t).getNotTrxnUri();
                        MmsLog.d(MmsApp.TXN_TAG, "    uriInList == " + uriInList);
                    }

                    /// M: Compare inboxUri above is for DialogModeActivity case.
                    if (uriInList != null && (uriInList.equals(uri) || uriInList.equals(inboxUri))) {
                        /// M: If any other transaction is in processing, just move this from mPending.
