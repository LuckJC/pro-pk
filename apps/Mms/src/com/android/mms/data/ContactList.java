        ...
        for (RecipientIdCache.Entry entry : RecipientIdCache.getAddresses(spaceSepIds)) {
            if (entry != null && !TextUtils.isEmpty(entry.number)) {
                ...
                if (MmsConfig.getIpMessagServiceId(MmsApp.getApplication()) == IpMessageServiceId.ISMS_SERVICE) {
                    if (IpMessageUtils.getContactManager(
                            MmsApp.getApplication().getApplicationContext()).isIpMessageSrvNumber(
                            entry.number)) {
                        contact.setName(IpMessageUtils.getContactManager(
                                MmsApp.getApplication().getApplicationContext())
                                .getIpMessageSrvNumberName(entry.number));
                    } else {
                        MmsLog.d(TAG, "isIpMessageSrvNumber false : number " + entry.number);
                    }
                }
                /// @}
