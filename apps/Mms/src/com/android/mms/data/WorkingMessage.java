    ...
    private class DraftInterface implements IDraftInterface {
        ...
        public void updateAfterSaveDraftFinished(final Uri msgUri, final int create, final boolean result) {
            ...
            try {
                ...
                if (mIsDeleteVcardFile && create == 1) {
                    Log.d(TAG_DRAFT, "[updateAfterSaveDraftFinished] remove file attachment temp files");
...
//            SlideshowEditor slideShowEditor = new SlideshowEditor(mActivity, mSlideshow);
    ...
    private void changeMedia(int type, Uri uri) throws MmsException {
        ...
        if (mMmsAttachmentEnhancePlugin!=null &&
            ...
            //OP01
            //In this case, we should justify weather media.getMediaPackagedSize() + SlideshowModel.mReserveSize + attachSize >
        ...
        /// M: fix bug ALPS00794745, throw when media.size > max size
        if (mMmsAttachmentEnhancePlugin!=null &&
            mMmsAttachmentEnhancePlugin.isSupportAttachmentEnhance() == true ) {
            //OP01
            if (media.getMediaPackagedSize() + SlideshowModel.mReserveSize + attachSize
    ...
    private void syncTextToSlideshow() {
        ...
        // Add a TextModel to slide 0 if one doesn't already exist
        text = new TextModel(mActivity, EncapsulatedContentType.TEXT_PLAIN, "text_0.txt",
        ...
        try {
            ...
            if (slide != null) {
                ...
                if (newLength - oldLength > 0) {
                    int attachmentSize = 0;
                    IMmsAttachmentEnhance mmsAttachmentEnhancePlugin =
                            (IMmsAttachmentEnhance) MmsPluginManager.getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_MMS_ATTACHMENT_ENHANCE);
...
//                if (mMessageUri == null) {
                    ...
                    if (mdd.getCreateOrUpdate() == 1) {
                        if (mIsDeleteVcardFile) {
                            Log.d(TAG_DRAFT, "[saveAsMms] remove file attachment temp files");
                            deleteFileAttachmentTempFile();
                        }
...
//            loadFromUri(uri);
    ...
    private class SplitToMmsAndSmsConversation {
        ...
        public SplitToMmsAndSmsConversation(final Conversation conv, final String msgTxt) {
            ...
            IMmsSmsMmsConversationSplit mMmsSmsMmsConversationSplitPlugin = (IMmsSmsMmsConversationSplit) MmsPluginManager
                    .getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_SMS_MMS_CONVERSATION_SPLIT);

            if (!mMmsSmsMmsConversationSplitPlugin.isConversationSplitSupported()) {
    ...
    /// M: Code analyze 047, For new feature ALPS00316567, add a parameter for gemini . @{
    private void preSendSmsWorker(Conversation conv, String msgText, String recipientsInUI, int simId, boolean hasBeenSplit) {
    /// @}
    ...
    /// @}
        ...
        try {
            ...
            else if (MmsConfig.isShowDraftIcon()) {
    ...
    private void markMmsMessageWithError(Uri mmsUri, int simId) {
        try {
            ...
            valuePendingTable.put(EncapsulatedTelephony.MmsSms.PendingMessages.SIM_ID, simId);
