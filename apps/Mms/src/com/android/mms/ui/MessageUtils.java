...
import com.mediatek.encapsulation.android.drm.EncapsulatedOmaDrmUtils;
...
import com.mediatek.encapsulation.com.google.android.mms.pdu.EncapsulatedMultimediaMessagePdu;
...
import com.mediatek.mms.ext.MmsPreferenceImpl;
import com.mediatek.mms.ext.StorageLowImpl;
...
public class MessageUtils {
    ...
    /// M: for OP09 @{
    private static IStorageLow sStorageLowPlugin;
    private static IMmsPreference sMmsPreference;
    /// @}
    ...
    private static String getMultimediaMessageDetails(Context context, MessageItem msgItem) {
        ...
        IMmsUtils mmsUtils = (IMmsUtils) MmsPluginManager.getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_MESSAGE_UTILS);
        try {
        ...
        // SentDate: ***
        if (new EncapsulatedMultimediaMessagePdu(msg).getDateSent() > 0
            ...
            if (MmsConfig.isSupportCTFeature()) {
                dateStr = mmsUtils.formatDateAndTimeStampString(context, 0,
                        new EncapsulatedMultimediaMessagePdu(msg).getDateSent() * 1000L, true, dateStr);
            /// @}
            } else {
                dateStr = MessageUtils.formatTimeStampString(context,
                        new EncapsulatedMultimediaMessagePdu(msg).getDateSent() * 1000L, true);
            }
    ...
    public static String formatTimeStampString(Context context, long when, boolean fullFormat) {
        ...
        /// M: For OP09 @{
        if (MmsConfig.isSupportCTFeature()) {
            IMmsUtils mmsUtils = (IMmsUtils) MmsPluginManager
    ...
    public static void selectAudio(Activity activity, int requestCode) {
        ...
        String[] mimeTypess = new String[] {EncapsulatedContentType.AUDIO_UNSPECIFIED,
                EncapsulatedContentType.AUDIO_OGG, "application/x-ogg"};
...
//        sizeLimit *= .85F;
    ...
    private static void selectMediaByType(
         ...
         if (context instanceof Activity) {
            ...
            if (EncapsulatedFeatureOption.MTK_S3D_SUPPORT) {
                innerIntent.putExtra("get_no_stereo_image", true);
            }
...
//                    launchSlideshowActivity(activity, msgUri, requestCode);
    ...
    public static String getLocalNumberGemini(int simId) {
        ...
        int slotId = EncapsulatedSimInfoManager.getSlotById(MmsApp.getApplication().getApplicationContext(), simId);
    ...
    public static CharSequence getSimInfoSync(Context context, int simId) {
        EncapsulatedSimInfoManager simInfo = EncapsulatedSimInfoManager.getSIMInfoById(context, simId);
        if (null != simInfo) {
    ...
    public static boolean checkUriContainsDrm(Context context, Uri uri) {
        return EncapsulatedOmaDrmUtils.isDrmContent(context, uri);
    }
    ...
    public static int getSimStatus(int id, List<EncapsulatedSimInfoManager> simInfoList, EncapsulatedTelephonyManagerEx telephonyManager) {
    ...
    public static boolean is3G(int id, List<EncapsulatedSimInfoManager> simInfoList) {
        int slotId = simInfoList.get(id).getSlot();
    ...
    public static void replyMessage(Context context, String address, int simId) {
        ...
        /// M: fix alps01007218. this put slot id here. @{
        int slot_id = EncapsulatedSimInfoManager.getSlotById(context, simId);
        intent.putExtra(EncapsulatedPhone.GEMINI_SIM_ID_KEY, slot_id);
        /// @}
...
//        final long totalBlocks = stat.getBlockCount();
    ...
    public static boolean checkNeedNotify(Context context, long threadId, Cursor cursor) {
        ...
        if (cursor != null) {
            ...
            Cursor c = context.getContentResolver().query(threadSettingsUri,
                    new String[] {EncapsulatedTelephony.ThreadSettings.NOTIFICATION_ENABLE,
                                  EncapsulatedTelephony.ThreadSettings.MUTE,
                                  EncapsulatedTelephony.ThreadSettings.MUTE_START,
                                  EncapsulatedTelephony.ThreadSettings.RINGTONE,
                                  EncapsulatedTelephony.ThreadSettings.VIBRATE},
                    null, null, null);
            ...
            if ((threadMute * 3600 + threadMuteStart / 1000) <= currentTime) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        contentValues.put(EncapsulatedTelephony.ThreadSettings.MUTE, 0);
                        contentValues.put(EncapsulatedTelephony.ThreadSettings.MUTE_START, 0);
    ...
    public static boolean isUSimType(int slot) {
        ...
        try {
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                return iTel.getIccCardType(slot).equals(sUim);
            } else {
                return iTel.getIccCardType(0).equals(sUim);
            }
        } catch (Exception e) {
    ...
    public static CharSequence getVisualTextName(Context context,
        ...
        IStringReplacement mStringReplacementPlugin = (IStringReplacement) MmsPluginManager.getMmsPluginObject(
        ...
        if (mStringReplacementPlugin.isEnableStringReplacement()) {
            if (EncapsulatedSimInfoManager.getInsertedSimInfoList(context).size() == 1
                    && isUSimType(EncapsulatedSimInfoManager.getInsertedSimInfoList(context).get(0).getSlot())
    ...
    public static String getMainCardDisplayName() {
        ...
        if (mainSimId != EncapsulatedSettings.System.DEFAULT_SIM_SETTING_ALWAYS_ASK
                && mainSimId != EncapsulatedSettings.System.DEFAULT_SIM_NOT_SET) {
            int slotId = EncapsulatedSimInfoManager.getSlotById(ct, mainSimId);
            EncapsulatedSimInfoManager info = EncapsulatedSimInfoManager.getSimInfoBySlot(ct, slotId);
            mainSimDisplayName = info.getDisplayName();
        } else {
            EncapsulatedSimInfoManager info = EncapsulatedSimInfoManager.getSimInfoBySlot(ct, 0);
            if (info != null && info.getSimId() > 0) {
                mainSimDisplayName = info.getDisplayName();
            } else {
                info = EncapsulatedSimInfoManager.getSimInfoBySlot(ct, 1);
                if (info != null && info.getSimId() > 0) {
                    mainSimDisplayName = info.getDisplayName();
                } else {
  ...
  //So we should deal with this situation.
    ...
    */
    ...
    private static void getStorageLowPlugin(Context context) {
        ...
        try {
            sStorageLowPlugin = (IStorageLow)EncapsulatedPluginManager.createPluginObject(
                    context.getApplicationContext(), IStorageLow.class.getName());
            MmsLog.d(MmsApp.TXN_TAG, "operator sStorageLowPlugin = " + sStorageLowPlugin);
        } catch (AndroidException e) {
            sStorageLowPlugin = new StorageLowImpl(context.getApplicationContext());
    ...
    private static void getMmsPreferencePlugin(Context context){
        ...
        try{
            sMmsPreference = (IMmsPreference)EncapsulatedPluginManager.createPluginObject(
                    context.getApplicationContext(), IMmsPreference.class.getName());
            MmsLog.d(MmsApp.TXN_TAG, "operator sMmsPreference = " + sMmsPreference);
        } catch (AndroidException e) {
            sMmsPreference = new MmsPreferenceImpl(context.getApplicationContext());
