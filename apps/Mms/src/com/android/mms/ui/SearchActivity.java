...
import com.mediatek.mms.ext.MmsAdvanceSearchImpl;
...
public class SearchActivity extends ListActivity {
    ...
    private IMmsAdvanceSearch mMmsAdvanceSearchPlugin;
    /// @}
    ...
    private void startQuery() {
        ...
        /// M: Add For OP09; @{
        if (mIsAdvancedQuery) {
            uri = mMmsAdvanceSearchPlugin.buildAdvanceSearchUri();
        }
        /// @}
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (MmsConfig.isSupportCTFeature()) {
            mIsAdvancedQuery = getIntent().getBooleanExtra(IMmsAdvanceSearch.ADVANCED_SEARCH_QUERY, false);
        }
        /// @}
        ...
        mQueryHandler = new AsyncQueryHandler(cr) {
            protected void onQueryComplete(int token, Object cookie, Cursor c) {
                ...
                if (c == null) {
                    ...
                    if (mIsAdvancedQuery) {
                        setTitle(getResources()
                            .getQuantityString(R.plurals.advance_search_results_title,
                                0,
                                0));
                        return;
                    }
                ...
                if(mIsAdvancedQuery){
                    setTitle(getResources().getQuantityString(
                        R.plurals.advance_search_results_title,
                        cursorCount,
                        cursorCount));
                } else {
                /// @}
     ...
     * @param intent
     */
    private void initPlugin(Context context, Intent intent) {
        try {
            mMmsAdvanceSearchPlugin = (IMmsAdvanceSearch)EncapsulatedPluginManager.createPluginObject(context, IMmsAdvanceSearch.class.getName());
            MmsLog.d(TAG, "operator mMmsAdvanceSearchPlugin = " + mMmsAdvanceSearchPlugin);
        } catch (AndroidException e) {
            mMmsAdvanceSearchPlugin = new MmsAdvanceSearchImpl(context);
            MmsLog.d(TAG, "default mMmsAdvanceSearchPlugin = " + mMmsAdvanceSearchPlugin);
        }
        mMmsAdvanceSearchPlugin.initAdvancedData(intent);
    }
            ...
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if(EncapsulatedFeatureOption.MTK_WAPPUSH_SUPPORT == true){
                        ...
                        if (threadType == EncapsulatedTelephony.Threads.IP_MESSAGE_GUIDE_THREAD) {
                    ...
                    if (msgBox == 1 && m_type == 130) {
                        ...
                        if (!MmsConfig.getMmsDirMode() && msgBox == 3 && sWaitSaveDraft) {
                            Toast.makeText(SearchActivity.this,
                                    R.string.cannot_load_message, Toast.LENGTH_SHORT).show();
                            return;
                        }
                        onClickIntent.putExtra("thread_id", threadId);
                        onClickIntent.putExtra("highlight", searchString);
