...
import com.mediatek.mms.ext.IMmsDialogModeHost;
import com.mediatek.mms.ext.IMmsUtils;
import com.mediatek.mms.ext.MmsDialogModeImpl;
...
public class DialogModeActivity extends Activity implements
    ...
    private IMmsDialogMode  mMmsDialogModePlugin;
    private IMmsUtils mMmsUtilsPlugin;
    private IMmsAskIfDownload mAskIfDownloadPlugin;
    /// @}
    ...
    public DialogModeActivity() {
        ...
        mAskIfDownloadPlugin = (IMmsAskIfDownload) MmsPluginManager
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState != null){
            if(!savedInstanceState.isEmpty()){
                ...
                MmsLog.d(TAG, "savedInstanceState mUris"+mUris);
                MmsLog.d(TAG, "savedInstanceState mCurUriIdx"+mCurUriIdx);
    ...
    protected void onDestroy() {
        ...
        try {
            ...
            MmsLog.e(TAG, "Error occured when unregisterReceiver: mSimReceiver;");
        }
        /// @}
        if (mCellMgr != null) {
            mCellMgr.unregister();
    ...
    public void onNewIntent(Intent intent) {
        ...
        if (newString.equals(mCurUri.toString())) {
            MmsLog.d(TAG, "DialogModeActivity.new Uri = current Uri , don't show ");
            return;
        }
        addNewUri(intent);
        loadCurMsg();
    ...
    private void setDialogView() {
        ...
        /// M: For OP09
        mMmsDialogModePlugin.setSimTypeDrawable(this, getCurrentSimId(), mSimName);
    ...
    private String getSIMName() {
        ...
        if (mCursor.moveToFirst()) {
            ...
            EncapsulatedSimInfoManager simInfo = EncapsulatedSimInfoManager.getSIMInfoById(getApplicationContext(), simId);
    ...
    public void onClick(View v) {
        ...
        if (v.equals(mSmsContentText)) {
            ...
            intent.putExtra(EncapsulatedPhone.GEMINI_SIM_ID_KEY, (int)getSimId());
            startService(intent);
    ...
    private void simSelection() {
        ...
        MmsLog.d(TAG, "mSelectedSimId = "+mSelectedSimId);
        if (!EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            confirmSendMessageIfNeeded();
        /// M: add for OP09. @{
        } else if (send_sim_id >= 1 && mMmsDialogModePlugin.isDualSendButtonEnable()) {
        ...
        /// @}
            ...
            } else if (messageSimId == EncapsulatedSettings.System.SMS_SIM_SETTING_AUTO && MmsConfig.isSupportAutoSelectSimId()) {
    ...
    private int getContactSIM(String number) {
        ...
        Cursor associateSIMCursor = getApplicationContext().getContentResolver().query(
                Data.CONTENT_URI,
                new String[] {
                    EncapsulatedContactsContract.Data.SIM_ASSOCIATION_ID
                },
                Data.MIMETYPE + "='" + CommonDataKinds.Phone.CONTENT_ITEM_TYPE + "' AND ("
                        + Data.DATA1 + "=?" + " OR " + Data.DATA1 + "=?" + " OR " + Data.DATA4
                        + "=?" + ") AND (" + EncapsulatedContactsContract.Data.SIM_ASSOCIATION_ID
                        + "!= -1)", new String[] {
    ...
    private void showSimSelectedDialog(Intent intent) {
        ...
        final List<EncapsulatedSimInfoManager> simListInfobySlot = new ArrayList<EncapsulatedSimInfoManager>();
        for(int slotId = 0; slotId < EncapsulatedPhone.GEMINI_SIM_NUM; slotId++){
            for (int i = 0; i < mSimInfoList.size(); i++) {
                EncapsulatedSimInfoManager simInfo = mSimInfoList.get(i);
                if (simInfo == null || slotId != simInfo.getSlot()) {
                    continue;
                }
                simListInfobySlot.add(simInfo);
                ...
                if (!TextUtils.isEmpty(simInfo.getNumber())) {
                    MmsLog.d(TAG, "format = " + simInfo.getDispalyNumberFormat());
                ...
                if (TextUtils.isEmpty(simNumber)) {
                    ...
                    MmsLog.d(TAG, "simNumber = " + simNumber);
                    entry.put("simNumberShort", simNumber);
                }
    
                entry.put("simName", simInfo.getDisplayName());
                MmsLog.d(TAG, "simName = " + simInfo.getDisplayName());
                if (TextUtils.isEmpty(simInfo.getNumber())) {
                    entry.put("simNumber", "");
                } else {
                    MmsLog.d(TAG, "simNumber = " + simNumber);
        ...
        b.setAdapter(a, new DialogInterface.OnClickListener() {
            ...
            public final void onClick(DialogInterface dialog, int which) {
                updateSendButtonState();
                //M:fix bug:ALPS00847106
                mSelectedSimId = (int) simListInfobySlot.get(which).getSimId();
            ...
            mCellMgr.handleCellConn(slotId, requestType, new Runnable() {
                public void run() {
                    ...
                    if (mCellMgr != null) {
                        ...
                        MmsLog.d(TAG, "serviceComplete result = "
                                + EncapsulatedCellConnMgr.resultToString(nRet));
                        ...
                        if (slotId != mCellMgr.getPreferSlot()) {
                            MmsLog.d(TAG, "111");
                            EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager
                                    .getSimInfoBySlot(DialogModeActivity.this, mCellMgr
                                            .getPreferSlot());
                            if (si == null) {
        ...
        try {
            ...
            if ((subject != null) && (subject.length() > 0)) {
                ...
                if (IpMessageUtils.getIpMessagePlugin(this).isActualPlugin()
                        && IpMessageUtils.getServiceManager(this).isFeatureSupported(
                                IpMessageConsts.FeatureId.PARSE_EMO_WITHOUT_ACTIVATE)) {
                    smilizedSubject = mParser.addSmileySpans(subject);
                } else {
                    smilizedSubject = subject;
                }
...
//        SmileyParser parser = SmileyParser.getInstance();
    ...
    private void initPlugin(Context context) {
        try {
            mMmsDialogModePlugin = (IMmsDialogMode) EncapsulatedPluginManager.createPluginObject(
                context, IMmsDialogMode.class.getName());
            MmsLog.d(TAG, "operator mMmsDialogModePlugin = " + mMmsDialogModePlugin);
        } catch (AndroidException e) {
            mMmsDialogModePlugin = new MmsDialogModeImpl(context);
            MmsLog.d(TAG, "default mMmsDialogModePlugin = " + mMmsDialogModePlugin);
        }
        mMmsDialogModePlugin.init(this);
    ...
    public boolean initCTSendButton() {
        ...
        if (mButtonSmall == null) {
            mButtonSmall = (ImageButton) this.findViewById(R.id.ct_send_button_dlg_small);
        }
        if (mButtonBig == null) {
            mButtonBig = (ImageButton) this.findViewById(R.id.ct_send_button_dlg_big);
        }
        ...
        mTextCounter = (TextView) findViewById(R.id.ct_text_dlg_counter);
        mTextCounter.setVisibility(View.VISIBLE);

        mMmsDialogModePlugin.setCTSendButtonType();
        hideCtSendPanel();
        return true;
    }
    ...
    public boolean updateCTSendButtonStatue(boolean enable) {
        if (mSendButton != null) {
            mSendButton.setVisibility(View.GONE);
        }
        if (mButtonBig == null) {
        ...
        // 1. If there is no sim card，disable two buttons.
        // 2. If there is only one sim card, Big button will be enabled, and disable small button
        ...
        boolean smallBtnEnable = false;
        if (mButtonSmall != null) {
            smallBtnEnable = mButtonSmall.isEnabled();
        }
    ...
    public boolean setCTSendButtonType() {
        ...
        resIds = mMmsUtilsPlugin.getSendButtonResourceIdBySlotId(this.getApplicationContext(), slotId, false);
        if (resIds != null) {
            mButtonDrawable[0] = (Drawable) resIds[0][0];
            mButtonDrawable[1] = (Drawable) resIds[1][0];
        }
        if (mButtonBig.isEnabled()) {
            resIds = mMmsUtilsPlugin.getSendButtonResourceIdBySlotId(this.getApplicationContext(), slotId, true);
        }
//        resIds = mmsUtilsPlugin.getSendButtonResourceIdBySlotId(this.getApplicationContext(), slotId);
    ...
    View.OnClickListener mCtButtonClickListener = new View.OnClickListener() {
        public void onClick(View v) {
            if (v == mButtonBig) {
                EncapsulatedSimInfoManager sm = EncapsulatedSimInfoManager.getSimInfoBySlot(DialogModeActivity.this,
                    mButtonSlotIds[0]);
