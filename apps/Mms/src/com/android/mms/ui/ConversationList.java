...
public class ConversationList extends ListActivity implements DraftCache.OnDraftChangedListener,
    ...
    public static final int OPTION_CONVERSATION_LIST_JOYN        = 4;
    public static final int OPTION_CONVERSATION_LIST_XMS         = 5;
    ...
    private static AlertDialog mDeleteAlertDialog;
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        /// M: fix bug ALPS00374917, cancel sim_sms menu when haven't sim card
        if (mSimSmsItem != null) {
            if (!EncapsulatedSimInfoManager.isSimMessageAccessable(this)) {
                mSimSmsItem.setVisible(false);
            } else {
                mSimSmsItem.setVisible(true);
            }
        }
        ...
        if (MmsConfig.getIpMessagServiceId(this) > IpMessageServiceId.NO_SERVICE) {
            ...
            if (conv.getRecipients().size() == 1 && number.startsWith(IpMessageConsts.GROUP_START)) {
                ...
                if (sConversationListOption == OPTION_CONVERSATION_LIST_IMPORTANT) {
                    openIpMsgThread(threadId, true);
                } else {
                    openIpMsgThread(threadId, false);
                }
                return;
            }
            /// M: add for ipmessage, handle important thread
            if (sConversationListOption == OPTION_CONVERSATION_LIST_IMPORTANT) {
                MmsLog.i(IPMSG_TAG, "open important thread by thread id " + threadId);
                if (conv.getType() != EncapsulatedTelephony.Threads.WAPPUSH_THREAD
                         && conv.getType() != EncapsulatedTelephony.Threads.CELL_BROADCAST_THREAD) {
...
//        protected void myonQueryComplete(int token, Object cookie, Cursor cursor) {
                ...
                if (MmsConfig.isActivated(ConversationList.this)) {
                    switch (sConversationListOption) {
                        case OPTION_CONVERSATION_LIST_ALL:
                            ...
                            break;
                        case OPTION_CONVERSATION_LIST_IMPORTANT:
                            selection = "threads._id IN (SELECT " + Sms.THREAD_ID + " FROM sms WHERE "
                                    + Sms.LOCKED + "=1" + " UNION SELECT " + Mms.THREAD_ID
                                    + " FROM pdu WHERE " + Mms.LOCKED + "=1)";
                            MmsLog.d(TAG,
                                    "setAllItemChecked(): query for important messages, selection = "
                                            + selection);
                            cursor = getContext().getContentResolver().query(
                                    Conversation.sAllThreadsUriExtend,
                                    Conversation.ALL_THREADS_PROJECTION_EXTEND, selection, null,
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (MmsConfig.getIpMessagServiceId(mContext) == IpMessageServiceId.ISMS_SERVICE) {
                    IpMessageUtils.getServiceManager(mContext).checkDefaultSmsAppChanged();
                }
                /// @}
    ...
    private void initPlugin(Context context) {
        mMmsConversationPlugin = (IMmsConversation) MmsPluginManager
    ...
    public void showSimSms() {
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            List<EncapsulatedSimInfoManager> listSimInfo = EncapsulatedSimInfoManager.getInsertedSimInfoList(this);
            if (listSimInfo.size() > 1) {
                ...
                simSmsIntent.putExtra("preference", SettingListActivity.SMS_MANAGE_SIM_MESSAGES);
