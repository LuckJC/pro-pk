...
public class AdvancedEditorPreference extends Preference {
    ...
    protected void onBindView(View view) {
        ...
        if (mSimInfo.getNumberFormat(mCurrentId) == EncapsulatedSimInfoManager.DISPLAY_NUMBER_FIRST) {
        ...
        } else if (mSimInfo.getNumberFormat(mCurrentId) == EncapsulatedSimInfoManager.DISPLAY_NUMBER_LAST) {
            if (numShow != null && numShow.length() > SHOW_NUMBER_LENGTH) {
        ...
        if (-1 != simStatusResourceId) {
            sSimStatus.setVisibility(View.VISIBLE);
            sSimStatus.setImageResource(simStatusResourceId);
        } else {
            sSimStatus.setVisibility(View.GONE);
        }
    ...
    private CharSequence getVisualTextName(String enumName, int choiceNameResId,
        ...
        /// M: For OP09feature: Replace string "SIM" with "UIM". @{
        CharSequence[] visualNames = null;
        String[] location = mStringReplacementPlugin.getSaveLocationString();
        if (mStringReplacementPlugin.isEnableStringReplacement() && location != null && MessageUtils.isUSimType(mCurrentId)) {
            visualNames = location;
        } else {
            visualNames = getContext().getResources().getTextArray(choiceNameResId);
        }
        /// @}
