...
public class SmsPreferenceActivity extends PreferenceActivity implements Preference.OnPreferenceChangeListener {
    ...
    private List<EncapsulatedSimInfoManager> mListSimInfo;
    ...
    private void setListPrefSummary() {
        ...
        if (MmsConfig.getSmsMultiSaveLocationEnabled()) {
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                int slotId = 0;
                if (simCount == 1) {
                    slotId = EncapsulatedSimInfoManager.getInsertedSimInfoList(this).get(0).getSlot();
        ...
        if (saveLocation == null) {
            /// M: fix bug ALPS00429244
            saveLocation = sp.getString(SMS_SAVE_LOCATION, "Phone"); 
        }
        if (getResources().getBoolean(R.bool.isTablet)) {
            ///@ M: fix bug ALPS00823808, avoid empty of the sms stroe lactaion at the first time.
    ...
    private void setMessagePreferences() {
        mListSimInfo = EncapsulatedSimInfoManager.getInsertedSimInfoList(this);
        mCurrentSimCount = mListSimInfo.isEmpty()? 0: mListSimInfo.size();
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (mCurrentSimCount == 1) {
                ...
                if (mListSimInfo.size() != 0) {
                    ...
                    if (mStringReplacementPlugin.isEnableStringReplacement()
                            && location != null && MessageUtils.isUSimType(slotId)) {
    ...
    private void changeSingleCardKeyToSimRelated() {
        ...
        mListSimInfo = EncapsulatedSimInfoManager.getInsertedSimInfoList(this);
        EncapsulatedSimInfoManager singleCardInfo = null;
        if (mListSimInfo.size() != 0) {
        ...
        if (mStringReplacementPlugin.isEnableStringReplacement() && ctString != null) {
            mManageSimPref.setTitle(ctString);
            mManageSimPref.setSummary(
                    mStringReplacementPlugin.getCTStrings(IStringReplacement.MANAGE_CARD_MSG_SUMMARY));
        }
        /// @}
        ...
        if (MmsConfig.getSmsMultiSaveLocationEnabled()) {
            ...
            if (!getResources().getBoolean(R.bool.isTablet)) {
                ...
                mSmsLocation.setValue(spr.getString((Long.toString(slotid) + "_" + SMS_SAVE_LOCATION), "Device"));
            }
            ///@
        }
    ...
    private void setMultiCardPreference() {
        ...
        if (mStringReplacementPlugin.isEnableStringReplacement() && ctString != null) {
            ...
            mManageSimPrefMultiSim.setSummary(
                    mStringReplacementPlugin.getCTStrings(IStringReplacement.MANAGE_CARD_MSG_SUMMARY));
        }
        /// @}
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mManageSimPref) {
            ...
            if (EncapsulatedFeatureOption.EVDO_DT_SUPPORT && MessageUtils.isUSimType(id)) {
                ...
                if (!EncapsulatedGeminiSmsManager.isSmsReady(id)) {
                ...
                try {
                    if (teleService == null) {
                        gotScNumber = null;
                        MmsLog.e(MmsApp.TXN_TAG, "teleService == null");
                    } else {
                        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                            int slotId = mListSimInfo.get(0).getSlot();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Dialog locationDialog = mSmsLocation.getDialog();
                if (action.equals(EncapsulatedTelephonyIntents.ACTION_SIM_INDICATOR_STATE_CHANGED)
                    || action.equals(EncapsulatedTelephonyIntents.ACTION_SIM_INFO_UPDATE)) {
                mListSimInfo = EncapsulatedSimInfoManager.getInsertedSimInfoList(context);
                mCurrentSimCount = mListSimInfo.isEmpty()? 0: mListSimInfo.size();
