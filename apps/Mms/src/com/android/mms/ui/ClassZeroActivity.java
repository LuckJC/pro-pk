...
public class ClassZeroActivity extends Activity {
    ...
    /// M: Plug-in for OP09.
    private IDisplayClassZeroMessage mDisplayClassZeroPlugin;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == ON_AUTO_SAVE) {
                ...
                processNextMessage();
            /// M: add for OP09. @{
            } else if (mDisplayClassZeroPlugin.isEnableClassZeroFeature()
                    && msg.what == SAVE_WITHOUT_DESTROY_ACTIVITY) {
                mRead = false;
                this.removeMessages(ON_AUTO_SAVE);
                saveMessage((SmsMessage[])msg.obj);
                processNextMessage();
            } /// @}
        }
    };
    ...
    /// M: add for OP09. @{
    private void saveMessage(SmsMessage[] message) {
        Uri messageUri = null;
        if (message[0].isReplace()) {
            messageUri = replaceMessage(message);
        } else {
            messageUri = storeMessage(message);
        }
        if (!mRead && messageUri != null) {
            MessagingNotification.nonBlockingUpdateNewMessageIndicator(
                    this,
    ...
    private ContentValues extractContentValues(SmsMessage sms) {
        ...
        values.put(EncapsulatedTelephony.Sms.Inbox.SIM_ID, EncapsulatedSmsMessageEx.getMessageSimId(sms));
        /// @}
    ...
    private Uri replaceMessage(SmsMessage[] Msgs) {
        ...
        selectionArgs = new String[] {
                ...
                Integer.toString(EncapsulatedSmsMessageEx.getMessageSimId(Msgs[0]))
        };
        /// @}
    ...
    private void displayZeroMessage(MessageContent messageContent) {
        ...
        /// M: for OP09 @{
        if (mDialog != null && mDisplayClassZeroPlugin.isEnableClassZeroFeature()) {
            mDialog.dismiss();
        }
        /// @}
...
class MessageContent {
    ...
    public MessageContent(SmsMessage[] messages, int simId) {
        mMessages = messages;
        mSimId = simId;
    }
    ...
    public MessageContent(Intent intent) {
        ...
        mSimId = intent.getIntExtra(EncapsulatedPhone.GEMINI_SIM_ID_KEY, EncapsulatedPhone.GEMINI_SIM_1);
    }
}
