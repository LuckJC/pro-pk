...
import com.mediatek.encapsulation.android.util.EncapsulatedPatterns;
import com.mediatek.encapsulation.com.android.ex.chips.EncapsulatedMTKRecipient;
import com.mediatek.encapsulation.com.android.ex.chips.EncapsulatedMTKRecipientEditTextView;
import com.mediatek.encapsulation.com.android.ex.chips.EncapsulatedMTKRecipientList;
...
//    }
    ...
    public void populate(ContactList list) {
        ...
        if (list.size() == 0) {
            ...
            EncapsulatedMTKRecipientList recipientList = new EncapsulatedMTKRecipientList();
            for (Contact c : list) {
                ...
                EncapsulatedMTKRecipient recipient = new EncapsulatedMTKRecipient(displayName,c.getNumberProtosomatic());
            ...
            MmsLog.v(TAG,"RecipientsEditor:populate, end");
            /// @}
        }
    }
...
//            }
    ...
    protected void updatePressedChip(UpdatePressedChipType type) {
        MmsLog.v(TAG,"RecipientsEditor:updatePressedChip, mChipX = " + mChipX + ", mChipY = "
                + mChipY + ", type = " + type);
    ...
    /// (131) will be think of comments, will be cut. so use PHONE_PATTERN_MTK to walk around. @{
    private static boolean isPhoneNumber(String number) {
        
        if (TextUtils.isEmpty(number)) {
            return false;
        }
        Matcher match = EncapsulatedPatterns.PHONE_PATTERN_MTK.matcher(number);
    ...
    /// M: fix ALPS01029174, if the last number in recipients editor is end without "," or ";",
    /// will not be shown on title, meanwhile, this number will not be reload after resume. fix it.
    private Contact mLastContact;
    public Contact getLastContactInEditor() {
        return mLastContact;
    }
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        MmsConfig.setRecipientsEditorOutAtts(outAttrs);
        return inputConn;
    }
    /// @}
}
