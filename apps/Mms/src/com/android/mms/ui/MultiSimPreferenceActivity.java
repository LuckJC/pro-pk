...
public class MultiSimPreferenceActivity extends PreferenceActivity implements GetSimInfo {
    ...
    private boolean isChecked(String preference,int sim_id){
        ...
        if (mIntentPreference.equals(SmsPreferenceActivity.SMS_DELIVERY_REPORT_MODE)) {
            ...
            if (EncapsulatedFeatureOption.EVDO_DT_SUPPORT && isUSimType((int)(mListSimInfo.get(sim_id)).getSlot())) {
    ...
    private void updateSimState(int slotId) {
        AdvancedCheckBoxPreference pref = null;
        for (AdvancedCheckBoxPreference simPreference : mSimPreferencesList) {
    ...
    public boolean isUSimType(int slot) {
        ...
        try {
            return iTel.getIccCardType(slot).equals("UIM");
        } catch (RemoteException e) {
