...
public class MmsConfig {
    ...
    private static void loadSimInfo(Context context) {
        ...
        if (sSim1Id <= 0 && sSlot1SimExist) {
            sSim1Id = EncapsulatedSimInfoManager.getIdBySlot(context, EncapsulatedPhone.GEMINI_SIM_1);
            sSlot1RetryCounter++;
        ...
        if (sSim2Id <= 0 && sSlot2SimExist) {
            sSim2Id = EncapsulatedSimInfoManager.getIdBySlot(context, EncapsulatedPhone.GEMINI_SIM_2);
            sSlot2RetryCounter++;
            /// M: if we tried 3 times and still can't get valid simId , we think the slot is empty.
    ...
    public static boolean isMobileDataEnabled() {
        return mMmsConfigPlugin.isMobileDataEnabled();
    }
    ...
    /// @}
    public static boolean isSupportAutoSelectSimId() {
        return mMmsConfigPlugin.isSupportAutoSelectSimId();
    }

    public static boolean isSupportAsyncUpdateWallpaper() {
        return mMmsConfigPlugin.isSupportAsyncUpdateWallpaper();
    }
    ...
    public static boolean isSupportVCardPreview() {
        return mMmsConfigPlugin.isSupportVCardPreview();
    }
    ...
    public static boolean isSupportCTFeature() {
        boolean isCTFreatur = mMmsConfigPlugin.isSupportCTFeature();
        MmsLog.d(TAG, "isSupportCTFeature:" + isCTFreatur);
        return isCTFreatur;
    }

    public static boolean isAllowDRWhenRoaming(Context context, int slotId) {
        return mMmsConfigPlugin.isAllowDRWhenRoaming(context, slotId);
    }
    /// @}
    ...
    public static boolean isSupportTabSetting() {
        return mMmsConfigPlugin.isSupportTabSetting();
    }
    ...
    public static boolean isSupportMessagingNotificationProxy() {
        return mMmsConfigPlugin.isSupportMessagingNotificationProxy();
    }
}
