...
import com.android.mms.ui.WPMessageDialogActivity;
...
public class SiManager extends WapPushManager {
    ...
    public void handleIncoming(ParsedMessage message) {
        ...
        if (MmsConfig.isSupportCTFeature()) {
            ...
            intent.setComponent(new ComponentName(mContext, WPMessageDialogActivity.class));
            intent.putExtra(WapPush.ADDR, siMsg.getSenderAddr());
            intent.putExtra(WapPush.SERVICE_ADDR, siMsg.getServiceCenterAddr());
            intent.putExtra(WapPush.SIM_ID, siMsg.getSimId());
            intent.putExtra(WapPush.URL, siMsg.url);
            intent.putExtra(WapPush.SIID, siMsg.siid);
            intent.putExtra(WapPush.ACTION, siMsg.action);
            intent.putExtra(WapPush.CREATE, siMsg.create);
            intent.putExtra(WapPush.EXPIRATION, siMsg.expiration);
            intent.putExtra(WapPush.TEXT, siMsg.text);
