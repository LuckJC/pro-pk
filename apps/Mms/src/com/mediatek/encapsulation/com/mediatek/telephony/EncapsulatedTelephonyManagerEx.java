...
public class EncapsulatedTelephonyManagerEx {
    ...
    public void listen(PhoneStateListener listener, int events, int simId) {
        if (EncapsulationConstant.USE_MTK_PLATFORM) {
            ...
            MmsLog.d("Encapsulation issue", "EncapsulatedTelephonyManagerEx -- listen()");
        }
    }
   ...
   public int getSimIndicatorStateGemini(int simId) {
       if (EncapsulationConstant.USE_MTK_PLATFORM) {
           ITelephonyEx iTelephonyEx = ITelephonyEx.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICEEX));
            try{
                return iTelephonyEx.getSimIndicatorState(simId);
            } catch (RemoteException ex) {
                Log.d(TAG,"getSimIndicatorStateGemini RemoteException " + ex);
                return EncapsulatedPhone.SIM_INDICATOR_UNKNOWN;
            } catch (NullPointerException ex) {
                Log.d(TAG,"getSimIndicatorStateGemini NullPointerException " + ex);
                return EncapsulatedPhone.SIM_INDICATOR_UNKNOWN;
            }
       } else {
           try {
