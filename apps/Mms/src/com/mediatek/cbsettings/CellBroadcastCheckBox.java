...
import static com.mediatek.cbsettings.TimeConsumingPreferenceActivity.EXCEPTION_ERROR;

import com.mediatek.encapsulation.com.android.internal.telephony.EncapsulatedPhone;
//import com.android.phone.PhoneGlobals;
import com.mediatek.encapsulation.MmsLog;
import com.android.internal.telephony.gsm.SmsBroadcastConfigInfo;
import com.mediatek.cbsettings.TimeConsumingPreferenceListener;
...
public class CellBroadcastCheckBox extends CheckBoxPreference {
    ...
    public void init(TimeConsumingPreferenceListener listener, boolean skipReading, int slotId) {
        MmsLog.d(LOG_TAG,"init, slotId = " + slotId);
        mListener = listener;
        mSlotId = slotId;

        if (!skipReading) {
            boolean hasIccCard;
            hasIccCard = hasIccCard(mSlotId);
    ...
    private void getCBState() {
        ...
        mIsCbEanbled = EncapsulatedSmsManager.queryCellBroadcastSmsActivation(mSlotId);
    ...
    public boolean hasIccCard(int slotId) {
        boolean hasIccCard;
        if (isGeminiSupport() && slotId < EncapsulatedPhone.GEMINI_SIM_NUM) {
            EncapsulatedTelephonyManager manager = new EncapsulatedTelephonyManager();
            hasIccCard = manager.hasIccCard(slotId);
        } else {
            hasIccCard = TelephonyManager.getDefault().hasIccCard();
        }
        MmsLog.d(LOG_TAG, "[hasIccCard], slotId = " + slotId + "; hasIccCard = " + hasIccCard);
        return hasIccCard;
    }
}
...
class RecoverChannelSettings {
    ...
    public RecoverChannelSettings(int simId, ContentResolver resolver) {
        ...
        if (isGeminiSupport()) {
            switch (mSimId) {
                ...
                break;
                default:
                MmsLog.d(LOG_TAG,"error with simid = " + mSimId);
                break;
            }
        }
    }
    ...
    private void setCellBroadcastConfig(SmsBroadcastConfigInfo[] objectList, int fromId, int toId) {
        ...
        boolean isSetConfigSuccess = EncapsulatedSmsManager.setCellBroadcastSmsConfig(objectList, objectList, mSimId);
