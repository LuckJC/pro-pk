...
public interface IMmsConfig {
    ...
    /**
     * Returns is if mobile data enable or not
     *
     * @return              true if retain, otherwise return false
     */
    ...
    /**
     *if support auto select simId to send/reply message for cmcc project.
     *when select "Auto" in simManager.
     *Conversation Mode: select the last message(in/out) sim Id, and for popup simselectdialog when
     *new message;
     *Fode Mode: when reply message, select the received sim id;
     *Dialog Mode: when reply message, select the sim id in current view message.
     */
    boolean isSupportAutoSelectSimId();

    /**
     * when update wallpaper in main thread will cause ANR.
     */
    boolean isSupportAsyncUpdateWallpaper();

    /**
     * Returns whether the feature vCard preview is enable.
     *
     * @return true if enable, otherwise return false
     */
    boolean isSupportVCardPreview();
     ...
     * @return
     */
    boolean isSupportCBMessage(Context context, int simId);

    /**
     * M:
     * @return  true: support OP09; false: not support OP09;
     */
    boolean isSupportCTFeature();

    /**
     * M: for OP09, whether allow to request delivery report when in international roaming status.
     * @param slotId. Only CDMA card is not allowed.
     * @return true: Request delivery report allowed.
     *         false: Request delivery report not allowed.
     */
    boolean isAllowDRWhenRoaming(Context context, int slotId);
    ...
    /**
     * M:
     * @return  true: support tab setting; false: not support tab setting;
     */
    boolean isSupportTabSetting();
    ...
    /* when view messaging notification,if there will lost the editing draft, dialog to indicate first.
     *
     */
    boolean isSupportMessagingNotificationProxy();
}
