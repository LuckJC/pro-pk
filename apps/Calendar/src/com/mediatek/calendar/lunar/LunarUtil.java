...
public class LunarUtil {
    ...
    public Calendar decreaseOrIncreaseALunarYear(Calendar calendar, int lunarMonth, int lunarDay,
        ...
        while (true) {
            ...
            /*
             * M: There is no corresponding lunar day in next year, for example:
             * there is 30 days in lunar 201303, but there is only 29 days in
             * lunar 201403, when move year form 2013 to 2014, we should adjust
             * the date.
             */
            if ((operatorType == INCREASE_A_LUANR_YEAR)
                    && (lunarDates[1] == lunarMonth)
                    && (lunarDates[2] < lunarDay)) {
                newCalendar.add(Calendar.DAY_OF_MONTH, 1);
                break;
            }
            /*
             * M: There is no corresponding lunar day in last year, for example:
             * there is 30 days in lunar 201003, but there is only 29 days in
             * lunar 200903, when move year form 2010 to 2009, we should adjust
             * the date.
             */
            if ((operatorType == DECREATE_A_LUANR_YEAR)
                    && ((lunarDates[0] + 1) == lunarYear)
                    && (lunarDates[1] > lunarMonth)) {
                break;
            }
