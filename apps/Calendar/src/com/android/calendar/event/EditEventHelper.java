...
public class EditEventHelper {
    ...
    public static final String[] EVENT_PROJECTION = new String[] {
            ...
            ///M:#Lunar#
            ExtConsts.LunarEvent.IS_LUNAR, ///M:25
    };
    ...
    ///M:#Lunar#
    protected static final int EVENT_INDEX_IS_LUNAR = 25;
    ...
    public static boolean saveReminders(ArrayList<ContentProviderOperation> ops, long eventId,
        ...
        /**M: Delete data in CalendarAlert to guarantee that a event's reminder doesn't come out
           if it has been removed. @{ */
        ArrayList<ReminderEntry> removedReminder = new ArrayList<ReminderEntry>();
        for (ReminderEntry oReminder : originalReminders) {
            if (!reminders.contains(oReminder)) {
                removedReminder.add(oReminder);
            }
        }
        if (!removedReminder.isEmpty()) {
            String wh = CalendarAlerts.EVENT_ID + "=? AND " + CalendarAlerts.MINUTES + "=?";
            for (ReminderEntry rReminder : removedReminder) {
                String[] arg = new String[] {Long.toString(eventId), Integer.toString(rReminder.getMinutes())};
                ContentProviderOperation.Builder bd = ContentProviderOperation
                        .newDelete(CalendarAlerts.CONTENT_URI);
                bd.withSelection(wh, arg);
                ops.add(bd.build());
            }
        }
        /** @} */
