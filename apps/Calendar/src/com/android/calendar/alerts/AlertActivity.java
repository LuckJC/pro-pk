...
public class AlertActivity extends Activity implements OnClickListener {
    ...
    /*
     * M: Flag used to manage notification update. Do notification update only
     * when an click operation occurred.
     */
    ...
    private final OnItemClickListener mViewListener = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position,
            ...
            /*
             * M: set the update flag to true, the update would be done in
             * later.
             */
            mDoUpdate = true;
    ...
    protected void onStop() {
        ...
        /** KK: AsyncTask task = new AsyncTask<Context, Void, Void>() {
            @Override
        ...
        /*
         * M: Comment out AlertService.updateAlertNotification, instead starting
         * AlertService to handle update. The function "updateAlertNotification"
         * would do database query and write operations on main thread, it would
         * block the main thread, and make ANR to occur sometimes.
         */
        if (mDoUpdate) {
            Intent i = new Intent();
            i.setClass(getApplicationContext(), AlertService.class);
            i.putExtra("action",
                    android.provider.CalendarContract.ACTION_EVENT_REMINDER);
            AlertReceiver.beginStartingService(this, i);
        }
