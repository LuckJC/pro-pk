...
public class EventInfoFragment extends DialogFragment implements OnCheckedChangeListener,
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mLunar = LunarUtil.getInstance(mContext);
        ///M:NFC.
    ...
    private void updateEvent(View view) {
        ...
        /** M: lunar calendar enhancement. @{ */
        if (mLunar.canShowLunarCalendar()) {
            displayedDatetime += getLunarDisplayedDate(localTimezone);
        }
        /**@}*/
   ...
   ///M: add removeDuplicate() function. @{
    ...
    private String getLunarDisplayedDate(String localTimezone) {
        Time startDate = new Time(localTimezone);
        startDate.set(mStartMillis);
        startDate.normalize(false);
        Time endDate = new Time(localTimezone);
        endDate.set(mEndMillis);
        if (mAllDay) {
            endDate.monthDay -= 1;
        }
        ...
        if ((startDate.year == endDate.year) && (startDate.month == endDate.month)
            ...
            if (isToday(mStartMillis, System.currentTimeMillis(), startDate.gmtoff, mAllDay, localTimezone)) {
                return "";
            }
        }

        return mLunar.formatLunarDateRange(mContext, startDate, endDate, showDateRange);
    }
    ...
    private boolean isToday(long dayMillis, long currentMillis, long localGmtOffset, boolean allday,
        ...
        if (allday) {
            ...
            dayMillis = Utils.convertAlldayUtcToLocal(null, dayMillis, localTimezone);
        }
