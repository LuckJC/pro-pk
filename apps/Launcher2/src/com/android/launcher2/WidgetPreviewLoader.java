...
public class WidgetPreviewLoader {
        ...
        new AsyncTask<Void, Void, Void>() {
            public Void doInBackground(Void ... args) {
                ...
                try {
                    ...
                    LauncherLog.e(TAG, "removeFromDb: Error delete", e);
                }
                synchronized(sInvalidPackages) {
    ...
    public Bitmap generateWidgetPreview(ComponentName provider, int previewImage,
        ...
        if (maxPreviewWidth <= 0) {
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "getWidgetPreview: packageName = " + packageName +", maxPreviewWidth = " + maxPreviewWidth);
            }
            maxPreviewWidth = Integer.MAX_VALUE;
        }
        if (maxPreviewHeight <= 0) {
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "getWidgetPreview: packageName = " + packageName +", maxPreviewHeight = " + maxPreviewHeight);
            }
