...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener, ScreenEffect {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "dispatchDraw: mScrollX = " + mScrollX + ", screenCenter = " + screenCenter
                    + ", mOverScrollX = " + mOverScrollX + ", mUnboundedScrollX = " + mUnboundedScrollX + ", mMaxScrollX = "
                    + mMaxScrollX + ", mLastScreenCenter = " + mLastScreenCenter + ", mLeft = " + mLeft + ", mRight = "
                    + mRight + ",mForceScreenScrolled = " + mForceScreenScrolled + ",getWidth() = " + getWidth()
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "getScrollProgress: screenCenter = " + screenCenter + ", page = " + page + ", v = " + v
                    + ",totalDistance = " + totalDistance + ", getScaledMeasuredWidth(v) = " + getScaledMeasuredWidth(v)
                    + ", mPageSpacing = " + mPageSpacing + ", delta = " + delta + ", halfScreenSize = " + halfScreenSize
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (isSupportCycleSlidingScreen() && (mCurrentPage == 0 && whichPage == getChildCount() - 1)
                || (mCurrentPage == getChildCount() - 1 && whichPage == 0)) {
            if (newX == 0 && (mScrollX <= 0 || mScrollX >= mMaxScrollX)) {
                delta += getWidth();
