...
public class ContactsMultiDeletionFragment extends MultiContactsPickerBaseFragment {
    ...
    public int getMultiChoiceLimitCount() {
        int allowdMaxItemsFromPlugin = ExtensionManager.getInstance().getContactListExtension()
                .getMultiChoiceLimitCount(ContactPluginDefault.COMMD_FOR_OP01);
        LogUtils.d(TAG, "[providAllowedMaxItems] allowdMaxItemsFromPlugin is:" + allowdMaxItemsFromPlugin);
        if (allowdMaxItemsFromPlugin != -1) {
            return allowdMaxItemsFromPlugin;
        } else {
            return super.getMultiChoiceLimitCount();
        }
    }
