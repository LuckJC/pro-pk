...
public class ContactGroupListFragment extends GroupBrowseListFragment {
    ...
    public void onOkClick() {
        if (getListView().getCheckedItemCount() == 0) {
            LogUtils.d(TAG, "[onOkClick]tap OK when no item selected");
        ...
        for (int position = 0; position < listSize; ++position) {
            if (getListView().isItemChecked(position)) {
                ...
                LogUtils.d(TAG, "[onOkClick]position " + position + " item is not checked");
            }
        }

        if (selectedGroupIdList.isEmpty()) {
            LogUtils.d(TAG, "[onOkClick]finally, no group selected");
            getActivity().setResult(Activity.RESULT_CANCELED, null);
            getActivity().finish();
            return;
        }
    ...
    public class GroupQueryTask extends WeakAsyncTask<List<String>, Void, long[], Activity> {
        ...
        protected long[] doInBackground(Activity target, List<String>... params) {
            ...
            if (groupIdList == null || groupIdList.isEmpty()) {
                LogUtils.e(TAG, "doInBackground groupIds is empty");
                return null;
            }
            ...
            LogUtils.i(TAG, whereBuilder.toString());
            ...
            try {
                ...
                if (cursor != null) {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        LogUtils.i(TAG, "dataId is " + String.valueOf(id));
            ...
            LogUtils.d(TAG, "[onOkClick]phoneIdsList size is " + phoneIdsList.size());
