...
public class GroupMembershipView extends LinearLayout
    ...
    /**
     * 
     * @param enable true to can create group,but this is just the fist switch.it will not do the finally determine.
    ...
    private boolean isCreateGroupEnable() {
        if (mAccountType == null) {
            LogUtils.w(TAG, "[isCreateGroupEnable] mAccountType is null,returl false to disable create group!");
            return false;
        }
        return (mIsCreateGroupEnable) && (!mAccountType.equals(AccountType.ACCOUNT_TYPE_USIM));
    }

    private boolean mIsCreateGroupEnable = true;
