...
public class KindSectionView extends LinearLayout implements EditorListener {
    ...
    public void rebuildFromState() {
        ...
        if (cae.isFeatureAccount(accountType, ExtensionManager.COMMD_FOR_AAS)
                && cae.isPhone(mKind.mimeType, ExtensionManager.COMMD_FOR_AAS)) {
    ...
    protected void updateAddFooterVisible(boolean animate) {
        if (!mReadOnly && (mKind.typeOverallMax != 1)) {
            ...
            if (!hasEmptyEditor() && RawContactModifier.canInsert(mState, mKind)) {
                /** M:AAS @ { */
                final int slotId = ExtensionManager.getInstance().getContactAccountExtension()
                ...
                if (ExtensionManager.getInstance().getContactAccountExtension().isFeatureAccount(
                    ...
                    ExtensionManager.getInstance().getContactDetailExtension().updateView(
                            mAddFieldFooter, 0,
                            ContactDetailExtension.VIEW_UPDATE_VISIBILITY,
                            ExtensionManager.COMMD_FOR_AAS);
                    return;
                }
                /** M: @ } */
    ...
    private void updateEmptyEditors() {
        ...
        // If there is more than 1 empty editor, then remove it from the list of
        // editors.
        /** M:AAS max default value is 1 @ { */
        int max = ExtensionManager.getInstance().getContactDetailExtension().getMaxEmptyEditors(
                mKind.mimeType, ExtensionManager.COMMD_FOR_AAS);
        Log.d(TAG, "updateEmptyEditors() max =" + max + " emptyEditors.size()="
                + emptyEditors.size() + ", mEditors="
    ...
    public void addSimItem() {
        ...
        if (mKind.typeOverallMax == 1) {
            ...
            ArrayList<ValuesDelta> entries = mState.getMimeEntries(mKind.mimeType);
            if (entries != null && entries.size() > 0) {
                simValues = entries.get(0);
            }
        }
        if (simValues == null) {
            simValues = RawContactModifier.insertChild(mState, mKind);
        }

        final View newField = createEditorView(simValues);
        if (newField instanceof Editor) {
            postWhenWindowFocused(new Runnable() {
                @Override
                public void run() {
    ...
    protected void onChildViewRemoved(View view) {
        ...
        if (mState.getMimeEntries(mKind.mimeType) != null) {
            LogUtils.d(TAG, "[onViewRemoved] remove the data,viewIndex:" + viewIndex + ",mimeType:" + mKind.mimeType
                    + ",title:" + mTitleString);
