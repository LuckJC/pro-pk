...
public class ContactDetailViewPagerAdapter extends PagerAdapter {
    ...
    public ContactDetailViewPagerAdapter() {
      ///M:@{
        mViewPagerAdapterEx = new ContactDetailViewPagerAdapterEx(mFragmentViewCount);
        ///@}
    }
    ...
    public void enableSwipe(boolean enable) {
        ...
        ///M:@{
        mViewPagerAdapterEx.updateOffset(mFragmentViewCount);
        ///@}
    ...
    @Override
    public int getCount() {
        return mFragmentViewCount + mViewPagerAdapterEx.getCount();
    }

    /** Gets called when the number of items changes. */
    @Override
    public int getItemPosition(Object object) {
        int pos = mViewPagerAdapterEx.getItemPosition(object);
        if (pos != -1) {
            return pos;
        }
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        View view = (View)mViewPagerAdapterEx.instantiateItem(container, position);
        if (view != null) {
    ...
    public void addFragmentView(View view, String tag) {
        mViewPagerAdapterEx.addFragmentView(view, tag);
    }
    private ContactDetailViewPagerAdapterEx mViewPagerAdapterEx ;
}

/**
 * Not ready Adapter,just Manager a list of Adapter's views. So no need to
 * extends Adapter.
 */
class ContactDetailViewPagerAdapterEx {
    private HashMap<String, View> mViewPagerViews = new HashMap<String, View>();
    private HashMap<Integer, String> mViewPositionMapTags = new HashMap<Integer, String>();
    private int mPositionOffset = 0;

    public ContactDetailViewPagerAdapterEx(int offset) {
        mPositionOffset = offset;
    }

    public void addFragmentView(View view, String tag) {
        mViewPositionMapTags.put(mViewPagerViews.size() + mPositionOffset, tag);
        mViewPagerViews.put(tag, view);
    }
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        if (mViewPositionMapTags.containsKey(position)) {
            View view = mViewPagerViews.get(mViewPositionMapTags.get(position));
            view.setVisibility(view.VISIBLE);
            return view;
        }
        return null;
    ...
    public int getItemPosition(Object object) {
        int position = -1;
        for (Integer key : mViewPositionMapTags.keySet()) {
            View view = mViewPagerViews.get(mViewPositionMapTags.get(key));
            if (view == object) {
                position = key;
                break;
            }
    ...
    public void updateOffset(int offset) {
        int x = offset - mPositionOffset;
        if (x != 0) {
            int size = mViewPositionMapTags.size();
            for (int i = mPositionOffset; i < (mPositionOffset + size); i++) {
                mViewPositionMapTags.put(i + x, mViewPositionMapTags.get(i));
                mViewPositionMapTags.remove(i);
            }
        }
        mPositionOffset = offset;
    }
