    ...
    public void onSaveCompleted(boolean hadChanges, Uri groupUri) {
        ...
        if (mListener != null) {
            ...
            if (resultIntent != null) {
                AccountCategoryInfo info = new AccountCategoryInfo(mAccountType, mSlotId, mSimId, mAccountName);
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
          ///M:check whether the fragment still in Activity@{
            ...
            while (data.moveToNext()) {
                ...
                SimInfoRecord simInfo = mSimInfoWrapper.getSimInfoBySlot(mSlotId);
                if (simInfo != null) {
                    i = simInfo.mColor;
                }
                if (simIndex > 0) {
...
//        private final int mSimSlotId;
    ...
    private boolean checkSimServiceState(boolean showTips) {
        ...
        if (mSlotId >= 0 && SIMServiceUtils.isServiceRunning(mSlotId)) {
            // for CT NEW FEATURE change uim/sim string start
            ...
            if (ExtensionManager.getInstance().getContactDetailEnhancementExtension()
                ...
                msg_loading_sim_contacts_toast = R.string.msg_loading_sim_contacts_toast_ct;
            }
            // for CT NEW FEATURE change uim/sim string end
            if (showTips) {
        ...
        if (ContactSaveService.isGroupTransactionProcessing()) {
            LogUtils.w(TAG, "checkSimServiceState(),mSlotId=" + mSlotId + ",SaveGroup Failed..");
            if (showTips) {
            ...
            try {
                if (rawContactIds != null && rawContactIds.size() > 0 && ContactsCursor != null) {
                    ...
                    do {
                        ...
                        SimInfoRecord simInfo = mFragment.mSimInfoWrapper
