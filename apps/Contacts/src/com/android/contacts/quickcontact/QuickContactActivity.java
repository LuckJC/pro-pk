...
public class QuickContactActivity extends Activity {
    ...
    private void bindData(Contact data) {
        ...
        /** M:AAS @ { */
        ExtensionManager.getInstance().getContactAccountExtension().setCurrentSlot(data.getSlot(),
        ...
        /*  M: add by mtk, keep star consistent with ContactDetailActivity, don't
         handle sim contact @ {  */
        if (data != null && data.getIndicate() < 0) {
            ...
            if (UriUtils.isEncodedContactUri(lookupUri)) {
                ...
                // If directory export support is not allowed, then don't allow
                // the user to add
                // to contacts
                ...
                configureHeaderClickActions(false);
                //M: if is userprofile also hide the star @{
                if (!data.isUserProfile() && !data.isDirectoryEntry()) {
                ...
                mStarImage.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        if (lookupUri != null) {
                            // Changes the state of the image already before
                            ...
                            final Intent intent = ContactSaveService
                                    .createSetStarredIntent(context, lookupUri, !isStarred);
        ...
        for (RawContact rawContact : data.getRawContacts()) {
            for (DataItem dataItem : rawContact.getDataItems()) {
                ...
                /**
                 * M: for RCSe, once the dataKind is null, it should new a dataKind instead
                 * so we remove the "final"
                 * google default code: @{
                final DataKind dataKind = AccountTypeManager.getInstance(this)
                        .getKindOrFallback(accountType, mimeType);
                 @} */
                DataKind dataKind = AccountTypeManager.getInstance(this)
                        .getKindOrFallback(accountType, mimeType);
             ...
             */
                ...
                boolean reslut = ExtensionManager.getInstance().getContactDetailExtension()
                        .getExtentionKind(mimeType, true,
                                dataItem.getContentValues().getAsString(Contacts.DISPLAY_NAME),
                                ExtensionManager.COMMD_FOR_RCS);
                if (reslut && dataKind == null) {
                    dataKind = new DataKind(mimeType, 0, 10, false);
                    dataKind.actionBody = new SimpleInflater(Data.DATA1);
                    dataKind.titleRes = 0;
                }
            /*
             ...
             */
                 ...
                 */
                    ...
                    final Action action = new QuickDataAction(context, dataItem, isDirectoryEntry, dataKind);
                /*
                 * New Feature  by Mediatek End.
                */
        ...
        // Collapse Action Lists (remove e.g. duplicate e-mail addresses from
        // different sources)
        ...
        ///M:fix ALPS01266882,support show display name sort by first-name or last-name@{
        // setHeaderNameText(R.id.name, data.getDisplayName());
        setHeaderNameText(R.id.name, ContactDetailDisplayUtils.getDisplayName(this, mContactData));
        ///@}
...
//                throw new IllegalStateException("Failed to load contact", data.getException());
    ...
    private class ViewPagerAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int position) {
             ...
             */
            QuickContactListFragment fragment = new QuickContactListFragment(QuickContactActivity.this, mimeType);
            /*
             * New Feature  by Mediatek End.
             */
    ...
    protected void onResume() {
        ...
        ///M:fix ALPS01266882,support show display name sort by first-name or last name@{
        ///the settings may be change when resume
        if (mContactData != null) {
            setHeaderNameText(R.id.name, ContactDetailDisplayUtils.getDisplayName(this, mContactData));
        }
        ///@}
