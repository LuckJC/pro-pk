...
public class ContactInfoCache implements ContactsAsyncHelper.OnImageLoadCompleteListener {
    ...
    public void findInfo(final CallIdentification identification, final boolean isIncoming,
        ...
        if (cacheEntry != null
                /// M: for ALPS01427149 && ALPS01437550 @{
                // DO NOT handle the temp contact info to avoid contacts info flash, see ALPS01437550
                && !mExpiredInfoMap.containsKey(callId)) {
                /// @}
    ...
    private void findInfoQueryComplete(CallIdentification identification,
        ...
        if (didLocalLookup) {
            if (!callerInfo.contactExists && cacheEntry.name == null &&
                ...
                /// M: for ALPS01427149 && ALPS01437550 @{
                handleExpiredContactsQueryComplete(callId, cacheEntry);
                /// @}
    ...
    public void onImageLoadComplete(int token, Drawable photo, Bitmap photoIcon, Object cookie) {
        ...
        /// M: for ALPS01427149 && ALPS01437550 @{
        handleExpiredContactsQueryComplete(callId, entry);
        /// @}
...
//                    }
    ...
    private final HashMap<Integer, ContactCacheEntry> mExpiredInfoMap = Maps.newHashMap();

    public interface ContactInfoExpiredListener {
        void onContactInfoExpired(int callId);
    }

    private List<ContactInfoExpiredListener> mContactInfoExpiredListener = new ArrayList<ContactInfoExpiredListener>();

    public void addContactInfoUpdateListener(ContactInfoExpiredListener listener) {
        if (!mContactInfoExpiredListener.contains(listener)) {
            mContactInfoExpiredListener.add(listener);
        }
    }

    public void removeContactInfoUpdateListener(ContactInfoExpiredListener listener) {
        if (mContactInfoExpiredListener.contains(listener)) {
            mContactInfoExpiredListener.remove(listener);
        }
    }

    public void notifyContactInfoExpired(int callId) {
        for(ContactInfoExpiredListener listener:mContactInfoExpiredListener) {
            listener.onContactInfoExpired(callId);
        }
    }

    public void updateContactInfoCache(CallIdentification identification) {
        Log.d(TAG, "updateContactInfoCache: " + identification);
        int callId = identification.getCallId();
        if(mInfoMap.containsKey(callId)) {
            mExpiredInfoMap.put(callId, mInfoMap.get(callId));
        }
        notifyContactInfoExpired(callId);
    }

    /**
     *  when expired contacts info query complete, update expired map and notify update
     * @param callId
     * @param entry
     */
    private void handleExpiredContactsQueryComplete(final int callId, final ContactCacheEntry entry) {
        if(mExpiredInfoMap.containsKey(callId)) {
            mExpiredInfoMap.remove(callId);
