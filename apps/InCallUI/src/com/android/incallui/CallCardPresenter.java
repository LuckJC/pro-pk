...
import com.android.incallui.ContactInfoCache.ContactInfoExpiredListener;
...
import com.mediatek.incallui.vt.VTUtils.VTTimingMode;
...
public class CallCardPresenter extends Presenter<CallCardPresenter.CallCardUi>
        /**
         * M: add PhoneRecorderListener, VTListener, ContactInfoExpiredListener
        implements InCallStateListener, AudioModeListener, IncomingCallListener {
         */
        implements InCallStateListener, AudioModeListener, IncomingCallListener, PhoneRecorderListener, VTListener, ContactInfoExpiredListener {
    ...
    public void init(Context context, Call call) {
        ...
        ContactInfoCache.getInstance(mContext).addContactInfoUpdateListener(this);
        /// @}
    ...
    public void onUiUnready(CallCardUi ui) {
        ...
        /// M: for ALPS01427149 @{
        ContactInfoCache.getInstance(mContext).removeContactInfoUpdateListener(this);
        /// @}
    }
    ...
    public void onStateChange(InCallState state, CallList callList) {
        ...
        */
        ...
        if (mPrimary != null && mPrimary.getState() == Call.State.ACTIVE) {
            ...
            /// M: For VT time count. see ALPS01425992 @{
            // for voice call, start timer at once; for VT call, wait VT_MSG_START_COUNTER message to trigger.
            // Original Code:
...
//            return contactInfo.location;
    ...
    /// M: For VT time count. see ALPS01425992 @{
    /**
     * This function to get call time for the certain call.
     * @param call
     * @return
     */
    private long getCallDuration(Call call) {
        if (FeatureOptionWrapper.isSupportVT() && call.isVideoCall()) {
            if (VTTimingMode.VT_TIMING_NONE == VTUtils.checkVTTimingMode(call.getNumber())) {
                return 0;
            } else if (VTTimingMode.VT_TIMING_DEFAULT == VTUtils.checkVTTimingMode(call.getNumber())) {
    ...
    // Below implement VTListener, we only handle onVTStateChanged() to handle VT_MSG_START_COUNTER message.
    @Override
    public void onVTStateChanged(int msgVT) {
        Log.d(this, "onVTStateChanged()... msgVT: " + msgVT);
        // Here we only handle CallCard related message, other logic will be done in VTManagerLocal.java.
        // here we only care VT_MSG_START_COUNTER message, which is UI unrelated, so can skip getUi() == null check.
    ...
    @Override
    public void onContactInfoExpired(int callId) {
        Log.d(this, "onContactInfoExpired, callId = " + callId);
        int type = -1;
        Call call = null;
        if(mPrimary != null && mPrimary.getCallId() == callId) {
            type = PRIMARY;
            call = mPrimary;
        } else if(mSecondary != null && mSecondary.getCallId() == callId) {
            type = SECONDARY;
            call = mSecondary;
        } else if(mSecondaryHoldCall != null && mSecondaryHoldCall.getCallId() == callId) {
            type = SECONDARY_ONHOLD;
            call = mSecondaryHoldCall;
        } else if(mSecondaryIncomingCall != null && mSecondaryIncomingCall.getCallId() == callId) {
            type = SECONDARY_INCOMING;
            call = mSecondaryIncomingCall;
        }
        if(type != -1 && call != null) {
            startContactInfoSearch(call.getIdentification(), type, call.isIncoming());
        }
    }
}
