...
public class CallList implements IInCallScreen {
    ...
    /// M: for CrssSupp Notification && ALPS01427149@{
    // need to update contacts info when number changed because of COLP
    public void onCrssSuppServiceNumberUpdate(Context context, int callId ,String number) {
        Log.i(this, "onCrssSuppServiceNumberUpdate, callId = " + callId + ", number = " + number);
        Call call = getCall(callId);
        if(call != null) {
            call.setNumber(number);
            ContactInfoCache.getInstance(context).updateContactInfoCache(call.getIdentification());
        }
    }
    /// @}
}
