...
public class SmsUtil {
	...
	private static final String sms_thread_id = Sms.THREAD_ID;	//会话的thread_id  同一个电话号码的所有短信此id都相同
	...
	public static ArrayList<Message_Thread> allMessage_Thread(Context context)
	{
		...
		String[] projection = new String[]{
				Sms.THREAD_ID,Sms.DATE,Sms.BODY,Sms.READ,Sms.ADDRESS
		};
		String selection = null;
		String[] selectionArgs = null;
		String sortOrder = Sms.DATE+" desc";
		Cursor cursor = context.getContentResolver().query(sms_uri, projection, selection, selectionArgs, sortOrder);
		while(cursor.moveToNext())
		{
			...
			//<------------  通过phone查找头像、姓名 --------------->
			 Uri uriNumber2Contacts = Uri.parse("content://com.android.contacts/data/phones/filter/" + phone);  
            ...
            Message_Thread thread = new Message_Thread(thread_id, thread_date, 0,
				thread_snippet, thread_read, name, phone, photo);
            list.add(thread);
	...
	@SuppressLint("InlinedApi") public static ArrayList<Message_> findMessageByThread_id(Context context,int thread_id)
	{
...
//			String message_send_ok = cursor.getString(cursor.getColumnIndex("message_send_ok"));/
			Message_ msg = new Message_(message_id, message_phone,message_content,
					message_see, message_state, message_time,
					"", "", "");
			list.add(msg);
		}
