...
public class ImportVCardActivity extends Activity {
    ...
    private class ImportRequestConnection implements ServiceConnection {
        ...
        /** M: ALPS01022668 first vcard import finished, second vcard still caching, can't stop service in this case. @{ */
    ...
    private class VCardCacheThread extends Thread
        ...
        public void run() {
            ...
            try {
                ...
                if (mSource != null) {
                    ...
                    for (Uri sourceUri : mSourceUris) {
                        ...
                        try {
                            ...
                            showFailureNotification(reason , displayName , cache_index);
                            continue;
                            /*
                ...
                ///M:M:fix ALPS00874343,if the vcf file is not exist,toast error msg.
                LogUtils.w(LOG_TAG, "[run] the vcf file is not found when import! exception:"+e);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        MtkToast.toast(getApplicationContext(), R.string.import_failure_no_vcard_file);
                    }
                });
                ///@}
            }catch (IOException e) {
    ...
    protected Dialog onCreateDialog(int resId, Bundle bundle) {
        switch (resId) {
            ...
            case R.id.dialog_searching_vcard: {
                ...
                /* Change for CR ALPS00683398.
                 * This will be called from Activity.restoreManagedDialogs() when the contacts process has been killed;
        ...
        if (volumes != null) {
            for (StorageVolume volume : volumes) {
                if (path != null && volume.getPath().equals(path)) {
                    mVolumeName = volume.getDescription(this);
                    Log.i(LOG_TAG, "[doScanExternalStorageAndImportVCard] mVolumeName : " + mVolumeName);
                }
            }
        }
        /*
     ...
     */
    void showFailureNotification(String reason, String displayName, int fileIndex) {

        final String displayTitle = getString(R.string.vcard_import_failed_v2) + " " + displayName;
        ...
        final Notification notification = NotificationImportExportListener.constructImportFailureNotification(
                ImportVCardActivity.this, reason, displayTitle);
        notificationManager.notify(NotificationImportExportListener.FAILURE_NOTIFICATION_TAG, fileIndex, notification);
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                // /M: to modify the toast last a long time issue @{
                // Toast.makeText(ImportVCardActivity.this,
                // getString(R.string.vcard_import_failed),
                // Toast.LENGTH_LONG).show();
                MtkToast.toast(ImportVCardActivity.this, displayTitle, Toast.LENGTH_LONG);
                // /@}
            }
        });
    }
