...
public class SearchableSource extends AbstractSource {
    ...
    private boolean canRead(Uri uri) {
        ...
        if (pathPermissions == null || pathPermissions.length == 0) {
            ...
            Log.d(TAG, "Missing " + readPermission);
        ...
        Log.d(TAG, "Missing " + readPermission + " and no path permission applies");
    ...
    public Drawable getSourceIcon() {
        if (mSourceIcon == null) {
            ...
            if (icon == null) {
                icon = getContext().getResources().getDrawable(R.drawable.corpus_icon_default_mirrored);
            }
            // Can't share Drawable instances, save constant state instead.
            mSourceIcon = (icon != null) ? icon.getConstantState() : null; //NOPMD
            // Optimization, return the Drawable the first time
            return icon;
        }

        return mSourceIcon.newDrawable();
    }
    ...
    public Uri getSourceIconUri() {
        ...
        if (resourceId == 0) {
            return Util.getResourceUri(getContext(), R.drawable.corpus_icon_default_mirrored);
        } else {
    ...
    /// M: our own query method accounting query from ApplicationsProvider
    private static Cursor query(Context context, String source, Uri uri,
            String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        Cursor c = context.getContentResolver().query(uri, projection, selection,
                selectionArgs, sortOrder);
        Log.d(TAG, "query(), got cursor from '" + source + "': " + c);

        /// M: This is for ApplicationsProvider on LCA projects. Because
        // ApplicationsProvider is running in android.process.acore, but on
        // LCA devices acore is easily to be killed, when you enter QSB
        // to search some apps but acore is dead, system will start acore,
        // ApplicationsProvider will build its database in memory, this will
        // cost some time. In order to avoid shortcut refresh problem,
        // ApplicationsProvider will return a special cursor when it is building
        // its database. So we will check this and if it is building database
        // we will sleep 100ms and then try again until we do not get the special
        // cursor.
        while (true) {
            // cursor is NOT null
            if (c != null) {
                // rows in cursor is 0, check if target provider is preparing
                if (c.getCount() == 0 && c.getColumnIndex("ongoing") != -1) {
                    Log.d(TAG, "query(), target provider is preparing...");

                    // it will not be used, so close it
                    c.close();
                    try {
                        Thread.sleep(100);
                    ...
                    // normal case, got contents from provider
                    // c.getCount() != 0, or c.getCount() == 0 but it is a
                    // normal cursor
                    Log.d(TAG, "query(), Got cursor from: " + source +
                            ", rows in cursor: " + c.getCount());
                    break;
                }
            }

            // cursor is null, return
            Log.d(TAG, "query(), problem ocurred when query, null cursor got for: " + source);
            break;
        }
    ...
    private Cursor getSuggestions(Context context, SearchableInfo searchable, String query,
        ...
        Cursor c = query(context, mName, uri, null, selection, selArgs, null);

        Log.d(TAG, "Got cursor from " + mName + ": " + c + ", rows got: " + c.getCount());
        return c;
    }
    ...
    private static Cursor getValidationCursor(Context context, SearchableInfo searchable,
        ...
        Log.d(TAG, "Requesting refresh shortcut from: " + uri);
        // finally, make the query
        return query(context, authority, uri, null, null, null, null);
    }
