...
public abstract class SearchActivityView extends RelativeLayout {
    ...
    // when put the search input view in ActionBar, height of ActionBar is smaller
    // when landscape than portrait, so input text is scaled smaller than it is
    // in portrait, the value is a "try" value
    private static final float TEXT_SIZE_LANDSCAPE = 0.81f;
    ...
    protected void onFinishInflate() {
        ...
        /// M:
        mQueryTextView = (QueryTextView) mActionBarView.findViewById(R.id.search_src_text);
        ...
        /// M: Search button is moved to ActionBar
        mSearchCloseButton = (ImageButton) mActionBarView.findViewById(R.id.search_close_btn);
        /// M:
        mSearchGoButton = (ImageButton) mActionBarView.findViewById(R.id.search_go_btn);
        mVoiceSearchButton = (ImageButton) mActionBarView.findViewById(R.id.search_voice_btn);
        ...
        // when in landscape on phone, scale the size of input text into smaller
        if ("default".equals(SystemProperties.get("ro.build.characteristics"))
                && Configuration.ORIENTATION_LANDSCAPE == getResources().getConfiguration().orientation) {
            mQueryTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                    (int) mQueryTextView.getTextSize() * TEXT_SIZE_LANDSCAPE);
        }
