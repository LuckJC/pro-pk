...
public class ShortcutRepositoryImplLog implements ShortcutRepository {
    ...
    /// M: 32 -> 33: update from JB to KK
    private static final int DB_VERSION = 33;
    /// M: call log search is in com.android.contacts on JB,
    // but in com.android.dialer on KK, so update it
    private static final String JB_CALL_LOG = "enable_corpus_com.android.contacts/com.mediatek.contacts.activities.CallLogSearchResultActivity";
    private static final String KK_CALL_LOG = "enable_corpus_com.android.dialer/com.mediatek.dialer.activities.CallLogSearchResultActivity";
    ...
    // multiple hits on the same suggestion(shortcut) at different times will insert
    // multiple records in the 'clicklog' table, so group by intent_key will divide
    // all the records into each suggestion(shortcut)'s group
    ...
    private void buildShortcutQueries() {
        ...
        /// M: init mShortcutNoQuery, order by click count and latest hit time @{
        // first order by clicked count, see GROUP_BY's comment
        orderBy = "COUNT(" + ClickLog.hit_time.fullName + ")" + " DESC";
        // if order by clicked count failed, order by latest hit time
        orderBy += ", " + LAST_HIT_TIME_EXPR + " DESC";
...
// -------------------------- END TABLES --------------------------
    ...
    private static class DbOpenHelper extends SQLiteOpenHelper {
        ...
        /// M: update call log setting when upgrade from JB to KK
        private void updateCallLogSetting(int oldVersion, int newVersion) {
            // the upgrade is from JB to KK
            if (oldVersion <= 32 && newVersion >= 33) {
                SharedPreferences sp = QsbApplication.get(mContext).getContext()
                        .getSharedPreferences(SearchSettingsImpl.PREFERENCES_NAME, Context.MODE_PRIVATE);
                Editor editor = sp.edit();

                // we just update call log when it is enabled before upgrade,
                // otherwise it will be created when user clicks it in searchable items setting
                if (sp.getBoolean(JB_CALL_LOG, false)) {
                    editor.putBoolean(KK_CALL_LOG, true).commit();
                    QsbApplication.get(mContext).getSettings().broadcastSettingsChanged();
                    Log.i(TAG, "Update call log setting.");
                }

                // remove the call log search setting from JB
                editor.remove(JB_CALL_LOG).commit();
            }
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            /// M: update call log search setting
            updateCallLogSetting(oldVersion, newVersion);

            Log.i(TAG, "Upgrading complete.");
        }
