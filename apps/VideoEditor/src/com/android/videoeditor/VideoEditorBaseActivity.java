...
public abstract class VideoEditorBaseActivity extends Activity {
    ...
    private class ServiceListener extends ApiServiceListener {
        ...
        public void onVideoEditorLoaded(String projectPath, VideoEditorProject project,
            ...
            if (exception != null || project == null) {
                ///M: For unmount sdcard when load, this case will return a exception, but the dialog is not proper @{
                // We need to finish this activity for  the project path is not exists.
                if ((mProjectPath != null) && !new File(mProjectPath).exists()) {
                    MtkLog.d(TAG, "onVideoEditorLoaded() project not exists, finish activity");
                    finish();
                    return;
                } 
                /// @}
                
                mProjectPath = null;// only show this dialog when other exception to delete the project.
        ...
        public void onEffectRemoved(String projectPath, String effectId, String mediaItemId,
            ...
            if (exception != null) {
                ...
                getMediaLayout().resetGeneratingEffectProgress(mediaItemId);
            }
    ...
    public void onDestroy() {
        MtkLog.d(TAG, "onDestory() mProjectPath:" + mProjectPath);
        super.onDestroy();
        ...
        if (mProjectPath != null) {
            if (!isChangingConfigurations() || (MtkFeatureSwitcher.isCancelGeneratingEnabled() && !isExporting())) {
                MtkLog.d(TAG, "onDestory() release");
