...
public class OverlayTitleEditor extends NoSearchActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (attributes != null) {
            ...
            /// M: ALPS01267449 Move cursor to end for better UX
            String title = MovieOverlay.getTitle(attributes);
            String subtitle = MovieOverlay.getSubtitle(attributes);
            if (title != null && !title.isEmpty()) {
                EditText titleET = (EditText) mTitleView;
                titleET.setSelection(title.length());
            }
            if (subtitle != null && !subtitle.isEmpty()) {
                EditText subtitleET = (EditText) mSubtitleView;
                subtitleET.setSelection(subtitle.length());
            }
            /// @}
        } else {
    ...
    private void updatePreviewImage() {
        ...
        /// M: ALPS01426486 to keep the background image overlay_generic.bmp the same
        // size with mOverlayImageView for different density
        // This case is w360 port but 1.33 density, the real overlay_generic.bmp is 255*144,
        // but showed is 227*128, it is smaller than mOverlayImageView(255*144) @
        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(mPreviewWidth, mPreviewHeight);
        ImageView overlayGeneric = (ImageView) findViewById(R.id.overlay_generic);
        overlayGeneric.setLayoutParams(params);
        MtkLog.d("OverlayTitleEditor", "mOverlayImageView w:" + mPreviewWidth + ", h:" + mPreviewHeight);
        /// @}
    }
