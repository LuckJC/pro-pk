...
public class MediaLinearLayout extends LinearLayout implements ProjectStatePuller {
    ...
    private class MediaItemActionModeCallback implements ActionMode.Callback {
        ...
        public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
            ...
            ///M: ALPS00929931 Disable delete media item while adding audio @{
            MtkLog.d(TAG, "onPrepareActionMode() ");
            final MenuItem dmimi = menu.findItem(R.id.action_delete_media_item);
            dmimi.setEnabled(enable);
            /// @}
        ...
        switch (event.getAction()) {
            case DragEvent.ACTION_DRAG_STARTED: {
                ...
                if(mDragMediaItemId == null) {
                    Log.d(TAG, "[ACTION_DRAG_STARTED] mDragMediaItemId = null");
                }
            ...
            case DragEvent.ACTION_DRAG_ENDED: {
                ...
                Log.d(TAG, "[ACTION_DRAG_ENDED] mDragMediaItemId = null");
                mDropIndex = -1;
    ...
    private long getLeftDropPosition() {
        ...
        for (int i = 0; i < mediaItemsCount; i++) {
            ...
            if (timeMs > beginMs && timeMs <= endMs) {
                ...
                if(mDragMediaItemId == null) {
                    Log.d(TAG, "[getLeftDropPosition] mDragMediaItemId is null");
                    return timeMs;
                }
    ...
    private long getRightDropPosition() {
        ...
        for (int i = 0; i < mediaItemsCount; i++) {
            ...
            if (timeMs >= beginMs && timeMs < endMs) {
                if(mDragMediaItemId == null) {
                    Log.d(TAG, "[getRightDropPosition] mDragMediaItemId is null");
                    return timeMs;
                }
                if (!mDragMediaItemId.equals(mediaItem.getId())) {
                    if (i < mediaItemsCount - 1) {
    ...
    public void setOnResume() {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            if (tag != null) {
                if (tag instanceof MovieMediaItem) {
                    ((MediaItemView) view).setOnResume();
                }
            }
        }
    }

    /// M: Reset the progress when remove error kenburns effect @{
    public void resetGeneratingEffectProgress(String mediaItemId) {
        final int childrenCount = getChildCount();
        for (int i = 0; i < childrenCount; i++) {
            final View childView = getChildAt(i);
            final Object tag = childView.getTag();
            if (tag != null && tag instanceof MovieMediaItem) {
                final MovieMediaItem mi = (MovieMediaItem) tag;
                if (mediaItemId.equals(mi.getId())) {
                    if (childView instanceof MediaItemView) {
                        MtkLog.d(TAG, "remove the effect " + mediaItemId);
                        ((MediaItemView) childView).resetGeneratingEffectProgress();
                        break;
                    }
                }
