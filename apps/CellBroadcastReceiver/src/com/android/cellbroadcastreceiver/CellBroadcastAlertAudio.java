...
public class CellBroadcastAlertAudio extends Service implements TextToSpeech.OnInitListener,
    ...
    private static final String TAG = "[ETWS]CellBroadcastAlertAudio";
    ...
    private boolean mHasTempVolume = false;
    private float mTempVolume = 1.0f;
    // Internal messages
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mSimId = intent.getIntExtra("sim_id", -1);
        }
        ...
        if (mEnableAudio) {
            ...
            try {
                ...
                if (mTelephonyManager.getCallState()
                    ...
                    float volume = 1.0f;
                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                        volume = prefs.getFloat(CellBroadcastSettings.KEY_ALERT_SOUND_VOLUME+"_"+mSimId, 1.0f);
                    } else {
                        volume = prefs.getFloat(CellBroadcastSettings.KEY_ALERT_SOUND_VOLUME, 1.0f);
                    }
                    if (mHasTempVolume) {
                        mHasTempVolume = false;
                        volume = mTempVolume;
                    }
                    mMediaPlayer.setVolume(volume, volume);
                    Xlog.d(TAG, "volume scale:"+volume);
                }
