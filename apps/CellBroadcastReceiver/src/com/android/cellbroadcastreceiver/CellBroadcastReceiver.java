...
public class CellBroadcastReceiver extends BroadcastReceiver {
    ...
    protected void onReceiveWithPrivilege(Context context, Intent intent, boolean privileged) {
        ...
        if (SMS_STATE_CHANGED_ACTION.equals(action)) {
            ...
            if (DBG) {
                log("SMS_STATE_CHANGED_ACTION " + isReady + " " + slotId);
            }
            if (isReady) {
        ...
        } else if (Intent.SIM_SETTINGS_INFO_CHANGED.equals(action)) {
            int simId = (int)intent.getLongExtra("simid", -1);
            CellBroadcastListItem.simInfoMap.remove(simId);
            CellBroadcastListItem.getSimString(context, simId);
    ...
    static void startConfigServiceGemini(Context context, int simId) {
        if (phoneIsCdma()) {
    ...
    private void configChannelGemini(Context context) {
        ...
        if (si != null) {
            ...
            startConfigServiceGemini(context, simId);
            //}
        }
