...
public class CellBroadcastAlertService extends Service {
    ...
    private void handleCellBroadcastIntent(Intent intent) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            SIMInfo si = SIMInfo.getSIMInfoBySlot(this.getApplicationContext(), slotId);
            if (si != null) {
                Xlog.d(TAG, "sim_id from intent is:"+si.mSimId);
                simId = (int)si.mSimId;
            }
        }
    ...
    private boolean isMessageEnabledByUser(CellBroadcastMessage message) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
            return isMessageEnabledByUserGemini(message);
        }
    ...
    private void openEmergencyAlertNotification(CellBroadcastMessage message) {
        ...
        if (message.isCmasMessage()) {
            ...
            duration = Integer.parseInt(prefs.getString(CellBroadcastSettings.KEY_ALERT_SOUND_DURATION+"_"+message.getSimId(),
                                        CellBroadcastSettings.ALERT_SOUND_DEFAULT_DURATION)) * 1000;
            vibrate = prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ALERT_VIBRATE + "_" + message.getSimId(), true);
        } else {
            duration = Integer.parseInt(prefs.getString(
                    CellBroadcastSettings.KEY_ALERT_SOUND_DURATION,
                    CellBroadcastSettings.ALERT_SOUND_DEFAULT_DURATION)) * 1000;
            vibrate = prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ALERT_VIBRATE, true);
        }
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            audioIntent.putExtra("sim_id", message.getSimId());
        }
        ...
        if (message.isEtwsMessage()) {
            ...
            audioIntent.putExtra(CellBroadcastAlertAudio.ALERT_AUDIO_VIBRATE_EXTRA, vibrate);
        ...
        // add for gemini
        boolean enableAlertSpeech;
        if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
            enableAlertSpeech = prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ALERT_SPEECH
                    + "_" + message.getSimId(), CellBroadcastSettings.ENABLE_ALERT_SPEECH_DEFAULT);
        } else {
            enableAlertSpeech = prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ALERT_SPEECH,
                    CellBroadcastSettings.ENABLE_ALERT_SPEECH_DEFAULT);
        }
        if (enableAlertSpeech) {
    ...
    private boolean isMessageEnabledByUserGemini(CellBroadcastMessage message) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
        if (message.isEtwsTestMessage()) {
            return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ETWS_TEST_ALERTS
                    + "_" + message.getSimId(), CellBroadcastSettings.ENABLE_ETWS_TEST_ALERTS_DEFAULT);
        }

        if (message.isCmasMessage()) {
            switch (message.getCmasMessageClass()) {
                case SmsCbCmasInfo.CMAS_CLASS_EXTREME_THREAT:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_EXTREME_THREAT_ALERTS+"_"+message.getSimId(),
                                        CellBroadcastSettings.ENABLE_CMAS_EXTREME_THREAT_ALERTS_DEFAULT);
                case SmsCbCmasInfo.CMAS_CLASS_SEVERE_THREAT:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_SEVERE_THREAT_ALERTS+"_"+message.getSimId(),
                                        CellBroadcastSettings.ENABLE_CMAS_SEVERE_THREAT_ALERTS_DEFAULT);
                case SmsCbCmasInfo.CMAS_CLASS_CHILD_ABDUCTION_EMERGENCY:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_AMBER_ALERTS+"_"+message.getSimId(),
                                        CellBroadcastSettings.ENABLE_CMAS_AMBER_ALERTS_DEFAULT);
                case SmsCbCmasInfo.CMAS_CLASS_REQUIRED_MONTHLY_TEST:
                case SmsCbCmasInfo.CMAS_CLASS_CMAS_EXERCISE:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_TEST_ALERTS+"_"+message.getSimId(),
                                        CellBroadcastSettings.ENABLE_CMAS_TEST_ALERTS_DEFAULT);
                default:
                    return true;    // presidential-level CMAS alerts are always enabled
            }
        }
