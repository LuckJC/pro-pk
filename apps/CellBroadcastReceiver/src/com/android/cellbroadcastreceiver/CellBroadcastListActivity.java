    ...
    public static class CursorLoaderListFragment extends ListFragment
        ...
        public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                List<SIMInfo> listSimInfo = SIMInfo.getInsertedSIMList(this.getActivity());
                if (listSimInfo == null || listSimInfo.isEmpty()) {
                    MenuItem item = menu.findItem(MENU_PREFERENCES);
                ...
                switch (item.getItemId()) {
                    case MENU_DELETE:
                        confirmDeleteThread(cursor.getLong(cursor.getColumnIndexOrThrow(
                                Telephony.CellBroadcasts.QUERY_COLUMNS[0])));
                        break;
        ...
        public boolean onOptionsItemSelected(MenuItem item) {
            switch(item.getItemId()) {
                ...
                case MENU_PREFERENCES:
                    // add for gemini
                    if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                        Intent it = new Intent(this.getActivity(), SelectCardActivity.class);
                        startActivity(it);
                        break;
                    }
