...
public class CellBroadcastAlertReminder extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // add for gemini
        int simId = intent.getIntExtra("sim_id", 0);
        if (queueAlertReminder(this, false, simId)) {
            return START_STICKY;
        } else {
     ...
     */
    static boolean queueAlertReminder(Context context, boolean firstTime, int simId) {
        // Stop any alert reminder sound and cancel any previously queued reminders.
        cancelAlertReminder();
        Log.d("LY"," Uri:" + Uri.parse(""));
        log("queueAlertReminder firstTime " + firstTime + " simId " + simId);
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        String prefStr;
        // add for gemini
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            prefStr = prefs.getString(CellBroadcastSettings.KEY_ALERT_REMINDER_INTERVAL + "_" + simId, null);
        } else {
            prefStr = prefs.getString(CellBroadcastSettings.KEY_ALERT_REMINDER_INTERVAL, null);
        }
