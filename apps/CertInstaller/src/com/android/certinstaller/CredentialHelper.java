    ...
    private void parseCert(byte[] bytes) {
        ...
        try {
            ...
            if (isCa(cert)) {
                ...
                /// M: add for WAPI EC algo cannot getPublicKey in AndroidOpenSSL provider @{
                String [] certlines = cert.toString().split("\n");

                for (String certline:certlines){
                    //Log.d(TAG, "certline : " + certline);
                    if (certline.contains("Public Key Algorithm:") &&
                        certline.contains("id-ecPublicKey")) {
                        mEcc = true;
                        Log.i(TAG, "User certificate in EC");
                        mWapiUserCert = bytes; 
                        ...
                        if (wapiUserCert.contains("-----BEGIN EC PRIVATE KEY-----") &&
                            ...
                            mHasPkey = true;
                            Log.i(TAG, "User certificate includes EC private key");
                        }
                        break;
                    }
                }
                Log.d(TAG, "Certificate info : mECC : " + mEcc + " mHasPkey : " + mHasPkey );
                /// @}
            }
