...
public class MediaPlaybackService extends Service {
    ...
    public static final int MEDIA_ERROR_MUSICFX_DIED = -22;//0x800000FF;
    ...
    /// M: add for indentify whether need to notify RemoteControl when META_CHANGE come
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case TRACK_WENT_TO_NEXT:
                    ...
                    if (mPlayPos >= 0 && mPlayListLen > mPlayPos) {
                        mCursor = getCursorForId(mPlayList[mPlayPos]);
                        if(mCursor == null){
                            MusicLogUtils.d(TAG, "switch to next song fail because mCursor is null");
                            return;
                        }
                    ...
                    // handles fade-in
                     switch (msg.arg1) {
                        ...
                        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                             ...
                             * @{
                             */
                            if(MusicFeatureOption.IS_LCA_RAM_OPTIMIZE){
                ...
                case REMOVE_TRACK:
                    int songId = msg.arg1;
                    MusicLogUtils.v(TAG, "received messsage : REMOVE_TRACK, songId = " + songId);
                    removeTrack(songId);
                    break;
                  /// @}
                default:
    ...
    public void closeExternalStorageFiles(String storagePath) {
		MusicLogUtils.d(TAG, "closeExternalStorageFiles() storagePath = "
    ...
    private void notifyChange(String what) {
        ...
        long currentId = getAudioId();
        i.putExtra("id", Long.valueOf(currentId));
        ...
        /// @}
            ...
            if (what.equals(PLAYSTATE_CHANGED)) {
                ...
                /// M: when you change the playing song,it may have two META_CHANGED message come,
                /// we only need notify at the first time @{
                if (currentId != mPreviousId) {
    ...
    public void enqueue(long [] list, int action) {
        synchronized(this) {
			MusicLogUtils.d(TAG, "enqueue() list = "
					+ ((list != null) ? list : "null") + ",action = " + action
    ...
    public void open(long [] list, int position) {
        synchronized (this) {
			...
			MusicLogUtils.d(TAG, "open() list = " + ((list != null) ? list : "null") + ",position = "
					+ position + ",newlist = " + newlist);
            if (newlist) {
    ...
    public void moveQueueItem(int index1, int index2) {
		MusicLogUtils.d(TAG, "moveQueueItem() index1 = " + index1
				+ ",index2 = " + index2);
        synchronized (this) {
    ...
    private Cursor getCursorForId(long lid) {
        ...
        /** M: 
         * Catch the IllegalStateException while do query during 
         * fast sd card plug in and plug out.@{ 
         **/
        Cursor c = null;
        try {
            ...
            MusicLogUtils.d(TAG, "getCursorForId()-IllegalStateException");
        }
        /**@}**/
        return c;
    }
    ...
    public boolean open(String path) {
        synchronized (this) {
            ...
            mFileToPlay = path;
            /// M: reset the status about player, track and playlist  @{
        ...
        try {
            if (isPodcast()) {
                ...
                if (mCursor != null) {
                    ...
                    MusicLogUtils.d(TAG, "saveBookmarkIfNeeded() fail caused by null mCursor");
                }
            }
    ...
    private void doAutoShuffleUpdate() {
		...
		MusicLogUtils.d(TAG, "doAutoShuffleUpdate() notify = " + notify);
        if (notify) {
            notifyChange(QUEUE_CHANGED);
        }
    }
    ...
    public int removeTracks(int first, int last) {
		MusicLogUtils.d(TAG, "removeTracks()-first = " + first + ",last = "
				+ last);
    ...
    private void removeInShuffleMode(int first, int last) {
        MusicLogUtils.d(TAG, "removeInShuffleMode f = " + first + ", l = " + last);
    ...
    private class MultiPlayer {
        ...
        public void setNextDataSource(String path) {
            ...
            /**
             * M: Gets current player duration from database in stead of quering from MediaPlayer instance.
             * To avoid the ANR that mediaplayer instance haven't prepared while calling getDuration().@{
             */
            long currentPlayerDuration = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DURATION));
            MusicLogUtils.i(TAG, "setNextDataSource: database duration = " + currentPlayerDuration);
            cursor.close();
            cursor = null;
            ...
            if (currentPlayerDuration < 1000) {
                MusicLogUtils.w(TAG, "Discard setNextDataSource because the current audio is so short.");
                return;
            }
            /**
             * @}
            ...
            cursor = getCursorForId(mPlayList[mNextPlayPos]);
            if (cursor == null) {
                MusicLogUtils.w(TAG, "setNextDataSource with null cursor!");
                return;
            }
        ...
        MediaPlayer.OnCompletionListener listener = new MediaPlayer.OnCompletionListener() {
            public void onCompletion(MediaPlayer mp) {
                if (mp == mCurrentMediaPlayer && mNextMediaPlayer != null && mNextMediaPlayer.gettPlayerReadyFlag() == true) {
                    ...
                    mNextMediaPlayer = null;
                    /// M: fix when NextPlayer is not prepared,case we don't setNextPlayer,
                    /// so when the currentPlayer completed,native will auto check 
                    /// whether the nextPlayer is not null,if not,will not start,ALPS01298224 @{
                    if (!isPlaying()) {
                        MusicLogUtils.d(TAG, "isPlaying()==false");
                        mCurrentMediaPlayer.start();
                    }
                    /// @}
                    if (mNextPlayPos != -1) {
                    ...
                    if(mp == mCurrentMediaPlayer && mNextMediaPlayer != null && mNextMediaPlayer.gettPlayerReadyFlag() == false){
                        mNextMediaPlayer.release();
                        mNextMediaPlayer = null;
                    }
                    ...
                    /// if the last song of playlist end,no need to save mPreAudioId
                    // avoid playing the last song again,will not getArtWork(see the play())
                    if (mPlayPos < mPlayListLen - 1) {
                    ...
                    MusicLogUtils.d(TAG, "mIsSupposedToBePlaying:" + mIsSupposedToBePlaying);
                    if (mIsSupposedToBePlaying) {
        ...
        private void handlePlaySongFail(MediaPlayer mediaPlayer) {
            ...
            if (mOpenFailedCounter == OPEN_FAILED_MAX_COUNT) {
            ...
            public void onPrepared(MediaPlayer mp) {
                synchronized (MediaPlaybackService.this) {
                    ...
                    if (!mp.equals(mCurrentMediaPlayer)) {
                        ...
                        mCurrentMediaPlayer.setNextMediaPlayer(mNextMediaPlayer);
                        mNextMediaPlayer.setPlayerReadyFlag(true);
                        /// M: if the current volume is min fade down volume, set next player with this volume
                        final float minFadeDownVolume = .2f;
                        if (Math.abs(mCurrentVolume - minFadeDownVolume) < 0.001) {
                            mNextMediaPlayer.setVolume(mCurrentVolume, mCurrentVolume);
    ...
    static class CompatMediaPlayer extends MediaPlayer implements OnCompletionListener {
        ...
        public void setPlayerReadyFlag(boolean isPlayerReady){
            mIsPlayerReady = isPlayerReady;
        }
        
        public boolean gettPlayerReadyFlag(){
            return mIsPlayerReady;
        }
    }
    ...
    private void reloadQueueAfterScan() {
        ...
        if (hasCard && !mIsReloadSuccess
        	...
        	MusicLogUtils.d(TAG, "reloadQueueAfterScan()-success");
            reloadQueue();
            notifyChange(QUEUE_CHANGED);
            notifyChange(META_CHANGED);
        }
    ...
    void quitContentObserverThread() {
        ...
        MusicLogUtils.i(TAG, "Quit ContentObserverThread when service will be destroy: isQuitSuccuss = " + isQuitSuccuss);
    }
    ...
    private boolean checkDrm(Cursor cursor) {
        if (!MusicFeatureOption.IS_SUPPORT_DRM) {
            MusicLogUtils.w(TAG, "checkDrm(): MusicFeatureOption.IS_SUPPORT_DRM was disabled");
            return true;
        }
        if (MusicFeatureOption.IS_SUPPORT_DRM && cursor != null) {
            int isDrm = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.IS_DRM));
            int drmMethod = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DRM_METHOD));
            MusicLogUtils.v(TAG, "checkDrm(),isDrm=" + isDrm + ", drmMethod=" + drmMethod);
