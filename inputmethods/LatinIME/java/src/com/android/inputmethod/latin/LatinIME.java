...
///M: import mediatek classes for frammar checker on thai @{
import com.mediatek.inputmethod.latin.grammarcheck.MediaTekGrammarChecker;
// @}
/**
...
public class LatinIME extends InputMethodService implements KeyboardActionListener,
    ...
    ///M: Mediatek grammar checker @{
    private MediaTekGrammarChecker mGrammarChecker;
    // @}
    ...
    /// @}
    /// M: Flag indicating whether grammar checker has been enabled by build.
    //     For more information, please refer to file
    //     <code>packages/inputmethods/LatinIME/java/res/values/mediatek_advanced_features.xml<\code>
    private static boolean mIsMediatekGrammarCheckerEnabled;
    // @}
    /// M: This two flag is used to save the Hardkeyboard state of the system. @{
    ///    use them in onCinfigurationChange.
    private int mConfigKeyboard = 0;
    private int mConfigHardKeyboard = 0;
    /// @}
    public static final class UIHandler extends StaticInnerHandlerWrapper<LatinIME> {
        ...
        public void onStartInput(final EditorInfo editorInfo, final boolean restarting) {
            if (hasMessages(MSG_PENDING_IMS_CALLBACK)) {
                ...
                /// M: Init the selections, the old selections maybe out-sync with the target TextView.
                ///    and we should avoid the startinput called from configuration change, because
                ///    that editorInfo was out of date. @{
                else {
                    final LatinIME latinIme = getOuterInstance();
                    latinIme.mLastSelectionStart = editorInfo.initialSelStart;
                    latinIme.mLastSelectionEnd = editorInfo.initialSelEnd;
                }
                /// @}
                final LatinIME latinIme = getOuterInstance();
        ...
        public void onStartInputView(final EditorInfo editorInfo, final boolean restarting) {
          ...
          ///M : Draw latinime object out here for using later @{
          final LatinIME latinIme = getOuterInstance();
          // @}

          ///M : There is a fault at the original design by google, each time keyboard is set
          //     within method OnStartInputView, however keyboard view is inflated within method
          //     OnCreateInputView which is called just a bit before OnStartInputView, this will
          //     bring us trouble when onStartInputView is bypassed here, this brings the result
          //     there is no keyboard related to keyboard view, and an empty canvas is drawn on
          //     screen. @{
          if (hasMessages(MSG_PENDING_IMS_CALLBACK)
                    && KeyboardId.equivalentEditorInfoForKeyboard(editorInfo, mAppliedEditorInfo)
                    && (latinIme.mKeyboardSwitcher.getMainKeyboardView().getKeyboard() != null)) {
          // @}
    ...
    public void onCreate() {
        ...
        ///M: Get config for grammar checker @{
        mIsMediatekGrammarCheckerEnabled = getResources().getBoolean(
                R.bool.config_mediatek_thai_grammar_checker_enabled_by_build_config);
        // @}
        /// M: init these two flags. @{
        mConfigKeyboard = getResources().getConfiguration().keyboard;
        mConfigHardKeyboard = getResources().getConfiguration().hardKeyboardHidden;
        /// @}
        ...
        /// M: When the system default input method is changed, we should dismiss the Option
        ///    Dialog, so that the New Ime keyboard can be popped by the Imms. @{
    ...
    public void onConfigurationChanged(final Configuration conf) {
        ...
        /// M: if the orientation or hard keyboard state not changed, @{
        ///    unnecessary initViews on the super.onConfigurationChanged.
        if (mDisplayOrientation == conf.orientation && mConfigKeyboard == conf.keyboard
                && mConfigHardKeyboard == conf.hardKeyboardHidden) {
            Log.d(TAG, "Ignored this configration changed of super!");
            return;
        }
        mDisplayOrientation = conf.orientation;
        mConfigKeyboard = conf.keyboard;
        mConfigHardKeyboard = conf.hardKeyboardHidden;
        /// @}
    ...
    public void onCurrentInputMethodSubtypeChanged(final InputMethodSubtype subtype) {
        ...
        ///M: Re-generate grammar checker since grammar checker is diffirent from locale to locale @{
        if (mIsMediatekGrammarCheckerEnabled) {
            mGrammarChecker = new MediaTekGrammarChecker(mSubtypeSwitcher.getCurrentSubtypeLocale());
        }
        // @}
        loadKeyboard();
    }
    ...
    private void onStartInputViewInternal(final EditorInfo editorInfo, final boolean restarting) {
        ...
        ///M: Generate grammar checker on the very first load for latinime inputview @{
        if (mIsMediatekGrammarCheckerEnabled) {
            mGrammarChecker = new MediaTekGrammarChecker(mSubtypeSwitcher.getCurrentSubtypeLocale());
        }
        // @}
        ...
        /// M: This will make the IME selections in confusion.@{
        //tryFixLyingCursorPosition();
        /// @}
        /// M: async selection position made the ime delete wrong text in the TextView, we should
        ///    fix it here. @{
        fixTheAsyncSelectionUpdate();
        /// @}
    ...
    public void onTextInput(final String rawText) {
        mConnection.beginBatchEdit();
        ///M: Grammar checking here @{
        if (mIsMediatekGrammarCheckerEnabled) {
            String strBefore = mConnection.getTextBeforeCursor(2, 0).toString();
            String strAfter = mConnection.getTextAfterCursor(2, 0).toString();

            if((rawText.length() > 0)
                && (!mGrammarChecker.isValidInput(strBefore, (char)rawText.charAt(0), strBefore.length())
              || !mGrammarChecker.isValidInput(strAfter, (char)rawText.charAt(rawText.length() - 1), 0))) {
                mConnection.endBatchEdit();
              return;
            }
        }
    ...
    private void handleCharacter(final int primaryCode, final int x,
        ...
        ///M: Grammar checking here for normal character @{
        if ((mIsMediatekGrammarCheckerEnabled) && (mConnection != null)) {
            String strBefore = mConnection.getTextBeforeCursor(2, 0).toString();
            String str       = strBefore + mConnection.getTextAfterCursor(2, 0).toString();

          if(!mGrammarChecker.isValidInput(str, (char)primaryCode, strBefore.length())) {
                return;
          }
        }
        // @}
    ...
    private boolean handleSeparator(final int primaryCode, final int x, final int y,
        ...
        ///M: Grammar checking here for separator @{
        if ((mIsMediatekGrammarCheckerEnabled) && (mConnection != null)) {
            String strBefore = mConnection.getTextBeforeCursor(2, 0).toString();
            String str = strBefore + mConnection.getTextAfterCursor(2, 0).toString();
            if(!mGrammarChecker.isValidInput(str, (char)primaryCode, strBefore.length())) {
    ...
    private void restartSuggestionsOnWordTouchedByCursor() {
        ...
        /// M: The Ap will support the completions, So Ime should't start it's suggestions.
        if (mSettings.getCurrent().isApplicationSpecifiedCompletionsOn())return;
        /// @}
            ...
            public void onClick(DialogInterface di, int position) {
                ...
                case 0:
                    final Intent intent = IntentUtils.getInputLanguageSelectionIntent(
                            ...
                            | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED
                            | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    /// M: set thie input language activity title. @{
    ...
    /// @}
    /// M: Async selection update, made latin use the wrong selection start and end
    ///    to delete the wrong text when Delete key pressed; we try to fix this error; @{
    private void fixTheAsyncSelectionUpdate(){
        final CharSequence selectedText = mConnection.getSelectedText(0);
        if (!TextUtils.isEmpty(selectedText) && (mLastSelectionEnd - mLastSelectionStart) != selectedText.length()) {
            CharSequence textBeforeCursor = mConnection.getTextBeforeCursor(Constants.EDITOR_CONTENTS_CACHE_SIZE, 0);
            CharSequence textAfterCursor = mConnection.getTextAfterCursor(Constants.EDITOR_CONTENTS_CACHE_SIZE, 0);
            if (textBeforeCursor != null && textBeforeCursor.length() == 0
                    && textAfterCursor != null && textAfterCursor.length() == 0) {
                //All The Text is Selected
                Log.w(TAG, "fixTheAsyncSelectionUpdate, all the text is selected!");
                mLastSelectionStart = 0;
                mLastSelectionEnd = selectedText.length();
            }
        }
    }
    /// @}
